...
const THEMES = [
  ["light", "Rotli", "light"],
  ["dark", "Rotli", "dark"],
  ["paper", "Paper & Charcoal", "light"],
  ["charcoal", "Paper & Charcoal", "dark"],
...
const css = ["src/brand/tokens/colors.css", "src/styles/themes.css"]
  .map((f) => `<style>${readFileSync(join(source, f), "utf8")}</style>`)
  .join("");
...
for (const [id, family, mode] of THEMES) {
  ...
  themes.push({
    ...
    mode,
    label:
      family === "Paper & Charcoal"
        ? mode === "light"
          ? "Paper"
          : "Charcoal"
        : `${family} ${mode === "light" ? "Light" : "Dark"}`,
    roles,
  });
}
await browser.close();
writeFileSync(
  out,
  JSON.stringify(
    { source: `${source.replace(homedir(), "~")}/src/styles/themes.css + src/brand/tokens/colors.css`, themes },
    null,
    1,
  ) + "\n",
);
