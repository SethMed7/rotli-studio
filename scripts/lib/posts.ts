...
export const pieceIds = (root: string) => new Set<string>(JSON.parse(readFileSync(join(root, "motion/pieces.json"), "utf8")).pieces.map((p: { id: string }) => p.id));
...
export function accountFor(url: string, accounts: Account[]): Account | null {
  ...
  return accounts.find((a) => a.match.startsWith("^") && a.match.endsWith("$") && new RegExp(a.match).test(url)) ?? null;
}
...
export function validatePosts(data: PostsFile, ids: Set<string>): string[] {
  const out: string[] = [];
  if (!Array.isArray(data.accounts) || !Array.isArray(data.posts)) return ["publish/posts.json needs `accounts` and `posts` arrays"];
  data.posts.forEach((p, i) => {
    ...
    else if (account.platform !== p.platform) out.push(`${at}: platform ${p.platform} does not match its account (${account.platform})`);
    if (typeof p?.date !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(p.date)) out.push(`${at}: date must be YYYY-MM-DD`);
    if (typeof p?.text !== "string" || p.text.length > 400) out.push(`${at}: text must be a string of at most 400 characters`);
    if (!Array.isArray(p?.pieces) || p.pieces.some((x) => !ids.has(x))) out.push(`${at}: pieces must be ids from motion/pieces.json`);
  });
  return out;
}
