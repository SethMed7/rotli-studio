...
const source =
  process.argv.slice(2).find((a) => !a.startsWith("--")) ?? process.env.ROTLI_REPO ?? join(homedir(), "rotli");
...
`,
  ...
  for (const look of spec.looks) {
    ...
    await page.goto(
      `http://127.0.0.1:${PORT}/tmp/studio-companions/index.html?pose=${look.pose}&style=${look.style}&accessory=${look.accessory}&hue=${spec.accessoryHue}`,
    );
