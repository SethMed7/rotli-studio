...
const pngs = (await page.evaluate(
  async ({ logo, P, SIZES }) => {
    ...
    const d = mg.getImageData(0, 0, M, M),
      ink = new Uint8Array(M * M),
      outside = new Uint8Array(M * M);
    for (let i = 0; i < M * M; i++) ink[i] = d.data[i * 4 + 3]! > 40 ? 1 : 0;
    let bottom = 0;
    for (let i = 0; i < M * M; i++) if (ink[i]) bottom = Math.max(bottom, Math.floor(i / M));
    const seal = bottom - 6;
    let lo = M,
      hi = 0;
    for (let y = seal - 40; y <= bottom; y++)
      for (let x = 0; x < M; x++)
        if (ink[y * M + x]) {
          lo = Math.min(lo, x);
          hi = Math.max(hi, x);
        }
    const barrier = new Uint8Array(M * M);
    for (let x = lo; x <= hi; x++) for (let t = 0; t < 4; t++) barrier[(seal + t) * M + x] = 1;
    const stack: number[] = [];
    for (let i = 0; i < M; i++) stack.push(i, (M - 1) * M + i, i * M, i * M + M - 1);
    while (stack.length) {
      const i = stack.pop()!;
      if (i < 0 || i >= M * M || outside[i] || ink[i] || barrier[i]) continue;
      outside[i] = 1;
      const x = i % M;
      stack.push(i - M, i + M);
      if (x > 0) stack.push(i - 1);
      if (x < M - 1) stack.push(i + 1);
    }
    const fill = mg.createImageData(M, M),
      [r, gg, b] = [1, 3, 5].map((k) => parseInt(P.sky!.slice(k, k + 2), 16));
    for (let i = 0; i < M * M; i++)
      if (!ink[i] && !outside[i] && Math.floor(i / M) < seal + 4) {
        fill.data[i * 4] = r!;
        fill.data[i * 4 + 1] = gg!;
        fill.data[i * 4 + 2] = b!;
        fill.data[i * 4 + 3] = 255;
    ...
    f.width = f.height = M;
    f.getContext("2d")!.putImageData(fill, 0, 0);
    const scene = (S: number, Y: number, H = 0.6) => {
      ...
      const horizon = N * H;
      g.fillStyle = P.sky!;
      g.fillRect(0, 0, N, horizon);
      g.fillStyle = P.seaDeep!;
      g.fillRect(0, horizon, N, N * 0.05);
      g.fillStyle = P.sea!;
      g.fillRect(0, horizon + N * 0.05, N, N);
      g.strokeStyle = P.seaLine!;
      g.lineWidth = N * 0.018;
      g.lineCap = "round";
      for (const [x, y, w] of [
        [0.16, 0.8, 0.14],
        ...
        g.beginPath();
        g.moveTo(N * x, N * y);
        g.quadraticCurveTo(N * (x + w / 2), N * (y - 0.018), N * (x + w), N * y);
        g.stroke();
      }
    ...
    const full = scene(1024 * 0.8, 1024 * 0.14),
      close = scene(1024 * 0.9, 1024 * 0.1, 0.52); // small icons: bigger head, higher sea so the blue still shows
    // every size, downsampled in halves for clean small icons
    return SIZES.map((size) => {
      let src: HTMLCanvasElement = size <= 48 ? close : full;
      while (src.width / 2 >= size) {
        const h = document.createElement("canvas");
        h.width = h.height = src.width / 2;
        const hg = h.getContext("2d")!;
        hg.imageSmoothingQuality = "high";
        hg.drawImage(src, 0, 0, h.width, h.height);
        src = h;
      }
      ...
      return [size, o.toDataURL("image/png")] as const;
    });
  },
  { logo, P, SIZES },
)) as [number, string][];
...
ico.forEach((e, i) => {
  const o = 6 + 16 * i;
  head.writeUInt8(e.s, o);
  head.writeUInt8(e.s, o + 1);
