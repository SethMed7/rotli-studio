...
const ROOT = join(import.meta.dir, ".."), FILE = POSTS_FILE(ROOT);
...
if (!account) { console.error(`refused: ${url} is not one of the owner's accounts in publish/posts.json`); process.exit(2); }
...
if (problems.length) { console.error(`refused: the file would not validate:\n  ${problems.join("\n  ")}`); process.exit(2); }
writeFileSync(FILE, JSON.stringify(data, null, 1) + "\n");
console.log(`${at >= 0 ? "updated" : "linked"} ${clean} (${account.platform}, ${date}${pieces.length ? `, pieces ${pieces.join(" ")}` : ""}) -> publish/posts.json`);
