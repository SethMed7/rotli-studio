...
const ROOT = join(import.meta.dir, ".."), MOTION = join(ROOT, "motion");
const DIST = join(ROOT, "site-dist"), STAGE = join(ROOT, "site-dist.staging"), PUB = join(STAGE, "public");
...
const fail = (why: string) => { rmSync(STAGE, { recursive: true, force: true }); console.error(`STOP: ${why}\nNothing was published: site-dist/ is unchanged.`); process.exit(1); };
...
let rules; try { rules = [...baseRules(), ...markerRules(ROOT, { required: !process.argv.includes("--no-markers") })]; } catch (e) { fail(String((e as Error).message)); }

// the owner's published posts must validate exactly as the helper that adds them does
{ const problems = validatePosts(readPosts(ROOT), pieceIds(ROOT)); if (problems.length) fail(`publish/posts.json does not validate:\n  ${problems.join("\n  ")}`); }

rmSync(STAGE, { recursive: true, force: true }); mkdirSync(PUB, { recursive: true });
...
const manifest = JSON.parse(readFileSync(join(MOTION, "out/manifest.json"), "utf8")) as { pieces: { id: string; slug: string; kind: string; error?: string; shots: { start: number; end: number }[]; posterFrame?: number; derive: { vertical: { frame: number }[]; slides: { frame: number }[]; single: { frame: number } } | null; video?: Video }[] };

// an incomplete snapshot is worse than none: every piece must load and every video must be rendered
{ const broken = manifest.pieces.filter((p) => p.error).map((p) => `${p.id}: ${p.error}`), unrendered = manifest.pieces.filter((p) => p.kind === "video" && !p.video).map((p) => p.id);
  if (broken.length) fail(`pieces that do not load:\n  ${broken.join("\n  ")}`);
  if (unrendered.length) fail(`videos not rendered yet (node motion/tools/studio.mjs render <id>): ${unrendered.join(" ")}`); }

// ---- the page (a meta tag marks the snapshot: no inline script, so the CSP can forbid them), script, styles
const hashName = (name: string, body: string | Uint8Array) => name.replace(/\.(\w+)$/, `.${hash(body).slice(0, 12)}.$1`);
let pageHtml = readFileSync(join(ROOT, "static/motion.html"), "utf8").replace("<head>", '<head>\n    <meta name="robots" content="noindex, nofollow" />\n    <meta name="studio-static" content="1" />');
...
const js = await bundle.outputs[0]!.text(), jsName = hashName("motion.js", js);
...
for (const f of trackedUnder("library/logo").filter((f) => /\/(_logo\.svg|favicon[^/]*)$/.test(f))) put(join(PUB, f), join(ROOT, f));
...
for (const f of trackedUnder("library/patterns")) { const hashed = hashName(f, readFileSync(join(ROOT, f))); put(join(PUB, hashed), join(ROOT, f)); cssAssets.push([`/${f}`, `/${hashed}`]); }
...
for (const f of [...trackedUnder("sound/web"), ...trackedUnder("sound/prompts"), "sound/catalog.json"]) put(join(PUB, f), join(ROOT, f));

// ---- text the pages link to: /m/… (motion room) and /s/… (studio), tracked files only
const MOTION_PUBLIC = ["pieces.json", "series.json", "README.md", "brand", "series", "workflows", "src/canvas-core", "src/hosts", "tools", "golden", "third_party"];
const STUDIO_PUBLIC = ["AGENTS.md", "ARCHITECTURE.md", "DESIGN.md", "README.md", "LICENSE", "NOTICE", "archive/README.md", "docs", "publish", "scripts", ".claude/skills", "library/companion-looks"];
for (const p of MOTION_PUBLIC) for (const f of trackedUnder(`motion/${p}`)) put(join(PUB, "m", relative("motion", f)), join(ROOT, f));
...
const collect = (x: unknown): void => { if (typeof x === "string") { if (/\.(png|jpe?g|webp)$/i.test(x)) media.add(x); } else if (x && typeof x === "object") Object.values(x).forEach(collect); };
collect(manifest);
for (const ref of media) {
  const abs = join(MOTION, ref), rel = relative(ROOT, abs);
  if (rel.startsWith("..") || !existsSync(abs)) fail(`the manifest references ${ref}, which is missing or outside the studio`);
...
await pool(vids, 3, async (p) => {
  ...
  const bytes = readFileSync(web); p.video!.bytes = bytes.length; p.video!.sha = hash(bytes).slice(0, 16);
});
...
cpSync(join(ROOT, "deploy/Dockerfile"), join(STAGE, "Dockerfile")); cpSync(join(ROOT, "deploy/Caddyfile"), join(STAGE, "Caddyfile"));
...
const all = walk(STAGE), hits: string[] = [];
let texts = 0;
for (const f of all) {
...
rmSync(DIST, { recursive: true, force: true }); renameSync(STAGE, DIST);
const size = all.reduce((a, f) => a + statSync(f.replace(STAGE, DIST)).size, 0);
console.log(`site-dist: ${all.length} files · ${(size / 1e6).toFixed(0)} MB · privacy scan clean over ${texts} text files -> ${relative(ROOT, DIST)}/`);
