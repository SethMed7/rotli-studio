...
async function piece(id: string) {
  ...
  const sections: [string, string][] = [
    ...
    ...(p.prompt || main_?.prompt ? [["prompt", "Prompt"] as [string, string]] : []),
      ...
      .then(
        (t) =>
          (body("prompt")!.innerHTML =
            `<p class="muted">Generated by <code>tools/${st ? "study-prompt" : "brief-to-prompt"}.mjs</code> from the brief: the ${st ? "study rules" : "preamble that worked"}, plus this ${st ? "study" : "episode"}. <a class="link" href="${m(promptFile)}" target="_blank">${esc(promptFile)}</a></p><div class="doc">${embedded(markdown(t))}</div>`),
      )
...
async function docPage(title: string, intro: string, docs: Doc[], active?: string) {
  ...
  $("#doc").innerHTML =
    `<p class="doc-path"><code>${esc(current.path.replace(/^skill:(\w+):/, "$1 skill: "))}</code>${url ? ` · <a class="link" href="${url}" target="_blank">raw</a>` : ""}</p>${/(\.(json|ts|mjs|py)|LICENSE|NOTICE)$/.test(current.path) ? `<pre class="code">${esc(body)}</pre>` : embedded(markdown(body))}`;
}
