...
function load(): Promise<void> {
  loading ??= (async () => {
    ...
    await Promise.all(
      catalog.sounds.map(async (s) =>
        buffers.set(s.id, await ctx!.decodeAudioData(await (await fetch(`/${s.file}`)).arrayBuffer())),
      ),
    );
  })();
  return loading;
...
function startMusic() {
  if (source || !ctx || !music) return;
  const buf = buffers.get("linen");
  if (!buf) return;
...
function render(button: HTMLButtonElement) {
  ...
  button.title = wanted ? "Turn studio sound off" : "Turn studio sound on (calm music and soft effects)";
