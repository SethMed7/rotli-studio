...
const trackedFiles = () => {
  if (!tracked || Date.now() - tracked.at > 15_000)
    tracked = {
      at: Date.now(),
      files: new Set(
        Bun.spawnSync(["git", "ls-files", "-z"], { cwd: ROOT }).stdout.toString().split("\0").filter(Boolean),
      ),
    };
...
const newest = (paths: string[]): number => {
  ...
  const walk = (p: string) => {
    ...
    if (st.isDirectory()) for (const e of readdirSync(p)) if (e !== "node_modules" && e !== "out") walk(join(p, e));
  };
  paths.forEach(walk);
  return t;
};
...
const MANIFEST_INPUTS = () => [
  join(MOTION, "pieces.json"),
  join(MOTION, "series.json"),
  join(MOTION, "workflows"),
  join(MOTION, "golden"),
  join(MOTION, "series"),
  join(MOTION, "brand"),
  join(MOTION, "src"),
...
const inputsStamp = () => {
  if (!stamp || Date.now() - stamp.at > 3000) stamp = { at: Date.now(), t: newest(MANIFEST_INPUTS()) };
  return stamp.t;
};
...
async function manifest(force = false): Promise<Response> {
  if (force || stale()) {
    building ??= run(["node", "tools/manifest.mjs"], MOTION).finally(() => (building = null));
    const r = (await building) as { code: number; err: string };
    if (r?.code) return Response.json({ error: r.err }, { status: 500 });
  }
  return new Response(Bun.file(MANIFEST), {
...
export const thumbArgs = (video: string, frame: number, out: string, fps = 30) => [
  "ffmpeg",
  "-y",
  "-loglevel",
  "error",
  ...
  "1",
  "-vf",
  "scale='if(gt(iw,ih),1280,-2)':'if(gt(iw,ih),-2,1280)':flags=lanczos",
  "-q:v",
  "3",
  out,
];
...
export function videoFps(video: string): number {
  const key = `${video}:${statSync(video).mtimeMs}`;
  if (!rates.has(key)) {
    ...
    const [n, d] = r.stdout.toString().trim().split("/").map(Number);
    rates.set(key, n && d ? n / d : 30);
  }
  return rates.get(key)!;
}
async function thumb(slug: string, frame: number): Promise<Response> {
  const video = join(OUT, "video", `${slug}.mp4`),
    dir = join(OUT, THUMB_DIR, slug),
    file = join(dir, `${frame}.jpg`);
  ...
  if (!existsSync(file) || statSync(file).mtimeMs < statSync(video).mtimeMs) {
    ...
    const r = await run(thumbArgs(video, frame, file, videoFps(video)), MOTION, 30_000);
  ...
  if (!existsSync(file) || statSync(file).mtimeMs < statSync(video).mtimeMs) {
    ...
    const r = await run(
      [
        ...
        "error",
        "-ss",
        (frame / videoFps(video)).toFixed(3),
        "-i",
        video,
        "-frames:v",
        "1",
...
async function exactFrame(id: string, frame: number): Promise<Response> {
  const dir = join(OUT, "frames", id),
    file = join(dir, `f${String(frame).padStart(4, "0")}.png`);
  // re-render when any piece source changed since this frame was cached (a frame depends on shared code too)
  if (!existsSync(file) || statSync(file).mtimeMs < newest([join(MOTION, "src"), join(MOTION, "brand")])) {
    const r = await run(["node", "tools/frames.mjs", id, String(frame), "--out", `out/frames/${id}`], MOTION);
    if (r.code || !existsSync(file)) return new Response(r.err || "render failed", { status: 500 });
  }
...
export function tools() {
  const list = (dir: string, base: string, re: RegExp) =>
    existsSync(dir)
      ? readdirSync(dir)
          .filter((f) => re.test(f))
          .sort()
          .map((f) => ({ file: `${base}/${f}`, about: header(join(dir, f)) || pyDoc(join(dir, f)) }))
      : [];
  return [
    ...list(join(MOTION, "tools"), "motion/tools", /\.(mjs|py)$/),
    ...list(join(ROOT, "sound/tools"), "sound/tools", /\.ts$/),
    ...list(join(ROOT, "scripts"), "scripts", /\.ts$/),
  ];
}
const pyDoc = (file: string) =>
  file.endsWith(".py") ? (readFileSync(file, "utf8").match(/"""([\s\S]*?)"""/)?.[1] ?? "").trim() : "";
export function skills({ global = true } = {}) {
  ...
  const others = ["brand-motion-studio", "anidoodle"]
    .map((d) => join(homedir(), ".claude/skills", d, "SKILL.md"))
    .filter(existsSync)
    .map((f) => ({
      name: f.split("/").at(-2)!,
      where: "global",
      path: f.replace(homedir(), "~"),
...
export function atmospheres() {
  ...
  return [
    ...src.matchAll(
      /"([\w-]+)": \{ id: "[\w-]+", label: "([^"]+)", family: "(\w+)", dark: (true|false), drift: ([\d.]+), why: "([^"]+)"[\s\S]*?score: \{ key: (\d+), melody: (\d)/g,
    ),
  ].map((m) => ({
    id: m[1],
    label: m[2],
...
export const looks = () => {
  const d = join(ROOT, "library/companion-looks");
  return existsSync(d) ? readdirSync(d).filter((f) => f.endsWith(".png")) : [];
};
...
export const motionRoutes = {
  "/motion": () =>
    new Response(Bun.file(join(ROOT, "static/motion.html")), { headers: { "cache-control": "no-store" } }),
  ...
  "/sound/*": (req: Request) =>
    serve(req, allowed(join(ROOT, "sound"), new Set(["catalog.json", "web", "prompts"]), path(req, "/sound/"))),
  ...
  "/api/motion/poster/:slug/:frame": (req: Request & { params: { slug: string; frame: string } }) => {
    const f = num(req.params.frame.replace(/\.jpg$/, ""));
    return SLUG.test(req.params.slug) && f !== null
      ? fullPoster(req.params.slug, f)
      : new Response("Bad request", { status: 400 });
  },
  "/api/motion/frame/:id/:frame": (req: Request & { params: { id: string; frame: string } }) => {
    const f = num(req.params.frame.replace(/\.png$/, ""));
    return ID.test(req.params.id) && f !== null
      ? exactFrame(req.params.id, f)
      : new Response("Bad request", { status: 400 });
  },
  "/api/motion/verify/:id": {
    POST: async (req: Request & { params: { id: string } }) => {
      ...
      return Response.json({
        ok: r.code === 0 && /SAME, audio SAME/.test(r.out),
        output: (r.out + r.err).trim().split("\n").slice(-3).join("\n"),
      });
    },
  },
