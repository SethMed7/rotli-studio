...
// rig, workflows and tools. Leaves behind: Rotli's pieces, season, goldens, renders. Writes pieces.json with
...
const skip = new Set(["node_modules", "out", "dist", "golden", ".tmp", "season", "examples", "runs"]);
cpSync(SRC, join(dst, "motion"), { recursive: true, filter: (p) => !skip.has(basename(p)) && !/workflows\/prompts/.test(p) });
