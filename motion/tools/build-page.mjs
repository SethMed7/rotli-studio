...
export const buildPage = async ({ entry, out, title, plugins = [] }) => {
  ...
  // brand packs (brand/packs/<id>/pack.json) lock their fonts the same way
  const packs = existsSync(join(process.cwd(), "brand/packs"))
    ? readdirSync(join(process.cwd(), "brand/packs"))
        .map((d) => join(process.cwd(), "brand/packs", d, "pack.json"))
        .filter(existsSync)
    : [];
  const lock = Object.fromEntries(
    [join(process.cwd(), "brand/brand.json"), ...packs].flatMap((b) =>
      JSON.parse(readFileSync(b, "utf8")).fonts.files.map((f) => [f.file, f.sha256]),
    ),
  );
  for (const f of Object.values(film.assets.fonts ?? {})) {
    const sha = createHash("sha256")
      .update(readFileSync(join(process.cwd(), f)))
      .digest("hex");
    if (lock[f] !== sha)
      throw new Error(
        `font ${f} does not match brand/brand.json (sha256 ${sha.slice(0, 12)}…, locked ${String(lock[f]).slice(0, 12)}…)`,
      );
  }
  const assets = Object.fromEntries(
    [
      ...Object.entries(film.assets.images),
      ...Object.entries(film.assets.fonts ?? {}).map(([n, f]) => [`font:${n}`, f]),
    ].map(([name, file]) => {
      const mime = MIME[extname(file).toLowerCase()];
      if (!mime) throw new Error(`asset '${name}': unsupported type ${file}`);
      return [name, `data:${mime};base64,${readFileSync(join(process.cwd(), file)).toString("base64")}`];
    }),
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const title = process.argv[2] ?? "fixtures";
  const r = await buildPage({ entry: `src/hosts/page-${title}.ts`, out: `dist/${title}.html`, title });
  console.log(`built ${r.out} (${(r.bytes / 1024).toFixed(0)} KB, self-contained)`);
}
