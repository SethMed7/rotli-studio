...
const MIME = { ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".png": "image/png", ".webp": "image/webp", ".woff2": "font/woff2", ".ttf": "font/ttf", ".svg": "image/svg+xml" };
export const buildPage = async ({ entry, out, title, plugins = [] }) => {
  ...
  for (const f of Object.values(film.assets.fonts ?? {})) { const sha = createHash("sha256").update(readFileSync(join(process.cwd(), f))).digest("hex"); if (lock[f] !== sha) throw new Error(`font ${f} does not match brand/brand.json (sha256 ${sha.slice(0, 12)}…, locked ${String(lock[f]).slice(0, 12)}…)`); }
  const assets = Object.fromEntries([...Object.entries(film.assets.images), ...Object.entries(film.assets.fonts ?? {}).map(([n, f]) => [`font:${n}`, f])].map(([name, file]) => { const mime = MIME[extname(file).toLowerCase()]; if (!mime) throw new Error(`asset '${name}': unsupported type ${file}`); return [name, `data:${mime};base64,${readFileSync(join(process.cwd(), file)).toString("base64")}`]; }));
