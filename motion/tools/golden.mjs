...
if (!entry) {
  console.error(`no piece ${piece} in pieces.json`);
  process.exit(2);
}
const sealedBy = entry.sealed
  ? "pieces.json"
  : JSON.parse(readFileSync(join(ROOM, "series.json"), "utf8")).series.find(
      (x) => x.sealed && x.pieces?.includes(piece),
    )?.id;
if (record && sealedBy && !process.argv.includes("--unseal")) {
  console.error(`refused: ${piece} is sealed (${sealedBy}); its golden may only be re-recorded with --unseal`);
  process.exit(2);
}
const outArg = process.argv.indexOf("--golden"),
  file = outArg > 0 ? resolve(process.argv[outArg + 1]) : join(ROOM, "golden", `${piece}.json`);
...
const s = await playwright.open(env, page.out, { scale: 1, workers: 1 }),
  meta = await s.info();
const frames = {};
for (let n = 0; n < meta.durationFrames; n += 30) frames[n] = md5((await s.frame(n, 0)).png);
...
const a = await s.audio(48000),
  audio = a ? md5(a.pcm16) : null;
await s.close();
...
if (record) {
  ...
  console.log(`recorded ${Object.keys(frames).length} frames + audio -> ${file}`);
  process.exit(0);
}
if (!existsSync(file)) {
  console.error(`no golden at ${file}; record one first`);
  process.exit(2);
}
...
const g = JSON.parse(readFileSync(file, "utf8")),
  keys = Object.keys(g.hashes),
  extra = Object.keys(now.hashes).filter((k) => !(k in g.hashes)).length;
const bad = keys.filter((k) => g.hashes[k] !== now.hashes[k]),
  audioOk = g.audio === now.audio;
const shape = ["W", "H", "frames"].filter((k) => g[k] !== now[k]).map((k) => `${k} ${g[k]} -> ${now[k]}`);
console.log(
  `${piece}: ${keys.length - bad.length}/${keys.length} frames SAME, audio ${audioOk ? "SAME" : "DIFF"}${shape.length ? `, SHAPE DIFF (${shape.join(", ")})` : ""}${extra ? ` (${extra} newer check frame(s) not in this golden)` : ""}${bad.length ? "  differing frames: " + bad.join(" ") : ""}`,
);
