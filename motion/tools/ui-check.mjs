...
const pages = ["#/", "#/library", "#/posts", "#/series/season-one", "#/series/the-film", "#/piece/s01e05LighthouseKeeper", "#/piece/s01e05Vertical", "#/brand", "#/workflows", "#/runs", "#/tools", "#/docs", "#/isolation"];
...
for (const w of widths) {
  ...
  for (const h of [...pages, ...(B.includes("127.0.0.1") ? ["POSTS"] : [])]) {
    await p.goto(h === "POSTS" ? B + "/create" : B + "/" + h, { waitUntil: "load" }); await p.waitForTimeout(h === "#/isolation" ? 14000 : 1200);
    const r = await p.evaluate(() => {
      ...
      for (let i = 0; i < els.length; i++) for (let j = i + 1; j < els.length; j++) { const a = els[i], c = els[j]; if (a.contains(c) || c.contains(a)) continue; if (a.closest("[data-player]") && a.closest("[data-player]") === c.closest("[data-player]")) continue; /* overlay controls sit on the film by design */ const ra = a.getBoundingClientRect(), rc = c.getBoundingClientRect(); const ix = Math.min(ra.right, rc.right) - Math.max(ra.left, rc.left), iy = Math.min(ra.bottom, rc.bottom) - Math.max(ra.top, rc.top); if (ix > 3 && iy > 3) out.push(`overlap: <${a.tagName.toLowerCase()} ${a.className}> "${(a.textContent||"").trim().slice(0,30)}" × <${c.tagName.toLowerCase()} ${c.className}> "${(c.textContent||"").trim().slice(0,30)}"`); if (out.length > 12) return out; }
      return out;
    });
    r.forEach((x) => problems.push(`[${w}] ${h}: ${x}`));
    if (["#/", "#/posts", "#/series/season-one", "#/piece/s01e05LighthouseKeeper", "#/library", "#/tools", "#/brand"].includes(h) || h === "POSTS") await p.screenshot({ path: `/tmp/ui-${w}-${h.replace(/[^a-z0-9]+/gi, "_")}.png` });
  }
  if (w === 1440) { await p.goto(B + "/#/", { waitUntil: "load" }); await p.waitForTimeout(1200); await p.screenshot({ path: "/tmp/ui-home-full.png", fullPage: true }); const main = await p.$("#main"); const H = await p.evaluate(() => document.querySelector("#main").scrollHeight); for (const y of [900, 1800, 2700, 3600]) { if (y > H) break; await p.evaluate((yy) => document.querySelector("#main").scrollTo(0, yy), y); await p.waitForTimeout(400); await p.screenshot({ path: `/tmp/ui-home-${y}.png` }); } }
  await p.close();
}
