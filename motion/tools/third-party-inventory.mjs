...
const ROOM = resolve(dirname(fileURLToPath(import.meta.url)), ".."),
  STUDIO = resolve(ROOM, "..");
const DIR = join(ROOM, "third_party/anidoodle"),
  UPSTREAM = join(DIR, "upstream.json"),
...
if (clone) {
  ...
  const files = Object.fromEntries(
    git("ls-files", "engine")
      .split("\n")
      .filter(Boolean)
      .map((f) => [f.replace(/^engine\//, ""), sha(readFileSync(join(clone, f)))]),
  );
  writeFileSync(
    UPSTREAM,
    JSON.stringify(
      {
        repo: "https://github.com/alexgreensh/anidoodle",
...
const ROOTS = [
  ...
  {
    dir: "archive/films/quokka-film-original",
    title: "Files from anidoodle in `archive/films/quokka-film-original/` (frozen archive of the first film)",
  },
];
// the room moved anidoodle's styles and examples into subfolders of canvas-core
const candidates = (root, rel) => {
  const m = rel.match(/^src\/canvas-core\/([^/]+)$/);
  return [
    `${root}/${rel}`,
    ...(m && root === "motion" ? ["styles", "examples"].map((d) => `${root}/src/canvas-core/${d}/${m[1]}`) : []),
  ];
};
...
for (const { dir, title } of ROOTS) {
  ...
  for (const [rel, hash] of Object.entries(upstream.files)) {
    ...
    if (!at) continue;
    const bytes = readFileSync(join(STUDIO, at)),
      moved = at !== `${dir}/${rel}` ? ` (moved from \`${rel}\`)` : "";
    if (sha(bytes) === hash) {
      same.push(`- \`${at}\`${moved}`);
      continue;
    }
...
const START = "<!-- inventory:start (node motion/tools/third-party-inventory.mjs) -->",
  END = "<!-- inventory:end -->";
...
if (process.argv.includes("--check")) {
  if (next !== text)
    problems.push("third_party/anidoodle/README.md is stale: run node tools/third-party-inventory.mjs");
  if (problems.length) {
    console.error(`third-party: FAIL\n  ${problems.join("\n  ")}`);
    process.exit(1);
  }
