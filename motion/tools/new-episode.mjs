...
const b = JSON.parse(readFileSync(process.argv[2], "utf8")),
  file = `src/canvas-core/${b.id}.ts`,
  base = b.id.replace(/^(s\d+e\d+).*/, "$1");
if (existsSync(file)) {
  console.error(`${file} exists; refusing to overwrite`);
  process.exit(1);
}
const scene = (sc) =>
  sc.id === "chapter"
    ? `const chapter: Scene = { id: "chapter", len: ${sc.len}, draw: (ctx, env, s) => chapterCard(ctx, env, s, { no: ${b.no}, title: ${JSON.stringify(b.title)}, pose: "base" }) };`
    : sc.id === "end"
      ? `const end: Scene = { id: "end", len: ${sc.len}, draw: (ctx, env, s) => storyEnd(ctx, env, s, { next: ${JSON.stringify(b.next || undefined)} }) };`
...
writeFileSync(
  file,
  `// SEASON ONE · ${String(b.no).padStart(2, "0")} — ${b.title.join(" ").toUpperCase()} (60 s). Brief: series/season-one/episodes/${base}.json
// Setup: ${b.story.setup}
// Turn: ${b.story.turn}
...
writeFileSync(
  `src/hosts/page-${b.id}.ts`,
  `import { ${b.id} } from "../canvas-core/${b.id}";\nimport { mountFilm } from "./page";\nmountFilm(${b.id});\n`,
);
