...
if (!url) {
  console.error("usage: node tools/discover.mjs <url> [--out dir] [--pages n]");
  process.exit(1);
}
const arg = (k, d) => {
...
const out = resolve(arg("out", "discovery")),
  maxPages = Number(arg("pages", 6));
...
const visit = async (u) => {
  ...
  const shot = join(out, "shots", `${new URL(u).pathname.replace(/\W+/g, "_") || "home"}.png`);
  ...
  const data = await page.evaluate(() => {
    ...
    for (const sheet of document.styleSheets) {
      ...
      const walk = (list, media) => {
        for (const r of list) {
          ...
          const props = {};
          for (const p of r.style) if (p.startsWith("--")) props[p] = r.style.getPropertyValue(p).trim();
          if (!Object.keys(props).length) continue;
          const key = media ? `@media ${media} ${r.selectorText}` : r.selectorText;
          if (/^(:root|html)$/.test(r.selectorText) && !media) Object.assign(tokens.root, props);
          else {
            tokens.variants[key] ??= {};
            Object.assign(tokens.variants[key], props);
          }
        }
      };
      walk(rules, "");
    }
    const sections = [...document.querySelectorAll("section, header, footer, main > div")].slice(0, 40).map((el) => {
      const s = getComputedStyle(el);
      return {
        tag: el.tagName.toLowerCase(),
        cls: String(el.className).slice(0, 60),
        background: s.backgroundColor,
        image: s.backgroundImage !== "none" ? s.backgroundImage.slice(0, 200) : "",
        color: s.color,
    ...
    const marks = {
      ...
      headerSvgs: [...document.querySelectorAll("header svg, nav svg, a[href='/'] svg")]
        .slice(0, 4)
        .map((s) => s.outerHTML.slice(0, 4000)),
      images: [...document.querySelectorAll("img")]
        .filter((i) => /logo|mascot|character|brand|hero/i.test(i.src + i.alt + i.className))
        .slice(0, 12)
    ...
    document.querySelectorAll("h1, h2, h3").forEach((h) => {
      ...
      for (let i = 0; n && i < 3; i++, n = n.nextElementSibling)
        if (/^(P|UL|OL|DIV)$/.test(n.tagName)) next.push(text(n).slice(0, 400));
      claims.push({ kind: h.tagName.toLowerCase(), heading: text(h), body: next.filter(Boolean) });
    });
    const faq = [...document.querySelectorAll("details")].map((d) => ({
      q: text(d.querySelector("summary") ?? d).slice(0, 200),
      a: text(d)
        .replace(text(d.querySelector("summary") ?? d), "")
        .trim()
        .slice(0, 600),
    }));
    ...
    return {
      ...
      type: {
        ...
        button: cs(document.querySelector("button, .btn, a[class*=button]")),
      },
      tokens,
      faces,
      sections,
...
const first = await visit(url),
  pages = [first],
  seen = new Set([url.replace(/\/$/, "")]);
for (const l of first.links) {
  if (pages.length >= maxPages) break;
  const u = l.split("#")[0].replace(/\/$/, "");
  if (!u.startsWith(origin) || seen.has(u) || /\.(png|jpg|svg|pdf|zip|dmg)$/i.test(u)) continue;
  seen.add(u);
  try {
    pages.push(await visit(u));
  } catch (e) {
...
const d = {
  ...
  pages: pages.map(({ links: _links, ...p }) => p),
};
writeFileSync(join(out, "discovery.json"), JSON.stringify(d, null, 1));
console.log(
  `discovered ${pages.length} page(s) of ${origin} -> ${join(out, "discovery.json")} (+ ${pages.length} full-page shots)`,
);
for (const p of d.pages)
  console.log(
    `  ${p.url}  h1 "${(p.claims.find((c) => c.kind === "h1") ?? {}).heading ?? ""}"  tokens ${Object.keys(p.tokens.root).length}+${Object.keys(p.tokens.variants).length} variants  faq ${p.faq.length}`,
  );
