...
YOUR STUDY: "Before / After" (study 20), brief at `series/studies/briefs/split-screen.json` (read it first; it is the contract).
Pieces to export from `src/canvas-core/studies/splitScreen.ts`: `splitScreen` (vertical), `splitScreenLandscape` (landscape). Primary size: vertical.
...
HARD RULES:
1. Create/modify ONLY `src/canvas-core/studies/splitScreen.ts` and its hosts `src/hosts/page-<pieceId>.ts` (one per piece, copy
...
7. Sound: beatScore soft at 120 bpm with a slide sound on each divider move, a hit on the full sweep and the sign-off.

VERIFY BY LOOKING, and iterate until right:
- `node tools/frames.mjs <pieceId> <8–12 frames across the piece> --out /tmp/split-screen --sheet /tmp/split-screen.png --cols 6`,
  then READ the PNG (and full frames in /tmp/split-screen/ where detail matters). Do this for EVERY size.
...
--- THE STUDY (from series/studies/briefs/split-screen.json) ---
Before / After · study 20 · video · 660 frames at 30 fps, 120 bpm · palette "butter"
Style: Split screen: the same scene shown twice, BEFORE (cluttered, off-kilter, a warm alarm colour) and AFTER (calm, aligned, the cool colour), divided by a line that wipes, tilts and finally sweeps all the way across so AFTER takes the whole frame. The same objects exist on both sides, so the change is obvious. Captions are serif word ladders (kit captions.ts): short stacks of mixed sizes, one key word in the accent italic, set beside the action and arriving word by word.
Learns from: Founder explainer reels (Instagram, @gregisenberg's team): a before/after reveal built from the same objects, a wipe that carries the argument.
Beats (frames · what):
  - 0–60 · hook: a vertical divider draws down the middle; labels 'Before' and 'After' pop in
  - 60–210 · BEFORE: a desk with a messy week, 9 meeting blocks overlapping; AFTER: the same week with 4 blocks and 2 focus blocks
  - 210–345 · the divider slides left and right like a comparison slider, revealing each side in turn
  - 345–480 · BEFORE: an inbox with 31 threads; AFTER: one card 'Thu 15:00 booked'
  - 480–600 · the divider sweeps fully across; AFTER takes the frame; everything settles into place with a spring
  - 600–660 · sign-off: 'Oriel' with the caption 'same week, calmer'
Sizes: Landscape splits left and right; vertical splits top and bottom (the divider is horizontal).
Teaches: A before/after works when both sides share objects: the eye compares the same thing in two states.
