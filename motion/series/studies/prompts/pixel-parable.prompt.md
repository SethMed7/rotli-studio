...
YOUR STUDY: "Pixel Parable" (study 11), brief at `series/studies/briefs/pixel-parable.json` (read it first; it is the contract).
Pieces to export from `src/canvas-core/studies/pixelParable.ts`: `pixelParable` (vertical), `pixelParableLandscape` (landscape). Primary size: vertical.
...
HARD RULES:
1. Create/modify ONLY `src/canvas-core/studies/pixelParable.ts` and its hosts `src/hosts/page-<pieceId>.ts` (one per piece, copy
...
3. Follow the brief's story beats and timing (you may retime inside a beat; every shot must start on the beat grid:
   one beat = 60 / bpm × fps frames; `validate()` in film.ts refuses anything else). Frames: 750 at 30 fps.
...
7. Sound: beatScore drive at 120 bpm, 8-bit flavoured (bright plucks), ticks for every icon pop, a hit when the serpent appears and the sign-off.

VERIFY BY LOOKING, and iterate until right:
- `node tools/frames.mjs <pieceId> <8–12 frames across the piece> --out /tmp/pixel-parable --sheet /tmp/pixel-parable.png --cols 6`,
  then READ the PNG (and full frames in /tmp/pixel-parable/ where detail matters). Do this for EVERY size.
...
--- THE STUDY (from series/studies/briefs/pixel-parable.json) ---
Pixel Parable · study 11 · video · 750 frames at 30 fps, 120 bpm · palette "arcade"
Style: Pixel art: 8-bit characters built from square pixels (no anti-aliasing: snap every pixel to a 6 px grid) stand on a dark ground strip under a pale sky. A cautionary tale told with pixel props, a swarm of notification icons, and a pixel serpent that coils along a path. A few crisp UI cards (not pixelated) cut in to show numbers. Captions are serif word ladders (kit captions.ts): short stacks of mixed sizes, one key word in the accent italic, set beside the action and arriving word by word.
Learns from: Founder explainer reels (Instagram, @gregisenberg's team): 8-bit characters at desks on a ground strip, pixel buildings, a swarm of notification icons, a pixel snake on a path, a grid of icons flipping colour, clean UI cards cut into pixel scenes.
Beats (frames · what):
  - 0–75 · hook: a pixel manager stands before a team of three at desks; caption 'We rewarded meetings booked.'
  - 75–210 · the team books meetings: calendar icons pop out of their desks in a growing swarm; a counter card ticks 12 → 240
  - 210–345 · the calendar tower wobbles: a grid of meeting icons flips from teal to magenta one by one ('no-shows'); caption 'so they booked everything'
  - 345–480 · a pixel serpent slides along a path through the office, eating the day; caption 'The metric became the goal.'
  - 480–630 · the fix: a UI card shows two metrics side by side ('meetings held' and 'decisions made'), both sparklines rise; the icons flip back to teal
  - 630–750 · sign-off: the team high-fives in pixels; 'Oriel' pixel sign rises; caption 'measure the outcome, not the count'
Sizes: Landscape widens the office so the team and the manager share one frame; captions sit top left.
Teaches: Pixel art is a grid decision: snap everything to one pixel size and the style holds, even when crisp UI cuts in.
