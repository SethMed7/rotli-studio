...
YOUR STUDY: "Print Carousel" (study 04), brief at `series/studies/briefs/print-carousel.json` (read it first; it is the contract).
Pieces to export from `src/canvas-core/studies/printCarousel.ts`: `printCarousel` (portrait), `printCarouselSquare` (square). Primary size: portrait.
...
HARD RULES:
1. Create/modify ONLY `src/canvas-core/studies/printCarousel.ts` and its hosts `src/hosts/page-<pieceId>.ts` (one per piece, copy
...
7. Sound: none (a carousel is stills)

VERIFY BY LOOKING, and iterate until right:
- `node tools/frames.mjs <pieceId> <8–12 frames across the piece> --out /tmp/print-carousel --sheet /tmp/print-carousel.png --cols 6`,
  then READ the PNG (and full frames in /tmp/print-carousel/ where detail matters). Do this for EVERY size.
