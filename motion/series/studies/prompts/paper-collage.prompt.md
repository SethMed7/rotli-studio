...
YOUR STUDY: "Paper Collage" (study 15), brief at `series/studies/briefs/paper-collage.json` (read it first; it is the contract).
Pieces to export from `src/canvas-core/studies/paperCollage.ts`: `paperCollage` (vertical), `paperCollageLandscape` (landscape). Primary size: vertical.
...
HARD RULES:
1. Create/modify ONLY `src/canvas-core/studies/paperCollage.ts` and its hosts `src/hosts/page-<pieceId>.ts` (one per piece, copy
...
7. Sound: beatScore soft at 120 bpm with paper rustles (short noise) on each paste, a hit when the piece clicks in and the sign-off.

VERIFY BY LOOKING, and iterate until right:
- `node tools/frames.mjs <pieceId> <8–12 frames across the piece> --out /tmp/paper-collage --sheet /tmp/paper-collage.png --cols 6`,
  then READ the PNG (and full frames in /tmp/paper-collage/ where detail matters). Do this for EVERY size.
...
--- THE STUDY (from series/studies/briefs/paper-collage.json) ---
Paper Collage · study 15 · video · 720 frames at 30 fps, 120 bpm · palette "kraft"
Style: Cut-paper collage on kraft paper: shapes with torn, slightly irregular edges, a paper-grain texture, drop shadows as if lifted off the page, halftone patches, two hands cut from paper that slide a puzzle piece into place, a retro computer made of paper shapes. Everything moves like stop motion (hold each pose 2 frames). Captions are serif word ladders (kit captions.ts): short stacks of mixed sizes, one key word in the accent italic, set beside the action and arriving word by word.
Learns from: Founder explainer reels (Instagram, @gregisenberg's team): editorial collage: cut-out hands, a puzzle piece that completes the picture, a retro computer beside a circle, textured paper.
Beats (frames · what):
  - 0–75 · hook: a torn-paper headline 'The missing piece' is pasted on, letter scraps sliding into place
  - 75–225 · a paper calendar with one torn-out hole; paper hands enter carrying the missing piece
  - 225–375 · a retro paper computer beside a big circle; on its screen, paper scraps rearrange into 'Thu 3:00'
  - 375–525 · three paper portraits (simple shapes) each get a paper check mark pasted on
  - 525–645 · the hands press the puzzle piece into the calendar; it clicks, the grain shifts, confetti scraps burst
  - 645–720 · sign-off: 'Oriel' in cut letters; caption 'the piece your week was missing'
Sizes: Landscape spreads the collage across the page, calendar left, computer right.
Teaches: Collage is texture plus timing: irregular edges, grain and shadows sell the paper, and holding each pose for two frames sells stop motion.
