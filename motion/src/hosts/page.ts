// Derived from anidoodle (Apache-2.0, motion/third_party/anidoodle); modified by Rotli contributors.
...
export const mountFilm = (film: Film) => {
  ...
  const surface = (w: number, h: number): Layer => {
    ...
    return { canvas: c, ctx: c.getContext("2d", opts) as unknown as Ctx } as Layer;
  };
  const mount = (scale = 1) => {
    canvas.width = Math.round(film.meta.W * scale);
    canvas.height = Math.round(film.meta.H * scale);
    ctx = canvas.getContext("2d", opts) as CanvasRenderingContext2D;
    env = { W: film.meta.W, H: film.meta.H, scale, cache: new Map(), canvas: surface, image: (n) => images.get(n) };
    return film.meta;
  };
  ...
  const ready = (async () => {
    ...
    await Promise.all(
      Object.entries(film.assets.fonts ?? {}).map(async ([spec, url]) => {
        const [family, weight = "400"] = spec.split("@");
        const f = new FontFace(family, `url(${window.__ASSETS__?.[`font:${spec}`] ?? url})`, { weight });
        await f.load();
        (document.fonts as unknown as { add(f: FontFace): void }).add(f);
      }),
    );
    await Promise.all(
      Object.entries(film.assets.images).map(async ([name, url]) => {
        const img = new Image();
        img.src = window.__ASSETS__?.[name] ?? url;
        await img.decode();
        images.set(name, img);
      }),
    );
  ...
  const seek = (frame: number) => {
    ...
    const shot = renderFrame(film, ctx, frame, env);
    ctx.getImageData(0, 0, 1, 1);
    current = frame;
    return { shot, ms: performance.now() - t0 };
  }; // getImageData forces the deferred raster so the timing is real
  const hash = () => {
  ...
  const audio = (sr: number) => {
    ...
    const [L, R] = film.audio(sr),
      pcm = new Int16Array(L.length * 2);
    for (let i = 0; i < L.length; i++) {
      pcm[i * 2] = Math.max(-1, Math.min(1, L[i])) * 32767;
      pcm[i * 2 + 1] = Math.max(-1, Math.min(1, R[i])) * 32767;
    }
    return { sampleRate: sr, frames: L.length, pcm16: b64(new Uint8Array(pcm.buffer)) };
  };
  const warm = () =>
    film.shots.forEach((s) => {
      seek(s.start);
      seek(s.start + ((s.end - s.start) >> 1));
    }); // first + middle frame of every shot: builds tiles, pre-allocates the layer pool
  window.FILM = {
    meta: film.meta,
    ready,
    mount,
    ...
    audio,
    warm,
    png: () => canvas.toDataURL("image/png").slice(22),
    frame: () => current,
  };
  ...
  ready.then(() => {
    ...
    if (q.has("adapter")) return; // a backend is driving: stay still
    ...
    const play = () => {
      ...
      playing = true;
      f0 = current >= film.meta.durationFrames - 1 ? 0 : current;
      t0 = ac.currentTime;
      if (film.audio) {
        const [L, R] = film.audio(ac.sampleRate),
        ...
        node.start(0, f0 / film.meta.fps);
      }
      const tick = () => {
        if (!playing) return;
        const f = f0 + Math.floor((ac!.currentTime - t0) * film.meta.fps);
        if (f >= film.meta.durationFrames) {
          seek(film.meta.durationFrames - 1);
          stop();
          return;
        }
        if (f !== current) seek(f);
    ...
    window.addEventListener("keydown", (e) => {
      ...
      if (e.key === "ArrowRight") {
        stop();
        seek(Math.min(film.meta.durationFrames - 1, current + 1));
      }
      if (e.key === "ArrowLeft") {
        stop();
