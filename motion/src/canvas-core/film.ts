...
export type Film = {
  meta: {
    ...
    raster?: "gpu" | "cpu";
    family?: string /* the theme family a derivative should dress in */;
  };
  assets: Assets;
  shots: Shot[];
  audio?: (sampleRate: number) => [Float32Array, Float32Array];
};

// Cheap structural checks every adapter runs before frame 0. (The full gate is Phase 2.)
export const validate = (film: Film): string[] => {
  const p: string[] = [],
    { fps, bpm, durationFrames } = film.meta,
    beat = (60 / bpm) * fps,
    ids = new Set<string>();
  if (!Number.isInteger(beat))
    p.push(`bpm ${bpm} at ${fps} fps gives a ${beat}-frame beat: cuts cannot sit on whole frames`);
  let t = 0;
  [...film.shots]
    .sort((a, b) => a.start - b.start)
    .forEach((s) => {
      if (ids.has(s.id)) p.push(`duplicate shot id '${s.id}'`);
      ids.add(s.id);
      if (s.start !== t) p.push(`shot '${s.id}' starts at ${s.start}, expected ${t} (gap or overlap)`);
      if (s.end <= s.start) p.push(`shot '${s.id}' has no length`);
      if (Number.isInteger(beat) && s.start % beat)
        p.push(`shot '${s.id}' cuts off the beat grid (frame ${s.start}, beat = ${beat} frames)`);
      t = s.end;
    });
...
export const shotAt = (film: Film, frame: number): Shot | undefined =>
  film.shots.find((s) => frame >= s.start && frame < s.end);
export const renderFrame = (film: Film, ctx: Ctx, frame: number, env: Env): string | null => {
  ...
  ctx.clearRect(0, 0, env.W * env.scale, env.H * env.scale);
