...
export const TRACK = _tr ?? -0.045,
  TRACK_CAPTION = _tr == null ? -0.04 : _tr * 0.9;
export const FONT = {
  ui: BRAND.fonts.ui,
  word: BRAND.fonts.word,
  mono: BRAND.fonts.mono,
  display: (BRAND.fonts as { display?: string }).display ?? BRAND.fonts.ui,
};
...
export const ink = (
  ...
  const base = jitter(pts, wob, seed),
    s = sample(jitter(base, boil, seed * 131 + Math.floor(frame / 4)), closed, 8);
  ...
  ctx.lineJoin = "round";
  ctx.beginPath();
  for (let i = 0; i < n; i++) (i ? ctx.lineTo : ctx.moveTo).call(ctx, s[i][0], s[i][1]);
  if (fill && progress >= 1) {
    ctx.closePath();
    ctx.fillStyle = fill;
    ctx.fill();
...
export const paperGround = (ctx: Ctx, frame: number, band = C.peach, alpha = 0.32, w = W, h = H, ground = C.linen) => {
  ...
  ctx.fillStyle = band;
  const step = 170,
    off = (frame * 1.2) % (step * 2);
  for (let x = -h - step * 2 + off; x < w + step; x += step * 2) {
    ctx.beginPath();
    ctx.moveTo(x, h);
    ctx.lineTo(x + step, h);
...
export const nightGround = (ctx: Ctx, frame: number, seed = 7, w = W, h = H) => {
  ...
  ctx.fillRect(0, 0, w, h);
  ctx.save();
  ctx.strokeStyle = "rgba(241,231,218,0.045)";
  ctx.lineWidth = 1;
  ...
  for (let i = 0; i < 70; i++) {
    const x = r() * w,
      ...
      ph = r() * 6.28,
      s = 1 + r() * 1.8;
    ctx.globalAlpha = 0.25 + 0.35 * (0.5 + 0.5 * Math.sin(frame / 14 + ph));
...
export const beatLabel = (ctx: Ctx, frame: number, local: number, n: number, label: string, sub = "", dark = false) => {
  ...
  ink(
    ...
    { w: 5, color: C.clay, seed: 90 + n, frame, progress: seg(local, 4, 20) },
  );
  if (sub)
    text(ctx, sub, x, y + 52, {
      size: 24,
      weight: 500,
      color: dark ? C.nightMuted : C.muted,
      alpha: seg(local, 10, 24),
    });
  ...
  ctx.save();
  ctx.lineWidth = 5;
  ctx.strokeStyle = dark ? C.nightBorder : C.border;
  ctx.beginPath();
  ctx.arc(cx, cy, 26, 0, 6.29);
  ctx.stroke();
  ctx.strokeStyle = C.clay;
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.arc(cx, cy, 26, -Math.PI / 2, -Math.PI / 2 + p * Math.PI * 2);
  ctx.stroke();
...
export const card = (ctx: Ctx, o: CardOpts) => {
  ...
  if (ghost) {
    ...
    rr(ctx, 0, 0, w, h, 14);
    ctx.strokeStyle = dark ? C.nightMuted : C.muted;
    ctx.lineWidth = 3;
    ctx.stroke();
    ctx.restore();
    return;
  ...
  ctx.lineTo(w, fold);
  ctx.lineTo(w, h - 14);
  ctx.quadraticCurveTo(w, h, w - 14, h);
  ctx.lineTo(14, h);
  ctx.quadraticCurveTo(0, h, 0, h - 14);
  ctx.lineTo(0, 14);
  ctx.quadraticCurveTo(0, 0, 14, 0);
  ctx.closePath();
  ctx.fillStyle = dark ? C.nightSurface2 : C.surface;
  ctx.fill();
  ctx.strokeStyle = dark ? C.nightBorder : C.ink;
  ctx.lineWidth = 3;
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(w - fold, 0);
  ctx.lineTo(w - fold, fold - 4);
  ctx.quadraticCurveTo(w - fold, fold, w - fold + 4, fold);
  ctx.lineTo(w, fold);
  ctx.closePath();
  ctx.fillStyle = token ? C.clay : dark ? C.nightBorder : C.peach;
  ctx.fill();
  ctx.stroke();
  let ty = 36;
  if (title) {
    text(ctx, title, 18, ty, { size: 21, weight: 600, color: dark ? C.nightText : C.cocoa });
    ty += 14;
  }
  const r = rng(seed);
  for (let i = 0; i < lines; i++) {
    const lw = (w - 44) * (0.55 + r() * 0.4);
    ink(
      ctx,
      [
        [18, ty + 12 + i * 20],
      ...
      { w: 4, color: dark ? C.nightMuted : C.border, seed: seed * 10 + i, frame, wob: 0.6, boil: 0.3 },
    );
  }
  if (lock) lockIcon(ctx, w - 30, h - 30, 16, C.clay);
  if (tag)
    chip(ctx, 16, h + 10, tag, { size: 17, bg: dark ? C.nightSurface : C.peach, fg: dark ? C.nightText : C.clayText });
  ctx.restore();
};
export const lockIcon = (ctx: Ctx, cx: number, cy: number, s: number, col: string) => {
  ...
  ctx.beginPath();
  ctx.arc(cx, cy - s * 0.3, s * 0.42, Math.PI, 0);
  ctx.stroke();
  rr(ctx, cx - s * 0.62, cy - s * 0.3, s * 1.24, s * 0.95, s * 0.18);
  ctx.fill();
  ctx.restore();
};
export const chip = (
  ...
  y: number,
  label: string,
  o: { size?: number; bg?: string; fg?: string; border?: string; alpha?: number; mono?: boolean } = {},
) => {
  const { size = 22, bg = C.peach, fg = C.clayText, border, alpha = 1, mono = false } = o,
    font = mono ? FONT.mono : FONT.ui,
    w = measure(ctx, label, size, 600, font) + size * 1.1,
    h = size * 1.6;
  ctx.save();
  ctx.globalAlpha *= alpha;
  fillRR(ctx, x, y, w, h, h / 2, bg, border, 2);
  text(ctx, label, x + size * 0.55, y + h * 0.68, { size, weight: 600, color: fg, font });
  ctx.restore();
  return w;
};
...
export const key = (ctx: Ctx, x: number, y: number, w: number, h: number, label: string, press: number) => {
  ...
  text(ctx, label, x + w / 2, y + 16 - up + h * 0.6, {
    size: Math.min(58, h * 0.4),
    weight: 600,
    align: "center",
    color: C.cocoa,
  });
...
export const folder = (
  ...
  w: number,
  h: number,
  o: { open?: number; label?: string; dark?: boolean; front?: boolean; back?: boolean } = {},
) => {
  const { open = 0, label = "", dark = false, front = true, back = true } = o,
    tabW = w * 0.34;
  ctx.save();
  ctx.lineWidth = 4;
  ...
  if (back) {
    ...
    ctx.moveTo(x, y + 20);
    ctx.lineTo(x, y);
    ctx.quadraticCurveTo(x, y - 10, x + 12, y - 12);
    ctx.lineTo(x + tabW - 20, y - 12);
    ctx.lineTo(x + tabW + 10, y + 14);
    ctx.lineTo(x + w - 14, y + 14);
    ctx.quadraticCurveTo(x + w, y + 14, x + w, y + 28);
    ctx.lineTo(x + w, y + h);
    ctx.lineTo(x, y + h);
    ctx.closePath();
    ctx.fillStyle = dark ? "#9c6a4e" : C.clay;
    ctx.fill();
    ctx.stroke();
  ...
  if (front) {
    ...
    ctx.beginPath();
    ctx.moveTo(x - 6, y + 46 + lift);
    ctx.lineTo(x + w + 6, y + 46 + lift);
    ctx.lineTo(x + w - 4, y + h);
    ctx.lineTo(x + 4, y + h);
    ctx.closePath();
    ctx.fillStyle = "#e3a585";
    ctx.fill();
    ctx.stroke();
  }
  ctx.restore();
...
export const badge = (ctx: Ctx, x: number, y: number, s: number, count: string, glyph: number, alpha = 1) => {
  ...
  if (glyph === 0) {
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(0, 18, 5, 0, 6.29);
    ctx.stroke();
  } else {
    rr(ctx, -24, -20, 48, 32, 10);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(-10, 12);
    ctx.lineTo(-16, 24);
...
export const cloud = (
  ...
  y: number,
  s: number,
  o: { dark?: boolean; frame?: number; seed?: number; eyes?: boolean; alpha?: number; face?: "curious" | "dizzy" } = {},
) => {
  ...
  ctx.fillStyle = dark ? "#e9ecf2" : C.surface;
  ctx.strokeStyle = C.ink;
  ctx.lineWidth = 4;
  ctx.beginPath();
  ...
  ink(
    ...
    { w: 4, seed: seed + 1, frame },
  );
  ctx.fillStyle = C.clay;
  ctx.beginPath();
  ctx.arc(4, -88, 7, 0, 6.29);
  ctx.fill();
  if (eyes) {
    ctx.fillStyle = C.ink;
    if (face === "dizzy") {
...
export const appWindow = (
  ...
  w: number,
  h: number,
  o: { notes?: string[]; active?: number; frame?: number; dark?: boolean } = {},
) => {
  const { notes = ["Trip ideas", "Ferry times", "Reading list"], active = 0, dark = false } = o,
    S = APP.side;
  ...
  const txt = dark ? C.nightText : C.cocoa,
    mut = dark ? C.nightMuted : "#5a4e44";
  let sy = y + 104;
  text(ctx, "Home", x + 34, sy, { size: 25, weight: 500, color: mut });
  sy += 46;
  text(ctx, "Main", x + 34, sy, { size: 25, weight: 600, color: txt });
  sy += 40;
  notes.forEach((n, i) => {
    if (i === active) fillRR(ctx, x + 18, sy - 28, S - 36, 40, 10, dark ? C.nightSurface2 : C.peach);
    text(ctx, n, x + 52, sy, { size: 24, weight: i === active ? 600 : 500, color: i === active ? txt : mut });
    sy += 44;
  });
  sy += 10;
  text(ctx, "System", x + 34, sy, { size: 25, weight: 500, color: mut });
...
export const speedLines = (
  ...
  for (let i = 0; i < n; i++) {
    const yy = y + (i - (n - 1) / 2) * 18,
      l = len * (0.6 + 0.4 * Math.sin(frame * 0.9 + i * 2));
    ink(
      ctx,
      [
        [x, yy],
      ...
      { w: 4, color: col, seed: i + 40, frame, alpha },
    );
  }
};
/** a soft round shadow under a character */
export const shadow = (ctx: Ctx, x: number, y: number, w: number, a = 0.18, col = "#3a3028") => {
  ctx.save();
  ctx.globalAlpha = a;
  ctx.fillStyle = col;
  ctx.beginPath();
  ctx.ellipse(x, y, w, w * 0.18, 0, 0, 6.29);
  ctx.fill();
  ctx.restore();
};
