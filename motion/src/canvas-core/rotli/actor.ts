...
export type Act = Omit<Quokka, "x" | "y" | "h"> & {
  x: number;
  y: number;
  h: number;
  lift?: number;
...
export const actor = (ctx: Ctx, env: Env, f: number, a: Act) => {
  ...
  const breath = 1 + 0.014 * Math.sin((f / 40) * Math.PI * 2),
    lift = a.lift ?? 0;
  if (!a.noShadow)
    shadow(
      ctx,
      a.x,
      a.y + 4,
      a.h * 0.2 * (1 - 0.35 * clamp01(lift / 120)),
      0.2 * (1 - 0.4 * clamp01(lift / 120)),
      a.shadowCol,
    );
  ...
  drawQuokka(ctx, env, {
    ...a,
    body: a.body ?? (LOOK.family === "rotli" ? undefined : LOOK.quokka),
    y: a.y - lift,
    blink: a.blink ?? blinkAt(f, 84, 3),
    squash: (a.squash ?? 1) * breath,
  });
};
  ...
  for (let i = 1; i < stops.length; i++) {
    ...
    if (f < f1) {
      if (x0 === x1 && y0 === y1)
        return {
          ...
          y: y0,
          lift: 0,
          squash: f - f0 < 6 && i > 1 ? 1 - 0.13 * Math.sin(((f - f0) / 6) * Math.PI) : 1,
          moving: false,
          dir: 1,
        }; // a hold, not a hop
      const t = (f - f0) / (f1 - f0),
        lift = Math.sin(t * Math.PI) * hgt;
    ...
    if (f < next && f - f1 < 6)
      return {
        ...
        y: y1,
        lift: 0,
        squash: 1 - 0.13 * Math.sin(((f - f1) / 6) * Math.PI),
        moving: false,
