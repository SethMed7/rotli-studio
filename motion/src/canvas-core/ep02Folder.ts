...
const camera = (ctx: Ctx, env: Env, l: number) => {
  const z = easeInOut(seg(l, Q.push[0], Q.push[1])) * (1 - easeInOut(seg(l, Q.pull[0], Q.pull[1]))),
    s = (1 + 0.015 * seg(l, 90, DEMO_END)) * (1 + 0.18 * z);
  const ax = lerp(960, 1420, z),
    ay = lerp(450, 480, z),
    px = lerp(960, 1290, z),
    py = lerp(450, 456, z);
  ctx.setTransform(env.scale * s, 0, 0, env.scale * s, env.scale * (px - s * ax), env.scale * (py - s * ay));
  drift(ctx, env, l);
};
/** scale-in/out around a point, for windows that open and close */
const around = (ctx: Ctx, cx: number, cy: number, s: number, a: number, draw: () => void) => {
  if (s <= 0 || a <= 0) return;
  ctx.save();
  ctx.globalAlpha *= a;
  ctx.translate(cx, cy);
...
const osWindow = (ctx: Ctx, r: Rect, title: string, left = "") => {
  ...
  ctx.fillRect(r.x, r.y, r.w, 70);
  ctx.fillStyle = R.border;
  ctx.fillRect(r.x, r.y + 70, r.w, 2);
  ctx.restore();
  ["#e8836f", "#e8c46f", "#9cc27e"].forEach((c, i) => {
    ctx.fillStyle = c;
    ctx.beginPath();
    ctx.arc(r.x + 32 + i * 28, r.y + 35, 9, 0, 6.29);
    ctx.fill();
  });
  if (left) text(ctx, left, r.x + 124, r.y + 44, { size: 26, weight: 500, color: R["text-muted"] });
  text(ctx, title, r.x + r.w / 2 + (left ? 60 : 0), r.y + 45, {
    size: 27,
    weight: 600,
    align: "center",
    color: R.text,
...
type Row = {
  name: string;
  kind: "dir" | "md" | "chat" | "board" | "doc" | "meta";
  depth?: number;
  tag?: string;
  note?: string;
};
const ROWS: Row[] = [
  { name: "wiki/", kind: "dir" },
  { name: "trip-plan.md", kind: "md", depth: 1, tag: "Markdown" },
  { name: "ferry-times.md", kind: "md", depth: 1, tag: "Markdown" },
  { name: "chats/", kind: "dir" },
  { name: "planning-with-claude.md", kind: "chat", depth: 1, tag: "Markdown" },
  { name: "storage/", kind: "dir" },
  { name: "system-map.excalidraw", kind: "board", depth: 1 },
  { name: "packing-list.docx", kind: "doc", depth: 1 },
...
const vaultList = (
  ...
  r: Rect,
  l: number,
  o: { shown: (i: number) => number; tag: (i: number) => number; sel?: number },
) => {
  ...
  ctx.beginPath();
  ctx.roundRect(fx, fy + 8, 30, 21, 4);
  ctx.fill();
  ctx.fillRect(fx, fy + 4, 12, 7);
  text(ctx, "~/Documents/My vault", fx + 44, fy + 28, { size: 26, font: FONT.mono, color: R["text-muted"] });
  ctx.fillStyle = R.border;
  ctx.fillRect(r.x, top + 66, r.w, 2);
  ROWS.forEach((row, i) => {
    const k = o.shown(i);
    if (k <= 0) return;
    const y = top + 80 + i * ROW_H,
      x = r.x + 26 + (row.depth ?? 0) * 38;
    ctx.save();
    ctx.globalAlpha *= Math.min(1, k * 2);
    ctx.translate(0, (1 - pop(k)) * 14);
          ...
          : row.kind === "board"
            ...
            : row.kind === "doc"
              ...
              : row.kind === "chat"
                ? "#a58bd9"
                : R["text-muted"];
    if (row.kind === "dir" || row.kind === "meta") {
      ctx.fillStyle = col;
      ctx.beginPath();
      ctx.roundRect(x, y + 16, 36, 26, 4);
      ctx.fill();
      ctx.fillRect(x, y + 11, 15, 8);
    } else {
      fillRR(ctx, x + 4, y + 8, 28, 38, 4, R.surface, col, 3);
      ctx.fillStyle = col;
      ctx.fillRect(x + 10, y + 21, 16, 3);
      ctx.fillRect(x + 10, y + 30, 12, 3);
    }
    text(ctx, row.name, x + 52, y + 37, {
      size: 28,
      weight: row.kind === "dir" || row.kind === "meta" ? 600 : 500,
      color: R.text,
    });
    const t = o.tag(i);
    if (t > 0 && row.tag) {
      const w = measure(ctx, row.tag, 26, 600) + 30,
        s = pop(t),
        cx = r.x + r.w - 30 - w / 2;
      ctx.save();
      ctx.translate(cx, y + 28);
      ctx.scale(s, s);
      fillRR(ctx, -w / 2, -21, w, 42, 21, R.tint);
      text(ctx, row.tag, 0, 9, { size: 26, weight: 600, align: "center", color: R["accent-text"] });
      ctx.restore();
    }
    if (t > 0 && row.note) {
      const s = row.note.slice(0, Math.round(row.note.length * seg(t, 0, 0.7))),
        nx = r.x + r.w - 30;
      ...
      if (a > 0) {
        ctx.save();
        ctx.globalAlpha *= a;
        ctx.translate(nx - 14, y + 28);
        ctx.rotate(l / 9);
        ctx.strokeStyle = R.accent;
        ctx.lineWidth = 4;
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.arc(0, 0, 12, 0.4, 5.4);
        ctx.stroke();
        ctx.fillStyle = R.accent;
        ctx.beginPath();
        ctx.moveTo(15, -4);
...
const plainEditor = (ctx: Ctx, r: Rect, l: number, t0: number) => {
  ...
  ctx.fillRect(r.x + 4, top, 66, r.h - 76);
  ctx.fillStyle = R.border;
  ctx.fillRect(r.x + 70, top, 2, r.h - 76);
  let endX = r.x + 100,
    endY = top + 70;
  PLAIN.forEach((s, i) => {
    const a = seg(l, t0 + i * 3, t0 + i * 3 + 6),
      y = top + 70 + i * 56;
    if (a <= 0) return;
    ...
    endX = r.x + 100 + measure(ctx, s, 30, 500, FONT.mono);
    endY = y;
  });
...
const appTile = (ctx: Ctx, cx: number, cy: number, s: number, label: number) => {
  ...
  ctx.restore();
  if (label > 0)
    text(ctx, "Rotli", 0, z / 2 + 44, { size: 30, weight: 600, align: "center", color: C.cocoa, alpha: label });
  ctx.restore();
};
...
const trashCan = (ctx: Ctx, cx: number, cy: number, s: number, lid: number) => {
  ...
  ctx.lineTo(70, -70);
  ctx.lineTo(58, 90);
  ctx.quadraticCurveTo(56, 100, 46, 100);
  ctx.lineTo(-46, 100);
  ctx.quadraticCurveTo(-56, 100, -58, 90);
  ctx.closePath();
  ctx.fillStyle = C.surface;
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = C.border;
  for (const x of [-30, 0, 30]) ctx.fillRect(x - 3, -44, 6, 120);
  ctx.save();
  ctx.translate(-80, -80);
  ctx.rotate(-0.55 * lid);
  fillRR(ctx, 0, -14, 160, 20, 8, C.surface2, C.ink, 4);
  fillRR(ctx, 58, -30, 44, 18, 8, C.surface2, C.ink, 4);
  ctx.restore();
...
// ---------------------------------------------------------------- the demo
const ROTLI_R: Rect = { x: 70, y: 90, w: 880, h: 730 },
  RIGHT_R: Rect = { x: 990, y: 90, w: 860, h: 730 },
  D_R: Rect = { x: 290, y: 90, w: 700, h: 720 };
const rotliWindow = (ctx: Ctx) => {
  const E = appFrame(ctx, T, ROTLI_R, {
  ...
  task(ctx, T, E.x, 330, "x", "book the ferry");
  task(ctx, T, E.x, 392, "/", "pack a snorkel");
  task(ctx, T, E.x, 454, " ", "find the pink lake");
...
const demo = (ctx: Ctx, l: number, env: Env) => {
  ...
  if (l < Q.sceneOut + 16) {
    const out = easeInOut(seg(l, Q.sceneOut, Q.sceneOut + 14)),
      inn = easeInOut(seg(l, Q.rotliIn, Q.rotliIn + 14));
    around(ctx, 510, 455, 1 - 0.1 * out, inn * (1 - out), () => {
      ctx.save();
      ctx.translate(0, (1 - inn) * 40);
      rotliWindow(ctx);
      ctx.restore();
    });
    const fin = backOut(seg(l, Q.finderIn, Q.finderIn + 16)),
      fout = easeInOut(seg(l, Q.finderOut, Q.finderOut + 10));
    around(ctx, 1420, 455, fin * (1 - 0.3 * fout), Math.min(1, fin * 2) * (1 - fout), () =>
      vaultList(ctx, RIGHT_R, l, {
        shown: (i) => seg(l, Q.rows + i * 9, Q.rows + i * 9 + 12),
        tag: (i) =>
          i === 8
            ? seg(l, Q.rotliNote, Q.rotliNote + 40)
            : i === 4
              ? seg(l, Q.chatTag, Q.chatTag + 14)
              : seg(l, Q.mdTags + (i - 1) * 4, Q.mdTags + (i - 1) * 4 + 14),
        sel: l >= Q.dbl[0] ? 1 : l >= Q.rotli ? 8 : l >= Q.chatTag ? 4 : l >= Q.mdTags ? 1 : -1,
      }),
    );
    const ein = backOut(seg(l, Q.editorIn, Q.editorIn + 16));
    around(ctx, 1420, 455, ein * (1 - 0.1 * out), Math.min(1, ein * 2) * (1 - out), () =>
      plainEditor(ctx, RIGHT_R, l, Q.editorIn + 6),
    );
    // the same file, two doors
    const sk = pop(seg(l, Q.same, Q.same + 14));
    if (sk > 0) {
      const s = "same file  ·  trip-plan.md",
        w = measure(ctx, s, 28, 600) + 48;
      ctx.save();
      ctx.globalAlpha *= 1 - out;
      ctx.translate(960, 52);
      ctx.scale(sk, sk);
      fillRR(ctx, -w / 2, -30, w, 60, 30, C.clay, C.ink, 3);
      text(ctx, s, 0, 10, { size: 28, weight: 600, align: "center", color: C.surface });
      ctx.restore();
    }
  }
  // C: the folder backs up any way you like
  if (l >= Q.vault && l < Q.dOut + 16) {
    const out = easeInOut(seg(l, Q.dOut, Q.dOut + 14)),
      k = backOut(seg(l, Q.vault, Q.vault + 16));
    around(ctx, 630, 460, k, Math.min(1, k * 2) * (1 - out), () => {
      ...
      [
        ...
        const up = 18 * Math.sin(l / 14 + i * 1.7);
        ctx.save();
        ctx.translate(x, 322 + up);
        ctx.rotate(rot);
        fillRR(ctx, -70, -60, 140, 110, 12, C.surface, C.ink, 3);
        ctx.fillStyle = C.border;
        for (let j = 0; j < 3; j++) ctx.fillRect(-50, -30 + j * 20, 100 - j * 22, 6);
        ctx.restore();
      });
    ...
    ["Time Machine", "a sync service", "git"].forEach((name, i) => {
      const t0 = Q.copies[i],
        y = 170 + i * 205,
        p = pop(seg(l, t0 + 8, t0 + 22)),
        a = 1 - out;
      ink(
        ctx,
        [
          [830, 470],
          [960, 470 + (y + 60 - 470) * 0.55],
          [1040, y + 60],
        ],
        { w: 5, color: C.clay, seed: 30 + i, frame: l, progress: seg(l, t0, t0 + 12), alpha: a },
      );
      if (p > 0) {
        ctx.save();
        ctx.globalAlpha *= a;
        ctx.translate(1130, y + 60);
        ctx.scale(p, p);
        folder(ctx, -80, -50, 160, 110, {});
        ctx.restore();
        ctx.save();
        ctx.globalAlpha *= a * Math.min(1, p);
        chip(ctx, 1240, y + 32, name, { size: 34, bg: C.surface, fg: C.cocoa, border: C.ink });
        ctx.restore();
      }
    });
  }
  ...
  if (l >= Q.finderBack) {
    const fk = backOut(seg(l, Q.finderBack, Q.finderBack + 16)),
      bump = 1 + 0.03 * Math.sin(seg(l, Q.drop + 12, Q.drop + 24) * Math.PI);
    around(ctx, 640, 450, fk * bump, Math.min(1, fk * 2), () =>
      vaultList(ctx, D_R, l, { shown: () => 1, tag: () => 0 }),
    );
    const lid = seg(l, 712, 728) * (1 - seg(l, Q.drop + 12, Q.drop + 20));
    trashCan(ctx, 1175, 640, backOut(seg(l, Q.trash, Q.trash + 14)), lid);
    const home = { x: 1175, y: 262 },
      p = ptrAt(l),
      drag = l >= Q.grab && l < Q.drop,
      gone = seg(l, Q.drop, Q.drop + 12);
    const tx = drag ? p.x : l >= Q.drop ? 1175 : home.x,
      ty = drag ? p.y - 6 : l >= Q.drop ? lerp(560, 600, gone) : home.y;
    if (gone < 1)
      appTile(
        ctx,
        tx,
        ty,
        backOut(seg(l, Q.tile, Q.tile + 14)) * (drag ? 0.82 : 1) * (1 - gone),
        1 - seg(l, Q.grab - 4, Q.grab),
      );
    ...
    if (l >= Q.hop[0])
      actor(ctx, env, l, {
        pose: l < Q.hop[1] ? "walking" : "stays_local",
        x: h.x,
        y: h.y,
        h: lerp(180, 300, seg(l, Q.hop[0], Q.hop[1])),
        lift: h.lift,
        squash: h.squash,
        flip: l < Q.hop[1],
      });
  }
  if ((l >= 360 && l < 452) || (l >= 680 && l < 770)) {
    const p = ptrAt(l);
    pointer(ctx, p.x, p.y, press(l, Q.dbl[0]) || press(l, Q.dbl[1]) || press(l, Q.grab, 10));
  }
  ...
  if (l < Q.hop[0])
    host(ctx, env, l, {
      x: 1830,
      h: 180,
      pose: l < 250 ? "inbox" : l < 380 ? "thoughtful" : l < 530 ? "notes" : l < 660 ? "base" : "attention",
    });
  lowerThird(
    ctx,
    l,
    100,
    245,
    "Your vault is an ordinary directory you pick.",
    "Notes are Markdown, chats are Markdown.",
  );
  ...
  shots: [
    {
      ...
      draw: (c, l, e) => {
        ...
        titleCard(c, e, l, l, { no: 2, title: ["One folder.", "Every way out", "stays open."], pose: "inbox" });
      },
    },
    {
      id: "demo",
    ...
    },
    {
      ...
      draw: (c, l, e) => {
        c.setTransform(e.scale, 0, 0, e.scale, 0, 0);
        endCard(c, e, DEMO_END + l, l, { pose: "inbox" });
      },
    },
  ],
  audio: makeScore({
...
export const ep02Derive: DeriveSpec = {
  ...
  vertical: [
    {
      frame: 110,
      len: 135,
      crop: { x: 970, y: 60, w: 910, h: 790 },
      title: "An ordinary directory.",
      sub: "Notes and chats are Markdown.",
      hi: "directory",
    },
    {
      frame: 388,
      len: 105,
      crop: { x: 975, y: 84, w: 910, h: 766 },
      title: "Open it in other apps.",
      sub: "rotli is never the only door.",
      hi: "other apps",
    },
    {
      frame: 530,
      len: 120,
      crop: { x: 340, y: 120, w: 1220, h: 680 },
      title: "Back up your way.",
      sub: "Time Machine, a sync service, git.",
      hi: "your way",
    },
    {
      frame: 660,
      len: 120,
      crop: { x: 250, y: 50, w: 1050, h: 800 },
  ...
  slides: [
    {
      frame: 345,
      crop: { x: 770, y: 0, w: 1060, h: 860 },
      title: "An ordinary directory you pick.",
      sub: "rotli's metadata is rebuildable.",
      hi: "directory",
    },
    {
      frame: 505,
      crop: { x: 360, y: 10, w: 1510, h: 850 },
      title: "Open your work in other apps.",
      sub: "rotli is never the only door.",
      hi: "other apps",
    },
    {
      frame: 640,
      crop: { x: 340, y: 120, w: 1220, h: 680 },
      title: "Back up the folder your way.",
      sub: "No export ritual.",
      hi: "your way",
    },
    {
      frame: 775,
      crop: { x: 250, y: 50, w: 1330, h: 800 },
      title: "Delete the app.",
      sub: "The work still makes sense.",
      hi: "Delete",
    },
