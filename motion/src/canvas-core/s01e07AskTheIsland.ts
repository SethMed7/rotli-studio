...
const mark = (ctx: Ctx, g: Parameters<typeof glyph>[1], x: number, y: number, s: number, col: string, acc: string) => {
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(s, s);
  glyph(ctx, g, col, acc);
...
const noteIcon = (ctx: Ctx, x: number, y: number, token = false) => {
  ...
  ctx.lineTo(x + 26, y - 20);
  ctx.closePath();
  ctx.fillStyle = token ? C.clay : R.border;
  ctx.fill();
  ctx.stroke();
  ctx.restore();
};
...
const fileIcon = (ctx: Ctx, kind: "dir" | "md" | "pdf", x: number, y: number) => {
  ...
  if (kind === "dir") {
    ...
    ctx.beginPath();
    ctx.roundRect(x, y - 26, 16, 8, 3);
    ctx.fill();
    ctx.beginPath();
    ctx.roundRect(x, y - 21, 36, 26, 4);
    ctx.fill();
    return;
  }
  const col = kind === "pdf" ? R.failure : R["text-muted"];
  fillRR(ctx, x + 5, y - 30, 26, 34, 4, R.surface, col, 3);
  ctx.fillStyle = col;
...
const pdfPage = (ctx: Ctx, x: number, y: number, s: number, rot: number, f: number) => {
  ...
  for (let i = 0; i < 5; i++)
    ink(
      ...
      [
        [-70, -40 + i * 28],
        [-70 + [130, 110, 140, 90, 120][i], -40 + i * 28],
      ],
      { w: 4, color: R.border, seed: 60 + i, frame: f, wob: 0.5, boil: 0.3 },
    );
  fillRR(ctx, 22, 84, 64, 32, 8, R.failure);
  text(ctx, "PDF", 54, 108, { size: 20, weight: 600, align: "center", color: R["on-accent"] });
  ctx.restore();
};

// ---------------------------------------------------------------- the Chat window (shared by ask · notes · pdf)
const WIN = { x: 110, y: 56, w: 1400, h: 790 },
  CX = 160,
  CR = 920,
  DIV = 960,
  PX = 996,
  PR = 1470;
const Q1 = "What's left before the trip?",
  Q2 = "Make this a PDF.",
  REPLY = "Made Trip plan.pdf, beside this chat.";
const ANSWER = [
  "The ferry is booked: Saturday, 8:30.",
  "Still to pack: the snorkel.",
  "Not planned yet: the pink lake.",
];
const READ = ["Trip ideas", "Ferry times", "Packing list"];
const VAULT = [
  { t: "Trip ideas", read: true, token: true },
  { t: "Ferry times", read: true },
  { t: "Packing list", read: true },
  { t: "Call Sam" },
  { t: "Salt lakes" },
  { t: "Quokka facts" },
];
const MODELS = ["On this Mac", "Claude Code", "Codex", "Cursor"];
const chipRect = (ctx: Ctx) => {
  const w = measure(ctx, MODELS[0], 26, 600) + 76;
  return { x: CR - w, y: 128, w, h: 46 };
};

/** the window itself, popping in with `k` */
const window_ = (ctx: Ctx, k: number, draw: () => void) => {
  if (k <= 0) return;
  const sc = 0.94 + 0.06 * pop(k),
    cx = WIN.x + WIN.w / 2,
    cy = WIN.y + WIN.h / 2;
  ctx.save();
  ...
  ctx.scale(sc, sc);
  ctx.translate(-cx, -cy);
  appFrame(ctx, T, WIN, { sidebar: false });
  draw();
  ctx.restore();
};
...
const header = (ctx: Ctx, open: boolean) => {
  ...
  ctx.save();
  ctx.strokeStyle = R["accent-text"];
  ctx.lineWidth = 3;
  ctx.strokeRect(c.x + 16, c.y + 14, 18, 18);
  for (let i = 0; i < 3; i++) {
    ctx.beginPath();
    ctx.moveTo(c.x + 19 + i * 6, c.y + 9);
    ctx.lineTo(c.x + 19 + i * 6, c.y + 14);
    ctx.moveTo(c.x + 19 + i * 6, c.y + 32);
    ctx.lineTo(c.x + 19 + i * 6, c.y + 37);
    ctx.stroke();
  }
  ctx.restore();
  text(ctx, MODELS[0], c.x + 44, c.y + 32, { size: 26, weight: 600, color: R["accent-text"] });
  ctx.beginPath();
  ctx.moveTo(CR - 26, c.y + 20);
  ctx.lineTo(CR - 19, c.y + 28);
  ctx.lineTo(CR - 12, c.y + 20);
  ctx.strokeStyle = R["text-muted"];
  ctx.lineWidth = 3;
  ctx.lineCap = "round";
  ctx.stroke();
  ...
  const gx = c.x - 38;
  ctx.beginPath();
  ctx.arc(gx, 151, 22, 0, 6.29);
  ctx.fillStyle = R.surface;
  ctx.fill();
  ctx.strokeStyle = R.border;
  ctx.lineWidth = 2;
  ctx.stroke();
  mark(ctx, "globe", gx, 151, 0.55, R["text-muted"], R["text-muted"]);
  ctx.fillStyle = R.border;
  ctx.fillRect(CX, 190, CR - CX, 2);
};
...
const bubble = (ctx: Ctx, s: string, y: number, k: number) => {
  if (k <= 0) return;
  const R = T.roles,
    w = measure(ctx, s, 30, 500) + 52,
    x = CR - w;
  ctx.save();
  ctx.translate(CR, y + 30);
  ctx.scale(k, k);
  ctx.translate(-CR, -y - 30);
  fillRR(ctx, x, y, w, 62, 22, R.tint);
  text(ctx, s, x + 26, y + 42, { size: 30, color: R.text });
  ctx.restore();
};
const dots = (ctx: Ctx, x: number, y: number, l: number, t0: number) => {
  for (let i = 0; i < 3; i++) {
    ctx.fillStyle = T.roles["text-muted"];
    ctx.beginPath();
    ctx.arc(x + i * 26, y - Math.max(0, Math.sin((l - t0) / 3 - i)) * 9, 7, 0, 6.29);
    ctx.fill();
  }
};
...
const readRow = (ctx: Ctx, l: number, at: number[]) => {
  if (l < at[0]) return;
  text(ctx, "Read", CX, 320, { size: 26, color: T.roles["text-muted"] });
  let x = CX + 76;
  READ.forEach((s, i) => {
    const k = pop(seg(l, at[i], at[i] + 10)),
      w = measure(ctx, s, 26, 600) + 30;
    if (k > 0) {
      ctx.save();
      ctx.translate(x + w / 2, 306);
      ctx.scale(k, k);
      ctx.translate(-x - w / 2, -306);
      tagChip(ctx, T, x, 320, s, 26);
      ctx.restore();
    }
    x += w + 12;
  });
...
const answer = (ctx: Ctx, l: number, at: number[]): [number, number] | null => {
  if (l < at[0]) return null;
  const R = T.roles;
  let cur: [number, number] | null = null;
  fillRR(ctx, CX, 348, CR - CX, 170, 18, R.ground, R.border, 2);
  ANSWER.forEach((a, i) => {
    const k = seg(l, at[i], at[i] + 24);
    if (k <= 0) return;
    const s = typed(a, k),
      y = 398 + i * 46;
    text(ctx, s, CX + 28, y, { size: 30, weight: i === 0 ? 600 : 500, color: R.text });
    if (k < 1) cur = [CX + 28 + measure(ctx, s, 30, i === 0 ? 600 : 500), y];
  });
  return cur;
};
...
const composer = (ctx: Ctx, l: number, draft: string) => {
  const R = T.roles;
  fillRR(ctx, CX, 726, CR - CX, 66, 20, R.surface, R.border, 2);
  if (draft) {
    text(ctx, draft, CX + 26, 769, { size: 30, color: R.text });
    caret(ctx, T, CX + 26 + measure(ctx, draft, 30, 500), 769, l, 32);
  } else text(ctx, "Ask the island…", CX + 26, 769, { size: 28, color: R["text-muted"] });
  ctx.fillStyle = draft ? R.accent : R.border;
  ctx.beginPath();
  ctx.arc(CR - 38, 759, 22, 0, 6.29);
  ctx.fill();
  ...
  ctx.lineJoin = "round";
  ctx.beginPath();
  ctx.moveTo(CR - 38, 770);
  ctx.lineTo(CR - 38, 748);
  ctx.moveTo(CR - 47, 757);
  ctx.lineTo(CR - 38, 748);
  ctx.lineTo(CR - 29, 757);
  ctx.stroke();
};
...
const panelHead = (ctx: Ctx, title: string, icon: "dir" | "note", k = 1) => {
  const R = T.roles;
  ctx.fillStyle = R.border;
  ctx.fillRect(DIV, WIN.y + 70, 2, WIN.h - 70);
  fading(ctx, k, () => {
    if (icon === "dir") fileIcon(ctx, "dir", PX, 160);
    else noteIcon(ctx, PX + 4, 160);
    text(ctx, title, PX + 50, 164, { size: 30, weight: 600, color: R.text });
    ctx.fillStyle = R.border;
    ctx.fillRect(PX, 190, PR - PX, 2);
  });
};
/** ask: the vault's notes, and the ones the model reads */
const vaultPanel = (ctx: Ctx, l: number, sweep: [number, number]) => {
  const R = T.roles;
  panelHead(ctx, "Island", "dir");
  const at = seg(l, sweep[0], sweep[1]) * VAULT.length;
  VAULT.forEach((n, i) => {
    const y = 256 + i * 64,
      passed = at > i + 0.5,
      lit = l >= sweep[0] && Math.abs(at - i - 0.5) < 0.6 && at < VAULT.length;
    if (lit) fillRR(ctx, PX - 12, y - 42, PR - PX + 24, 58, 12, R["surface-2"]);
    if (n.read && passed) fillRR(ctx, PX - 12, y - 42, PR - PX + 24, 58, 12, R.tint);
    noteIcon(ctx, PX + 4, y, n.token);
    text(ctx, n.t, PX + 50, y, {
      size: 28,
      weight: n.read && passed ? 600 : 500,
      color: n.read && passed ? R.text : R["text-muted"],
    });
    if (n.read && passed) {
      const k = pop(
        seg(
          l,
          sweep[0] + ((i + 0.5) / VAULT.length) * (sweep[1] - sweep[0]),
          sweep[0] + ((i + 0.5) / VAULT.length) * (sweep[1] - sweep[0]) + 10,
        ),
      );
      ctx.save();
      ctx.translate(PR - 20, y - 12);
      ctx.scale(k, k);
      ink(
        ctx,
        [
        ...
        { w: 4.5, color: R["accent-text"], wob: 0.2, boil: 0 },
      );
      ctx.restore();
    }
  });
  if (l >= sweep[1])
    text(ctx, "Read 3 of 6 notes, on this Mac", PX, 680, {
      size: 26,
      color: R["text-muted"],
      alpha: seg(l, sweep[1], sweep[1] + 12),
    });
...
const NOTE_ROWS: { t: string; h?: boolean }[] = [
  { t: "The trip", h: true },
  { t: "Ferry booked: Sat, 8:30." },
  { t: "To pack: the snorkel." },
  { t: "Open: the pink lake." },
];
const notesPanel = (ctx: Ctx, l: number, at: number[], upd: number) => {
  const R = T.roles;
  panelHead(ctx, "Conversation notes", "note");
  const u = pop(seg(l, upd, upd + 12)) * (1 - seg(l, upd + 120, upd + 132));
  if (u > 0) {
    ctx.save();
    ctx.globalAlpha *= Math.min(1, u);
    const w = measure(ctx, "rewritten", 26, 600) + 36;
    fillRR(ctx, PR - w, 214, w, 44, 22, R.success);
    text(ctx, "rewritten", PR - w / 2, 245, { size: 26, weight: 600, align: "center", color: R["on-accent"] });
    ctx.restore();
  }
  NOTE_ROWS.forEach((r, i) => {
    const k = pop(seg(l, at[i], at[i] + 12));
    if (k <= 0) return;
    const y = 300 + i * 66 + (i ? 14 : 0);
    ctx.save();
    ctx.globalAlpha *= Math.min(1, k);
    ctx.translate(PX, y - 10);
    ctx.scale(1, k);
    ctx.translate(-PX, -y + 10);
    if (r.h) text(ctx, r.t, PX, y, { size: 30, weight: 600, color: R.text });
    else {
      ctx.fillStyle = R.accent;
      ctx.beginPath();
      ctx.arc(PX + 10, y - 10, 6, 0, 6.29);
      ctx.fill();
      text(ctx, r.t, PX + 32, y, { size: 28, color: R.text });
    }
    ctx.restore();
  });
};
...
const filesPanel = (ctx: Ctx, l: number, pdfAt: number) => {
  const R = T.roles;
  panelHead(ctx, "chats", "dir");
  const rows: { name: string; kind: "md" | "pdf"; at: number; note: string }[] = [
    { name: "Trip chat.md", kind: "md", at: -99, note: "this chat" },
    { name: "Trip plan.pdf", kind: "pdf", at: pdfAt, note: "new" },
  ];
  rows.forEach((r, i) => {
    const k = pop(seg(l, r.at, r.at + 14));
    if (k <= 0) return;
    const y = 262 + i * 70;
    ctx.save();
    ctx.translate(PX, y);
    ...
    fileIcon(ctx, r.kind, PX, y);
    text(ctx, r.name, PX + 50, y, { size: 28, weight: i === 1 ? 600 : 500, color: R.text });
    if (i === 1) {
      const w = measure(ctx, r.note, 26, 600) + 28;
      fillRR(ctx, PR - w, y - 36, w, 44, 22, R.success);
      text(ctx, r.note, PR - w / 2, y - 5, { size: 26, weight: 600, align: "center", color: R["on-accent"] });
...
const dune = (ctx: Ctx, f: number) => {
  ...
  ctx.beginPath();
  ctx.moveTo(-60, GY + 30);
  ctx.bezierCurveTo(400, GY - 40, 900, GY + 10, 1300, GY - 20);
  ctx.bezierCurveTo(1600, GY - 40, 1800, GY, 1990, GY - 10);
  ctx.lineTo(1990, 1140);
  ctx.lineTo(-60, 1140);
  ctx.closePath();
  ...
  ink(
    ...
    { w: 4, color: C.ink, seed: 3, frame: f, wob: 1 },
  );
  ...
  [
    ...
    const lean = Math.sin(f / 18 + i) * 6;
    for (let j = -1; j <= 1; j++)
      ink(
        ctx,
        [
          [x + j * 10, y],
          [x + j * 16 + lean, y - 34 - (j ? 0 : 12)],
        ],
        { w: 3.5, color: C.muted, seed: 20 + i * 3 + j, frame: f, wob: 0.4 },
      );
  });
};
const cold: Scene = {
  ...
  draw: (ctx, env, s) => {
    ...
    [
      ...
      card(ctx, {
        x,
        y: y + Math.sin(s.f / 16 + i) * 10,
        rot: Math.sin(s.f / 22 + i) * 0.12,
        scale: 0.75 * pop(s.t(at, at + 14)),
        ghost: true,
        frame: s.f,
      }),
    );
    ...
    const lift = easeInOut(s.t(92, 108)),
      bx = h.x + 120,
      by = h.y - h.lift - 200;
    card(ctx, {
      x: lerp(bx, 1040, lift),
      y: lerp(by, 650, lift) + Math.sin(s.f / 12) * 3,
      rot: lerp(-0.2, -0.06, lift) + Math.sin(s.f / 15) * 0.02,
      scale: lerp(0.8, 1.25, lift),
      title: "Trip ideas",
      token: true,
      lines: 3,
      seed: 7,
...
const chapter: Scene = {
  id: "chapter",
  len: 120,
  draw: (ctx, env, s) => chapterCard(ctx, env, s, { no: 7, title: ["Ask the island."], pose: "ai_chat" }),
};
...
const A = {
  type: [24, 84],
  send: 92,
  sweep: [104, 150] as [number, number],
  read: [116, 126, 136],
  dots: 150,
  ans: [168, 194, 220],
  ok: 252,
...
const ask: Scene = {
  ...
  draw: (ctx, env, s) => {
    ...
    window_(ctx, s.t(0, 16), () => {
      ...
      if (dk > 0) {
        const R = T.roles,
          ...
          dx = CR - dw,
          dy = c.y + c.h + 8,
          hv = A.hover.filter((h) => l >= h).length;
        ctx.save();
        ...
        MODELS.forEach((m, i) => {
          const y = dy + 10 + i * 58;
          if (i === 0) fillRR(ctx, dx + 8, y, dw - 16, 54, 10, R.tint);
          else if (i === hv && l < A.back) fillRR(ctx, dx + 8, y, dw - 16, 54, 10, R["surface-2"]);
          text(ctx, m, dx + 28, y + 37, { size: 28, weight: i === 0 ? 600 : 500, color: R.text });
        });
        ctx.restore();
      }
    });
    const c = chipRect(ctx),
      cy = c.y + 24;
    const p = path(l, [
      [20, 700, 820],
      [70, 760, 800],
    ...
    sideQuokka(ctx, env, s, [
      [0, "ai_chat"],
      [A.send + 4, "listening"],
      [A.ans[2] + 20, "celebrating"],
      [A.pick - 20, "thoughtful"],
      [A.back + 8, "base"],
    ]);
    fading(ctx, 1 - s.t(A.pick - 30, A.pick - 20), () =>
      thought(ctx, s, QX + 20, QY - QH + 40, "check", s.t(A.ok, A.ok + 16)),
    );
    lowerThird(
      ctx,
      l,
      24,
      262,
      "Chat reads the vault you are in, or the note you started it from,",
      "and answers from what is actually there.",
    );
    lowerThird(
      ctx,
      l,
      268,
      s.len,
      "On-device by default, or through an official",
      "Claude Code, Codex, or Cursor client already installed on your Mac.",
    );
  },
};
...
const MD: [string, number][] = [
  ["# Trip chat", 2],
  ["What's left before the trip?", 0],
  ["The ferry is booked: Saturday, 8:30.", 0],
  ["Still to pack: the snorkel.", 0],
  ["Not planned yet: the pink lake.", 0],
  ["## Conversation notes", 3],
  ["### The trip", 4],
  ["- Ferry booked: Sat, 8:30.", 2],
  ["- To pack: the snorkel.", 2],
  ["- Open: the pink lake.", 2],
];
const fileView = (ctx: Ctx, l: number) => {
  const R = T.roles;
  // the Files column: the chats folder and the chat, a plain .md file
  ctx.fillStyle = R.ground;
  ctx.fillRect(WIN.x + 2, WIN.y + 62, 370, WIN.h - 64);
  ctx.fillStyle = R.border;
  ctx.fillRect(WIN.x + 372, WIN.y + 62, 2, WIN.h - 64);
  text(ctx, "Files", WIN.x + 34, 164, { size: 28, weight: 600, color: R.text });
  fileIcon(ctx, "dir", WIN.x + 34, 240);
  text(ctx, "chats", WIN.x + 84, 240, { size: 28, weight: 600, color: R.text });
  fillRR(ctx, WIN.x + 20, 272, 336, 60, 12, R.tint);
  fileIcon(ctx, "md", WIN.x + 64, 312);
  text(ctx, "Trip chat.md", WIN.x + 114, 312, { size: 28, weight: 600, color: R.text });
  fileIcon(ctx, "dir", WIN.x + 34, 392);
  text(ctx, "wiki", WIN.x + 84, 392, { size: 28, color: R["text-muted"] });
  // the file, raw: the transcript and the Conversation notes section, plain Markdown
  const x = WIN.x + 420;
  text(ctx, "chats/Trip chat.md", x, 164, { size: 28, weight: 600, color: R["text-muted"], font: FONT.mono });
  ctx.fillStyle = R.border;
  ctx.fillRect(x, 190, WIN.x + WIN.w - 50 - x, 2);
  const hk = easeOut(seg(l, N.hl, N.hl + 16));
  if (hk > 0) {
    ctx.save();
    ctx.globalAlpha *= hk;
    fillRR(ctx, x - 18, 250 + 5 * 48 - 38, 660, 4 * 48 + 12, 12, R.tint);
    ctx.restore();
  }
  MD.forEach(([ln, syn], i) => {
    const k = seg(l, N.flip[1] - 6 + i * 3, N.flip[1] + 6 + i * 3);
    if (k > 0) raw(ctx, T, x, 250 + i * 48, ln, syn, k, 30);
  });
};
const notes: Scene = {
  id: "notes",
  ...
  draw: (ctx, env, s) => {
    const l = s.l,
      flip = seg(l, N.flip[0], N.flip[1]),
      showFile = flip >= 0.5,
      sy = Math.abs(Math.cos(flip * Math.PI));
    ctx.save();
    ...
    window_(ctx, 1, () => {
      if (!showFile) {
        header(ctx, false);
        bubble(ctx, Q1, 208, 1);
        readRow(ctx, 999, A.read);
        answer(ctx, 999, A.ans);
        composer(ctx, l, "");
        notesPanel(ctx, l, N.rows, N.upd);
        // the file button in the chat header: opens the chat as the file it is
        const R = T.roles,
          bx = chipRect(ctx).x - 96;
        ctx.beginPath();
        ctx.arc(bx, 151, 22, 0, 6.29);
        ctx.fillStyle = l >= N.click - 20 ? R.tint : R.surface;
        ctx.fill();
        ctx.strokeStyle = R.border;
        ctx.lineWidth = 2;
        ctx.stroke();
        mark(ctx, "doc", bx, 151, 0.5, R["text-muted"], R["text-muted"]);
      } else fileView(ctx, l);
    });
    ctx.restore();
    const bx = chipRect(ctx).x - 96,
      p = path(l, [
        [0, CR - 120, 250],
        [110, 1060, 560],
        [150, 1060, 560],
...
const pdf: Scene = {
  ...
  draw: (ctx, env, s) => {
    const l = s.l,
      back = seg(l, 0, 14),
      sy = Math.abs(Math.sin((back * Math.PI) / 2));
    ctx.save();
    const cy = WIN.y + WIN.h / 2;
    ctx.translate(0, cy);
    ctx.scale(1, Math.max(0.02, sy));
    ...
    window_(ctx, 1, () => {
      ...
      if (l >= P.reply[0]) {
        const R = T.roles,
          k = seg(l, P.reply[0], P.reply[1]),
          t = typed(REPLY, k);
        fillRR(ctx, CX, 620, CR - CX, 76, 18, R.ground, R.border, 2);
        text(ctx, t, CX + 28, 669, { size: 30, color: R.text });
        if (k < 1) cur = [CX + 28 + measure(ctx, t, 30, 500), 669];
      }
    ...
    const hk = easeInOut(seg(l, P.hand[0], P.hand[1])),
      from = [PR - 120, 330],
      to = [QX - 10, 470];
    if (l >= P.filed + 8)
      pdfPage(
        ctx,
        lerp(from[0], to[0], hk),
        lerp(from[1], to[1], hk) - Math.sin(hk * Math.PI) * 140,
        0.35 + 0.45 * hk + 0.04 * Math.sin(s.f / 14) * hk,
        lerp(0, -0.12, hk) + Math.sin(s.f / 17) * 0.03 * hk,
        s.f,
      );
    const p = path(l, [
      [0, 1260, 720],
    ...
    lowerThird(
      ...
      20,
      s.len,
      "In the Mac app it can also make a Word document,",
      "a PDF, or a board, filed beside the chat.",
    );
  },
};
...
const payoff: Scene = {
  ...
  draw: (ctx, env, s) => {
    ...
    card(ctx, {
      x: 680,
      y: 900 + Math.sin(s.f / 13) * 6,
      rot: -0.1 + Math.sin(s.f / 19) * 0.03,
      scale: 1.1 * pop(s.t(36, 52)),
      title: "Trip ideas",
      token: true,
      lines: 3,
      seed: 7,
...
const end: Scene = {
  id: "end",
  len: 120,
  draw: (ctx, env, s) => storyEnd(ctx, env, s, { next: "The Locked Box", pose: "waving" }),
};

const SCENES = [cold, chapter, ask, notes, pdf, payoff, end];
const AS = sceneStart(SCENES, "ask"),
  NO = sceneStart(SCENES, "notes"),
  PD = sceneStart(SCENES, "pdf"),
  PO = sceneStart(SCENES, "payoff");
export const s01e07AskTheIsland: Film = story({
  id: "s01e07AskTheIsland",
  no: 7,
  title: "Ask the island.",
  atmosphere: "iris-dusk",
  scenes: SCENES,
  score: {
    key: 5,
    melody: 2,
    pops: [
      ...
      [166, 84],
      [AS + A.send + 2, 84],
      ...A.read.map((r, i) => [AS + r, [79, 81, 84][i]] as [number, number]),
      ...A.ans.map((r, i) => [AS + r, [86, 88, 91][i]] as [number, number]),
      [AS + A.pick, 91],
      [AS + A.back, 88],
      [NO + N.upd, 84],
      ...N.rows.map((r, i) => [NO + r, [79, 81, 84, 86][i]] as [number, number]),
      [NO + N.click, 91],
      [NO + N.flip[1], 93],
      [PD + P.send + 2, 84],
      [PD + P.reply[0], 88],
...
export const s01e07Derive: DeriveSpec = {
  no: 7,
  series: "season-one",
  label: "Rotli · Season One · 07",
  vertical: [
    {
      frame: 20,
      len: 180,
      crop: { x: 300, y: 260, w: 1400, h: 800 },
      title: "Evening on the island.",
      sub: "What's left before the trip?",
      hi: "Evening",
    },
    {
      frame: AS + 20,
      len: 270,
      crop: { x: 100, y: 40, w: 1420, h: 830 },
      title: "Ask the island.",
    ...
    },
    {
      ...
      sub: "The chat is saved as Markdown too.",
      hi: "notes",
    },
    {
      frame: PD + 40,
      len: 180,
      crop: { x: 120, y: 50, w: 1780, h: 1000 },
      title: "A PDF of the plan.",
