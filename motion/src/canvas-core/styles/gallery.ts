...
// ---------------------------------------------------------------- polylines
export const polyLen = (s: P[]) =>
  s.reduce((a, p, i) => (i ? a + Math.hypot(p[0] - s[i - 1][0], p[1] - s[i - 1][1]) : 0), 0);
...
export const resample = (s: P[], n: number): P[] => {
  ...
  for (let k = 0; k < n; k++) {
    const d = (L * k) / (n - 1);
    while (j < s.length - 1 && cum[j] < d) j++;
    const f = (d - cum[j - 1]) / (cum[j] - cum[j - 1] || 1);
    out.push(lerpP(s[j - 1], s[j], clamp(f)));
  }
  return out;
};
const normals = (s: P[], closed = false): P[] =>
  s.map((_, i) => {
    const n = s.length,
      a = closed ? s[(i - 1 + n) % n] : s[Math.max(0, i - 1)],
      b = closed ? s[(i + 1) % n] : s[Math.min(n - 1, i + 1)];
    const dx = b[0] - a[0],
      dy = b[1] - a[1],
      l = Math.hypot(dx, dy) || 1;
    return [-dy / l, dx / l];
  });
export const area = (s: P[]) =>
  s.reduce((a, p, i) => {
  ...
  for (let i = 1; i < knots.length; i++)
    if (t <= knots[i][0]) {
      const [t0, v0] = knots[i - 1],
        [t1, v1] = knots[i],
        f = (t - t0) / (t1 - t0);
      return lerp(v0, v1, (1 - Math.cos(f * Math.PI)) / 2);
    }
...
export const body = (ctrl: P[], hw: (t: number) => number, n = 120): Body => {
  const spine = resample(smooth(ctrl, false, 16), n),
    nrm = normals(spine);
  const idx = (t: number) => clamp(t) * (n - 1);
  const pick = (arr: P[], t: number): P => {
    const f = idx(t),
      i = Math.floor(f),
      k = Math.min(n - 1, i + 1);
    return lerpP(arr[i], arr[k], f - i);
  };
  const at = (t: number, s: number): P => {
    const p = pick(spine, t),
      q = pick(nrm, t),
      w = hw(t);
    return [p[0] + q[0] * w * s, p[1] + q[1] * w * s];
  };
...
export const trace = (c: Ctx, pts: P[], closed = true) => {
  c.beginPath();
...
export const fillShape = (g: Gfx, pts: P[], color: string, alpha = 1) => {
  const b = boxOf(pts);
  g.touch(b.x0 - 2, b.y0 - 2, b.x1 + 2, b.y1 + 2);
  const c = g.cur;
  c.globalAlpha = alpha;
  c.fillStyle = color;
  trace(c, pts);
...
export const clipped = (g: Gfx, pts: P[], fn: () => void) => {
  const b = boxOf(pts);
  g.touch(b.x0 - 2, b.y0 - 2, b.x1 + 2, b.y1 + 2);
  const c = g.cur;
  c.save();
...
export const inkRibbon = (center: P[], o: InkOpts): P[] => {
  ...
  const s = closed ? center : center,
    n = s.length,
    nr = normals(s, closed),
    out = closed ? (area(s) > 0 ? -1 : 1) : side,
    r = rng(seed);
  ...
  for (let i = 0; i < n; i++) {
    ...
    const j1 = (r() - 0.5) * rough,
      j2 = (r() - 0.5) * rough;
    L.push([s[i][0] - q[0] * (wv + j1), s[i][1] - q[1] * (wv + j1)]);
    R.push([s[i][0] + q[0] * (wv + j2), s[i][1] + q[1] * (wv + j2)]);
  }
...
export const ink = (g: Gfx, center: P[], color: string, o: InkOpts, alpha = 1) => {
  const pts = inkRibbon(center, o),
    b = boxOf(pts);
  g.touch(b.x0 - 3, b.y0 - 3, b.x1 + 3, b.y1 + 3);
  const c = g.cur;
  c.globalAlpha = alpha;
  c.fillStyle = color;
  if (o.closed) {
    ...
    c.beginPath();
    pts.slice(0, n).forEach(([x, y], i) => (i ? c.lineTo(x, y) : c.moveTo(x, y)));
    c.closePath();
    pts.slice(n).forEach(([x, y], i) => (i ? c.lineTo(x, y) : c.moveTo(x, y)));
    c.closePath();
    c.fill("evenodd");
  } else {
    trace(c, pts);
...
export const blob = (cx: number, cy: number, rx: number, ry: number, seed: number, k = 0.18, n = 14, rot = 0): P[] => {
  ...
  for (let i = 0; i < n; i++) {
    const a = (i / n) * Math.PI * 2,
      f =
        1 +
        (fractal(seed, Math.cos(a) * 3 + 10, Math.sin(a) * 3 + 10, 0.6, 0.6, 2) - 0.5) * 2 * k +
        Math.sin(a * 3 + ph) * k * 0.3;
...
export const hatchRuns = (
  ...
  const c = Math.cos(angle),
    ...
    cx = (b.x0 + b.x1) / 2,
    cy = (b.y0 + b.y1) / 2,
    R = Math.hypot(b.x1 - b.x0, b.y1 - b.y0) / 2 + gap,
    r = rng(seed),
    runs: P[][] = [];
  for (let v = -R; v <= R; v += gap) {
    const off = (r() - 0.5) * gap * 0.35;
    let cur: P[] = [];
    for (let u = -R; u <= R; u += step) {
      const x = cx + u * c - (v + off) * s,
        y = cy + u * s + (v + off) * c;
      if (keep(x, y)) cur.push([x, y]);
      else if (cur.length) {
...
export const inside = (pts: P[], x: number, y: number) => {
  let k = false;
  for (let i = 0, j = pts.length - 1; i < pts.length; j = i++) {
    const a = pts[i],
      b = pts[j];
    if (a[1] > y !== b[1] > y && x < ((b[0] - a[0]) * (y - a[1])) / (b[1] - a[1]) + a[0]) k = !k;
  }
  return k;
};
