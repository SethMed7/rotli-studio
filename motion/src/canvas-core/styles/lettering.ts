...
const oval = (cx: number, a0 = 50, a1 = 420): P[] => {
  const out: P[] = [];
  for (let a = a0; a <= a1; a += 22) {
    const r = (a * Math.PI) / 180;
    out.push([cx + 0.29 * Math.cos(r), 0.5 + 0.5 * Math.sin(r)]);
  }
  return out;
};
const on = (cx: number, deg: number): P => [
  cx + 0.29 * Math.cos((deg * Math.PI) / 180),
  0.5 + 0.5 * Math.sin((deg * Math.PI) / 180),
];
...
const ANI: Stroke = [
  ...
  [2.12, 0.0],
  [2.28, 0.06],
  [D1 - 0.04, 0.3], // the i's exit just touches the d's bowl
];
const D1S: Stroke = [...d(D1), [D1 + 1.06, 0.14], on(O1, 208)]; // out of the d and up to kiss the first o
const OO: Stroke = oval(O1),
  BRIDGE: Stroke = [on(O1, 22), [(O1 + O2) / 2, 0.86], on(O2, 158)],
  OO2: Stroke = oval(O2);
...
export const place = (n: Nib, [x, y]: P): P => [
  n.origin[0] + (x + y * Math.tan(n.slant)) * n.em,
  n.origin[1] - y * n.em,
];

export const inkStroke = (n: Nib, s: Stroke, tail = 0): Inked => {
  const up = sample(s, false, 18),
    L = up.reduce((a, p, i) => (i ? a + Math.hypot(p[0] - up[i - 1][0], p[1] - up[i - 1][1]) : 0), 0);
  ...
  const raw = u.map((_, i) => {
    const a = u[Math.max(0, i - 1)],
      b = u[Math.min(u.length - 1, i + 1)],
      dx = b[0] - a[0],
      dy = b[1] - a[1],
      l = Math.hypot(dx, dy) || 1;
    return Math.pow(clamp(-dy / l), 1.7);
  });
  ...
  const w = sm.map((v, i) => {
    const t = i / (N - 1),
      taper = Math.min(1, t / 0.03, (1 - t) / (0.08 + tail));
    return (n.hair + (n.shade - n.hair) * v) * (0.35 + 0.65 * clamp(taper));
  });
  const spine = u.map((p) => place(n, p)),
    len = [0];
  for (let i = 1; i < N; i++)
...
export const outlineOf = (k: Inked, upto = Infinity): P[] => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const a = s[Math.max(0, i - 1)],
      ...
      nx = -dy / l,
      ny = dx / l;
    Lft.push([s[i][0] + nx * k.w[i], s[i][1] + ny * k.w[i]]);
    Rgt.push([s[i][0] - nx * k.w[i], s[i][1] - ny * k.w[i]]);
  }
