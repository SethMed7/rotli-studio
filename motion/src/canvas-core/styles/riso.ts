...
export const INK = { pink: "#ff48b0", blue: "#0078bf", yellow: "#ffd400", teal: "#00838a", red: "#f15060" },
  PAPER = "#f5efe2";
...
export const drum = (g: Gfx, off: P, fn: () => void) =>
...
export const across = (pts: P[], axis: "x" | "y") => {
  ...
  return (x: number, y: number) => {
    ...
    if (!span) {
      ...
      for (let i = 0, j = pts.length - 1; i < pts.length; j = i++) {
        const p = pts[i],
          r = pts[j];
        if (p[o] > key !== r[o] > key) {
          const v = p[a] + ((key - p[o]) * (r[a] - p[a])) / (r[o] - p[o]);
          lo = Math.min(lo, v);
          hi = Math.max(hi, v);
        }
      }
    ...
    return span[1] <= span[0] ? 0 : Math.max(0, Math.min(1, (q[a] - span[0]) / (span[1] - span[0])));
  };
};
...
export const nearField = (lines: P[][], cell = 28) => {
  ...
  lines.forEach((l) => {
    for (let i = 1; i < l.length; i++) {
      const a = l[i - 1],
        b = l[i],
        n = Math.max(1, Math.ceil(Math.hypot(b[0] - a[0], b[1] - a[1]) / 4));
  ...
  return (x: number, y: number) => {
    ...
    for (let i = -1; i <= 1; i++)
      for (let j = -1; j <= 1; j++)
        for (const p of grid.get(`${cx + i},${cy + j}`) ?? []) d = Math.min(d, Math.hypot(p[0] - x, p[1] - y));
    return d;
  };
};
// a fold that wanders like a real one, plus the little side branches folds always have
export const wander = (pts: P[], amp: number, wl: number, seed: number): P[] => {
  const s = sample(pts, false, 14),
    r = rng(seed),
    ph = r() * 6.28;
  let len = 0;
  return s.map((p, i) => {
    ...
    const ends = Math.min(1, i / 8, (s.length - 1 - i) / 8),
      w = (Math.sin((len / wl) * 6.28 + ph) + 0.45 * Math.sin((len / wl) * 15.1 + ph * 2)) * amp * ends;
    return [p[0] - (dy / l) * w, p[1] + (dx / l) * w] as P;
  });
};
export const twigs = (line: P[], every: number, seed: number, lenLo = 16, lenHi = 38): P[][] => {
  const r = rng(seed),
    out: P[][] = [];
  let acc = every * r();
  for (let i = 2; i < line.length - 2; i++) {
    ...
    acc = 0;
    if (r() < 0.25) continue;
    const dx = line[i + 1][0] - line[i - 1][0],
      dy = line[i + 1][1] - line[i - 1][1],
      a = Math.atan2(dy, dx) + (r() < 0.5 ? 1 : -1) * (1.1 + r() * 0.8),
      L = lenLo + r() * (lenHi - lenLo),
      b = (r() - 0.5) * 0.9;
    out.push([
      line[i],
      [line[i][0] + Math.cos(a) * L * 0.5, line[i][1] + Math.sin(a) * L * 0.5],
      [line[i][0] + Math.cos(a + b) * L, line[i][1] + Math.sin(a + b) * L],
    ]);
  }
  return out;
};
export const plateMarks = (g: Gfx, W: number, H: number, inks: string[]) => {
  ...
  drum(g, [0, 0], () =>
    inks.forEach((col, i) => {
      const c = g.cur;
      c.fillStyle = col;
      c.fillRect(W / 2 - inks.length * 16 + i * 32, H - 34, 28, 14);
    }),
  );
  g.paper("paper", 0.26);
  g.paper("coldpress", 0.22);
};
