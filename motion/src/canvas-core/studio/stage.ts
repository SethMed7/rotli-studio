...
export const lookAssets = (ids: readonly string[]) =>
  Object.fromEntries(ids.map((id) => [`look:${id}`, `../library/companion-looks/${id}.png`]));
export const STYLE_NAME = BRAND.character.styles as Record<string, string>;
export const STYLE_HEX: Record<string, string> = {
  line: "#ffffff",
  cocoa: "#C6845F",
  green: "#6FA68B",
  ocean: "#6EABD4",
...
const FAMILY_IDS: Record<string, [string, string]> = {
  ...
  grove: ["grove-light", "grove-dark"],
  iris: ["iris-light", "iris-dark"],
  midnight: ["midnight-light", "midnight-dark"],
};
/** the companion colour that belongs with each family */
export const FAMILY_QUOKKA: Record<string, string> = {
  rotli: "#C6845F",
  paper: "#C6845F",
  ocean: "#6EABD4",
  grove: "#6FA68B",
...
export const familyThemes = (fam: Family): [Theme, Theme] => {
  const L = THEME_LIST.find((t) => familyKey(t.family) === fam && t.mode === "light"),
    D = THEME_LIST.find((t) => familyKey(t.family) === fam && t.mode === "dark");
  if (L && D) return [L, D];
...
export const useFamily = (fam: Family, uiLight?: Theme, uiDark?: Theme) => {
  ...
  const known =
    THEME_LIST.some((t) => familyKey(t.family) === fam) ||
    (FAMILY_IDS[fam] && THEME_LIST.some((t) => t.id === FAMILY_IDS[fam][0]));
  const [L, D] = familyThemes(known ? fam : firstFamily()),
    l = L.roles,
    d = D.roles;
  Object.assign(C, {
    linen: l.ground,
    surface: l.surface,
    surface2: l["surface-2"],
    peach: l.tint,
    border: l.border,
    cocoa: l.text,
    ink: l.text,
    muted: l["text-muted"],
    clay: l.accent,
    clayText: l["accent-text"],
    olive: l.success,
    oliveText: l.success,
    oliveBright: d.success,
    blue: l["syntax-blue"],
    night: d.ground,
    nightSurface: d.surface,
    nightSurface2: d["surface-2"],
    nightText: d.text,
    nightMuted: d["text-muted"],
    nightBorder: d.border,
  });
  if (uiLight) Object.assign(uiLight, L);
  if (uiDark) Object.assign(uiDark, D);
  LOOK.family = fam;
  LOOK.quokka = FAMILY_QUOKKA[fam] ?? LOOK.quokka;
};
