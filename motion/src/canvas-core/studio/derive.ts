...
const home = (ep: Film) => {
  if (ep.meta.family) useFamilyUI(ep.meta.family);
};
...
export type DeriveSpec = {
  ...
  series: string;
  label?: string;
  line?: boolean /* line-style episode: the vertical end uses the line-art mark */;
  vertical: (Beat & { len: number })[];
  slides: Beat[];
  single: Beat;
};

const tag = (ctx: Ctx, no: number | string, x: number, y: number) => {
  const s =
      typeof no === "string" ? no.replace(/ · /g, "  ·  ") : `Rotli in 30 seconds  ·  ${String(no).padStart(2, "0")}`,
    w = measure(ctx, s, 26, 600) + 44;
  fillRR(ctx, x - w / 2, y, w, 52, 26, C.surface, C.ink, 3);
  text(ctx, s, x, y + 35, { size: 26, weight: 600, align: "center" });
};
...
const framed = (ctx: Ctx, env: Env, ep: Film, b: Beat, x0: number, y: number, w0: number, k = 0, maxH = 900) => {
  const w = Math.min(w0, (maxH * b.crop.w) / b.crop.h),
    x = x0 + (w0 - w) / 2;
  const z = 1 - 0.1 * k,
    cw = b.crop.w * z,
    ch = b.crop.h * z,
    h = (w * b.crop.h) / b.crop.w;
  fillRR(ctx, x - 3, y - 3, w + 6, h + 6, 27, C.ink);
  CAPTIONS.on = false;
  reframe(
    ctx,
    env,
    ep,
    b.frame,
    { x: b.crop.x + (b.crop.w - cw) / 2, y: b.crop.y + (b.crop.h - ch) / 2, w: cw, h: ch },
    { x, y, w, h, r: 24 },
  );
  CAPTIONS.on = true;
  return y + h;
...
export const verticalCut = (id: string, ep: Film, d: DeriveSpec): Film => {
  ...
  const draw = (ctx: Ctx, l: number, env: Env) => {
    ...
    if (l >= t) {
      const e = l - t;
      drift(ctx, env, l, 0.6);
      lockup(ctx, W / 2, 760, seg(e, 0, 40), l, { s: 0.9 });
      text(ctx, (BRAND as { promise?: string }).promise ?? BRAND.tagline, W / 2, 1120, {
        size: 38,
        weight: 600,
        align: "center",
        color: C.cocoa,
      ...
      if (d.line) lineQuokka(ctx, W / 2, 1640 + Math.sin(l / 16) * 6, 420, C.cocoa);
      else host(ctx, env, l, { x: W / 2, y: 1640, h: 420, pose: "waving" });
      return;
    }
    let i = 0;
    for (let j = 0; j < starts.length; j++) if (l >= starts[j]) i = j;
    const b = d.vertical[i],
      k = l - starts[i];
    tag(ctx, d.label ?? d.no, W / 2, 110);
    caption(ctx, [b.title], W / 2, 300, seg(k, 0, 16), l, {
      size: measure(ctx, b.title, 82, 600) > 980 ? 64 : 82,
      hi: b.hi,
    });
    if (b.sub)
      text(ctx, b.sub, W / 2, 380, { size: 38, weight: 500, align: "center", color: C.muted, alpha: seg(k, 8, 20) });
    const cw = W - 80,
      ch = (cw * b.crop.h) / b.crop.w; // the footage floats a little on two periods, so a held frame still moves
    framed(
      ctx,
      env,
      ep,
      { ...b, frame: b.frame + k },
      40 + Math.sin(l / 23) * 8,
      Math.max(470, 1080 - Math.min(ch, 1200) / 2) + Math.cos(l / 29) * 6,
      cw,
      k / b.len,
      1200,
    );
    fillRR(ctx, W / 2 - 150, 1770, 300, 76, 38, C.surface, C.ink, 3);
    text(ctx, BRAND.url, W / 2, 1820, { size: 34, weight: 600, align: "center" });
  };
  const shots = d.vertical.map((b, i) => ({
    id: `beat${i + 1}`,
    start: starts[i],
    end: starts[i] + b.len,
    draw: (c: Ctx, lo: number, e: Env) => draw(c, lo + starts[i], e),
  }));
  shots.push({ id: "end", start: t, end: total, draw: (c, lo, e) => draw(c, lo + t, e) });
  return {
    meta: { title: id, W, H, fps: 30, bpm: 120, durationFrames: total, raster: "cpu" },
    assets: { images: ep.assets.images, fonts: FONTS },
    shots,
    audio: makeScore({
      frames: total,
      energeticFrom: 60,
      endAt: Math.floor(total / 60) * 60 - 60,
      bellAt: [t],
      pops: starts.map((s, i) => [s, [79, 81, 84, 86, 88][i % 5]] as [number, number]),
    }),
  };
};
...
export const carousel = (id: string, ep: Film, d: DeriveSpec, cover: { title: string[]; hi?: string }): Film => {
  ...
  return {
    ...
    shots: Array.from({ length: n }, (_, i) => ({
      ...
      draw: (c: Ctx, l: number, e: Env) => draw(c, i, i * 15 + l, e),
    })),
  };
};
...
export const single = (id: string, ep: Film, d: DeriveSpec): Film => {
  const [W, H] = FORMATS["ig-portrait"],
    b = d.single;
  return {
    meta: { title: id, W, H, fps: 30, bpm: 120, durationFrames: 15, raster: "cpu" },
    assets: { images: ep.assets.images, fonts: FONTS },
    shots: [
      {
        id: "single",
        ...
        draw: (ctx, l, env) => {
          ...
          ctx.setTransform(env.scale, 0, 0, env.scale, 0, 0);
          envGround(ctx, l, "base", { w: W, h: H });
          caption(ctx, [b.title], W / 2, 200, 1, l, { size: measure(ctx, b.title, 76, 600) > 960 ? 60 : 76, hi: b.hi });
          if (b.sub) text(ctx, b.sub, W / 2, 282, { size: 36, weight: 500, align: "center", color: C.muted });
          const y = framed(ctx, env, ep, b, 60, 360, 960, 0, 840);
          text(
            ctx,
            [BRAND.url, (BRAND as { cta?: string }).cta].filter(Boolean).join("  ·  "),
            W / 2,
            Math.min(H - 60, y + 90),
            { size: 32, weight: 600, align: "center", color: C.clayText },
          );
        },
      },
    ],
...
const lineQuokka = (ctx: Ctx, x: number, y: number, h: number, col: string) => {
  ...
  const p = POSES.stays_local,
    k = h / p.vb;
  ctx.save();
  ctx.translate(x - h / 2, y - h * 0.93);
  ctx.scale(k, k);
  ctx.fillStyle = col;
