...
const meta = (env: Env, id: string, img: CanvasImageSource & { width: number; height: number }): Meta => {
  const key = `look-meta:${id}`,
    hit = env.cache.get(key) as Meta | undefined;
  if (hit) return hit;
  const N = img.width,
    L = env.canvas(N, N);
    ...
    for (let x = 0; x < N; x++) {
      ...
      while (st.length) {
        const j = st.pop()!,
          ...
          jy = (j - jx) / N;
        b.n++;
        b.x0 = Math.min(b.x0, jx);
        b.x1 = Math.max(b.x1, jx);
        b.y0 = Math.min(b.y0, jy);
        b.y1 = Math.max(b.y1, jy);
        for (const q of [j - 1, j + 1, j - N, j + N])
          if (q >= 0 && q < N * N && !seen[q] && dark(q)) {
            seen[q] = 1;
            st.push(q);
          }
      }
  ...
  const cand = blobs
    .filter((b) => {
      const w = b.x1 - b.x0 + 1,
        h = b.y1 - b.y0 + 1;
      return b.n > N * N * 0.0002 && b.n < N * N * 0.003 && b.n / (w * h) > 0.5 && w / h > 0.6 && w / h < 1.7;
    })
    .sort((a, b) => a.y0 - b.y0);
  const eyes = cand
    .filter((b) => Math.abs(b.y0 - cand[0].y0) < N * 0.03)
    .slice(0, 2)
    .map((b) => {
      const x = (b.x0 + b.x1) / 2,
        y = (b.y0 + b.y1) / 2,
        r = Math.max(b.x1 - b.x0, b.y1 - b.y0) / 2,
        s = (Math.round(y - r * 2.4) * N + Math.round(x)) * 4;
      return { x: x / N, y: y / N, r: r / N, fur: `rgb(${d[s]},${d[s + 1]},${d[s + 2]})` };
    });
  const m = { foot: { x: fn ? fx / fn / N : 0.5, y: y1 / N }, eyes };
  env.cache.set(key, m);
  return m;
};

export type LookActor = {
  id: string;
  x: number;
  y: number;
  h: number;
...
export const drawLook = (ctx: Ctx, env: Env, a: LookActor) => {
  ...
  const img = env.image?.(`look:${a.id}`) as (CanvasImageSource & { width: number; height: number }) | undefined;
  if (!img) throw new Error(`look ${a.id} not in the piece's assets`);
  const M = meta(env, a.id, img),
    sq = (a.squash ?? 1) * (1 + 0.014 * Math.sin(((a.f ?? 0) / 40) * Math.PI * 2)),
    s = a.h;
  ctx.save();
  ctx.globalAlpha *= a.alpha ?? 1;
  ctx.translate(a.x, a.y);
  ctx.rotate(((a.lean ?? 0) * Math.PI) / 180);
  ctx.scale((a.flip ? -1 : 1) / Math.sqrt(sq), sq);
  ctx.translate(-M.foot.x * s, -M.foot.y * s);
  ...
  if (b > 0.05)
    for (const e of M.eyes) {
      ...
      ctx.fillStyle = e.fur;
      ctx.beginPath();
      ctx.ellipse(ex, ey, r * 1.3, r * 1.3 * Math.min(1, b * 1.4), 0, 0, Math.PI * 2);
      ctx.fill();
      if (b > 0.5) {
        ...
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.arc(ex, ey - r * 0.6, r * 1.0, Math.PI * 0.22, Math.PI * 0.78);
        ctx.stroke();
      }
    }
  ctx.restore();
