...
import { INK, LITHO, PAPER, REG, across, drum, nearField, plateMarks, screen } from "../styles/riso";
import { blob, clamp, fillShape, inside, smooth } from "../styles/gallery";
...
const ROCKS: P[][] = [
  blob(872, 910, 74, 36, 81, 0.25, 16, -0.1),
  blob(150, 972, 92, 44, 82, 0.22, 16, 0.08),
  blob(600, 800, 34, 18, 83, 0.25, 12),
];
const TOWER: P[] = [
  [278, 580],
  [382, 580],
...
const DOME: P[] = Array.from({ length: 25 }, (_, i) => {
  const a = Math.PI + (i / 24) * Math.PI;
  return [330 + Math.cos(a) * 32, 178 + Math.sin(a) * 28] as P;
});
const COTTAGE: P[] = [
  [132, 516],
  [252, 516],
...
const topOf = (() => {
  ...
  for (let x = 0; x <= W; x++) {
    ...
    for (let i = 0, j = LAND.length - 1; i < LAND.length; j = i++) {
      const a = LAND[i],
        b = LAND[j];
      if (a[0] > x !== b[0] > x) m = Math.min(m, a[1] + ((x - a[0]) * (b[1] - a[1])) / (b[0] - a[0]));
    }
    t.push(m);
  }
  return (x: number) => t[Math.max(0, Math.min(W, Math.round(x)))];
})();
...
const turf = (x: number, y: number) =>
  inside(LAND, x, y) && y - topOf(x) < 46 && Math.abs(topOf(x + 6) - topOf(x - 6)) < 9;
...
const beam = (x: number, y: number) => {
  const dx = x - LAMP[0],
    dy = y - LAMP[1],
    d = Math.hypot(dx, dy);
  if (d < 30 || dx > 0) return 0;
  const a = Math.atan2(dy, dx),
    c = -2.94,
    spread = 0.07 + d * 0.0001;
  return Math.pow(clamp(1 - Math.abs(a - c) / spread), 0.7) * clamp(1.25 - d / 820);
};
...
const glitter = (x: number, y: number) => {
  const col = clamp(1 - Math.abs(x - SUN[0]) / (70 + (y - HY) * 0.35));
  return col * clamp((fractal(51, x * 0.5, y * 3, 0.06, 0.06, 2) - 0.46) * 5);
};
const foam = (x: number, y: number) =>
  sea(x, y) &&
  shore(x, y) <
    Math.max(0, fractal(61, x, y, 0.035, 0.05, 3) - 0.38) * 70 + (fractal(62, x, y, 0.2, 0.2, 2) > 0.62 ? 5 : 0); // broken, bunched where the swell breaks

// ---------------------------------------------------------------- the three plates
const yellowPlate = (g: Gfx) =>
  drum(g, REG.yellow, () => {
    ...
    screen(
      ...
      ANG.yellow,
      (x, y) => {
        if (y >= HY || occluded(x, y) || inSun(x, y)) return 0;
        return 0.12 + 0.62 * Math.pow(y / HY, 1.6) + 0.5 * Math.exp(-Math.pow(sunD(x, y) / 260, 2)) + 0.45 * beam(x, y);
      },
      INK.yellow,
    );
    fillShape(
      ...
      Array.from(
        ...
        (_, i) =>
          [
            SUN[0] + Math.cos((i / 48) * Math.PI * 2) * SR,
            Math.min(HY, SUN[1] + Math.sin((i / 48) * Math.PI * 2) * SR),
          ] as P,
      ),
      INK.yellow,
    );
    ...
    screen(
      ...
      PITCH,
      ANG.yellow,
      (x, y) => (turf(x, y) ? 0.74 + 0.25 * (x / 540) : cliff(x, y) || rock(x, y) ? 0.2 : 0),
      INK.yellow,
    );
    ...
    screen(
      ...
      PITCH,
      ANG.yellow,
      (x, y) => (inside(TOWER, x, y) ? 0.26 * Math.pow(towerX(x, y), 1.8) : 0),
      INK.yellow,
      TOWER,
    );
    fillShape(g, LANTERN, INK.yellow);
    fillShape(g, blob(LAMP[0], LAMP[1], 40, 34, 7, 0.1, 16), INK.yellow, 0.5);
    [
      [150, 532, 26, 22],
      [212, 532, 26, 22],
    ].forEach(([x, y, w, h]) =>
...
const pinkPlate = (g: Gfx) =>
  drum(g, REG.pink, () => {
    ...
    screen(
      ...
      ANG.pink,
      (x, y) => {
        if (y >= HY || occluded(x, y)) return 0;
        if (inSun(x, y)) return 0.18 + 0.7 * clamp((y - (SUN[1] - SR * 0.3)) / (SR * 0.9));
        return (
          0.3 * (1 - y / HY) +
          0.6 * Math.exp(-Math.pow((y - 430) / 170, 2)) +
          0.35 * Math.exp(-Math.pow(sunD(x, y) / 220, 2)) +
          0.08
        );
      },
      INK.pink,
    ...
    screen(
      ...
      PITCH,
      ANG.pink,
      (x, y) => (sea(x, y) && !foam(x, y) ? 0.42 * clamp(1 - (y - HY) / 230) + 0.55 * glitter(x, y) : 0),
      INK.pink,
    );
    // the tower's four red bands, the dome, the cottage roof
    const band = (y0: number, y1: number): P[] => {
      const hw = (y: number) => 52 - ((580 - y) / 328) * 18;
      return [
        [330 - hw(y1), y1],
        [330 + hw(y1), y1],
        [330 + hw(y0), y0],
    ...
    [
      ...
      [460, 502],
      [540, 580],
    ].forEach(([a, b]) => fillShape(g, band(b, a), INK.pink));
    fillShape(g, DOME, INK.pink);
    fillShape(g, GALLERY, INK.pink, 0.6);
    ...
    screen(
      ...
      ANG.pink,
      (x, y) =>
        cliff(x, y) ? 0.2 + 0.55 * clamp((x - 220) / 320) * (0.6 + 0.6 * fractal(84, x, y, 0.04, 0.02, 2)) : 0,
      INK.pink,
    ); // the cliff face warms toward the sun
    ROCKS.forEach((rk) =>
      screen(
        g,
        all,
        ...
        ANG.pink,
        (x) => {
          const b = rk.reduce((m, p) => Math.max(m, p[0]), -1e9),
            a = rk.reduce((m, p) => Math.min(m, p[0]), 1e9);
          return 0.15 + 0.6 * clamp((x - a) / (b - a));
        },
        INK.pink,
        rk,
      ),
...
const bluePlate = (g: Gfx) =>
  drum(g, REG.blue, () => {
    ...
    screen(
      ...
      (x, y) => {
        if (y >= HY || occluded(x, y) || inSun(x, y)) return 0;
        return (0.05 + 0.66 * Math.pow(1 - y / HY, 1.4)) * (1 - 0.8 * beam(x, y));
      },
      INK.blue,
    );
    ...
    screen(
      ...
      ANG.blue,
      (x, y) => {
        if (!sea(x, y) || foam(x, y)) return 0;
        const k = (y - HY) / (W - HY),
          swell = 0.5 + 0.5 * Math.sin(y * 0.09 + fractal(71, x, y, 0.004, 0.02, 2) * 6);
        return (0.34 + 0.34 * k) * (0.75 + 0.35 * swell) * (1 - 0.6 * glitter(x, y));
      },
      INK.blue,
    );
    ...
    screen(
      ...
      (x, y) =>
        turf(x, y)
          ? 0.44 - 0.2 * (x / 540)
          : cliff(x, y)
            ? 0.5 + 0.25 * clamp((y - topOf(x)) / 160) - 0.25 * clamp((x - 300) / 260)
            : rock(x, y)
              ? 0.62
              : 0,
      INK.blue,
    ...
    screen(
      ...
      PITCH,
      ANG.blue,
      (x, y) => (inside(TOWER, x, y) ? 0.08 + 0.62 * Math.pow(1 - towerX(x, y), 1.8) : 0),
      INK.blue,
      TOWER,
    );
    ...
    solid(
      Array.from(
        { length: 12 },
        (_, i) => [330 + Math.cos((i / 12) * Math.PI * 2) * 6, 146 + Math.sin((i / 12) * Math.PI * 2) * 6] as P,
      ),
    );
    solid([
      [329, 126],
    ...
    const pen = (pts: P[], w: number, seed: number, op = 0.9) => {
      g.pen(pts, { w, color: INK.blue, seed, wobble: 0.6, boil: 0, opacity: op, retrace: false });
    };
    pen(
      [
        [278, 580],
    ...
    for (let i = 0; i < 38; i++) {
      const y = HY + 14 + Math.pow(r(), 1.3) * 460,
        x = 560 + r() * 520,
        l = 18 + (y - HY) * 0.12 + r() * 20;
      if (!sea(x, y) || foam(x, y) || Math.abs(x - SUN[0]) < 60) continue;
      pen(
        [
          [x, y],
          [x + l * 0.5, y - 3],
