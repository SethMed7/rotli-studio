...
import { blob, bounds, clamp, fillShape, hatchRuns, inside, lerp, lerpP, mix, smooth } from "../styles/gallery";
...
const scribble = (
  ...
  const { angle, gap, w, alpha, seed, keep = () => true, over = 1.035 } = o,
    r = rng(seed),
    b = bounds(region);
  const cx = (b.x0 + b.x1) / 2,
    cy = (b.y0 + b.y1) / 2,
    grown = region.map(([x, y]) => [cx + (x - cx) * over, cy + (y - cy) * over] as P);
  const runs = hatchRuns(
    { x0: b.x0 - 8, y0: b.y0 - 8, x1: b.x1 + 8, y1: b.y1 + 8 },
    angle,
    gap,
    (x, y) => inside(grown, x, y) && keep(x, y),
    5,
    seed,
  );
  const c = g.cur;
  g.touch(b.x0 - 14, b.y0 - 14, b.x1 + 14, b.y1 + 14);
  c.save();
  c.lineCap = "round";
  c.lineJoin = "round";
  c.strokeStyle = color;
  ...
  runs.forEach((run, i) => {
    const pts = (i % 2 ? [...run].reverse() : run)
      .filter((_, k, a) => k % 3 === 0 || k === a.length - 1)
      .map(([x, y]) => [x + (r() - 0.5) * 1.6, y + (r() - 0.5) * 1.6] as P);
    const jump = prev ? Math.hypot(pts[0][0] - prev[0], pts[0][1] - prev[1]) : 0;
    c.globalAlpha = alpha * (0.72 + r() * 0.4);
    c.beginPath();
    if (prev && jump < gap * 3.2) {
      c.moveTo(prev[0], prev[1]);
      pts.forEach(([x, y]) => c.lineTo(x, y));
    } // the turn of the zigzag
    else {
...
const line = (g: Gfx, pts: P[], color: string, w: number, seed: number, op = 0.92, wob = 1) =>
  g.pen(pts, { w, color, seed, wobble: wob, boil: 0, opacity: op, retrace: true, taper: 1 });
...
const GORES = 12,
  phiAt = (k: number) => -Math.PI / 2 + (k / GORES) * Math.PI;
...
const envelope = (g: Gfx) => {
  ...
  for (let k = 0; k < GORES; k++) {
    const a = merid(phiAt(k)),
      b2 = merid(phiAt(k + 1)),
      gore = [...a, ...[...b2].reverse()],
      col = RAINBOW[k % 6];
    const phiC = (phiAt(k) + phiAt(k + 1)) / 2,
      litG = clamp(Math.cos(phiC + 0.62) * 0.9 + 0.1);
    wax(g, () => {
      ...
      const sh = (x: number, y: number) => {
        const u = (x - CX) / R,
          v = (y - EQ) / R;
        return u * 0.9 + v * 0.8 + (fractal(9, x, y, 0.02, 0.02, 2) - 0.5) * 0.35 > 0.15 + litG * 0.4;
      };
      scribble(g, gore, dark(col), { angle: 0.55, gap: 6.5, w: 5, alpha: 0.72, seed: 102 + k * 7, keep: sh });
      const deep = (x: number, y: number) => {
        const u = (x - CX) / R,
          v = (y - EQ) / R;
        return u * 0.9 + v * 0.9 > 0.75 + litG * 0.35;
      };
      scribble(g, gore, mix(dark(col), K.black, 0.3), {
        angle: -0.6,
        gap: 7,
        w: 4.4,
        alpha: 0.6,
      ...
      const hi = (x: number, y: number) => {
        const u = (x - CX) / R + 0.42,
          v = (y - EQ) / R + 0.52;
        return u * u * 1.4 + v * v < 0.1 + (fractal(10, x, y, 0.03, 0.03, 2) - 0.5) * 0.06;
      };
  ...
  wax(g, () => {
    for (let k = 1; k < GORES; k++)
      line(
        g,
        merid(phiAt(k)).filter((_, i) => i % 4 === 0),
        mix(K.black, RAINBOW[k % 6], 0.2),
        1.1,
        200 + k,
        0.8,
        0.8,
      );
    [0.3, 0.55].forEach((f, j) => {
      const i = Math.round(f * (profile.length - 1)),
        [w, y] = profile[i],
        pts: P[] = [];
      for (let q = 0; q <= 24; q++) {
        const ph = -Math.PI / 2 + (q / 24) * Math.PI;
        pts.push([CX + w * Math.sin(ph), y + w * Math.cos(ph) * TILT]);
      }
      line(g, pts, K.black, 1.1, 230 + j, 0.7, 0.9);
    });
    const top = merid(0)[0];
    fillShape(g, blob(top[0], top[1] + 6, 20, 8, 240, 0.1, 10), K.blue);
    line(g, [...blob(top[0], top[1] + 6, 20, 8, 240, 0.1, 10)], dark(K.blue), 1.5, 241, 0.9, 0.6);
    line(
      g,
      merid(-Math.PI / 2).filter((_, i) => i % 3 === 0),
      K.black,
      2.2,
      250,
      0.92,
...
// ---------------------------------------------------------------- below the envelope
const rigging = (g: Gfx) => {
  const throatY = EQ + R * 1.48,
    tw = R * 0.2,
    skirt: P[] = [
      [CX - tw, throatY - 4],
      [CX + tw, throatY - 4],
      [CX + tw * 0.9, throatY + 34],
      [CX - tw * 0.9, throatY + 34],
    ];
  ...
  wax(g, () => {
    [
      ...
      line(
        ...
        [
          [CX + a, throatY + 30],
          [lerp(CX + a, b2, 0.5), lerp(throatY + 30, bk.y0, 0.5)],
          [b2, bk.y0],
        ],
        "#4a3a36",
        0.9,
    ...
    fillShape(g, burner, "#aab4c4");
    scribble(g, burner, "#7f8aa0", { angle: 0.3, gap: 4, w: 3.5, alpha: 0.8, seed: 320, keep: (x) => x > CX - 4 });
    ...
    const flame = smooth(
      ...
      true,
      8,
    ); // it disappears up into the skirt
    fillShape(g, flame, PAPER);
    scribble(g, flame, K.orange, { angle: 1.5, gap: 4.5, w: 5, alpha: 0.9, seed: 330 });
    scribble(g, flame, K.yellow, {
      angle: 1.4,
      gap: 4.5,
      w: 4.5,
      alpha: 0.95,
      seed: 331,
      keep: (x, y) => Math.abs(x - CX) < 10 - (792 - y) * 0.08 && y > 748,
    });
    scribble(g, flame, K.red, {
      angle: 0.2,
      gap: 6,
      w: 3.5,
      alpha: 0.55,
      seed: 332,
      keep: (x) => Math.abs(x - CX) > 12,
    });
    line(
      g,
      flame.filter((_, i) => i % 3 === 0),
      K.red,
      1.4,
      333,
      0.8,
  ...
  wax(g, () => {
    ...
    [
      ...
      const hx = x as number,
        hy = bk.y0 - 30;
      fillShape(g, blob(hx, hy + 18, 13, 14, 340 + i, 0.1, 10), PAPER);
      scribble(g, blob(hx, hy + 18, 13, 14, 340 + i, 0.1, 10), c as string, {
        angle: 1.2,
        gap: 4,
        w: 4,
        alpha: 0.9,
      ...
      fillShape(g, blob(hx, hy, 11, 12, 345 + i, 0.08, 10), "#f6c8a0");
      scribble(g, blob(hx, hy, 11, 12, 345 + i, 0.08, 10), "#e8a37c", {
        angle: 0.4,
        gap: 5,
        w: 3.4,
        alpha: 0.6,
      ...
      if (s === 1) {
        ...
        fillShape(g, blob(hx - 31, hy - 24, 5, 5, 356, 0.1, 8), "#f6c8a0");
      } // waving
      else {
        line(
          g,
    ...
    for (let yy = bk.y0 + 8; yy < bk.y1 - 3; yy += 8)
      for (let xx = bk.x0 + 4 + ((yy / 8) % 2) * 6; xx < bk.x1 - 6; xx += 12)
        line(
          g,
          [
            [xx, yy],
    ...
    fillShape(g, rim, "#7a4a2c");
    scribble(g, rim, "#5a3420", { angle: 0.1, gap: 3, w: 3, alpha: 0.7, seed: 353, keep: (x) => x > CX });
    line(g, [...basket, basket[0]], "#4a2c1c", 1.9, 354, 0.9, 0.7);
    line(g, [...rim, rim[0]], "#3a2216", 1.6, 355, 0.9, 0.6);
  });
};
...
const sky = (g: Gfx) => {
  wax(g, () => {
    ...
    scribble(g, all, mix(K.sky, K.blue, 0.4), {
      angle: -0.5,
      gap: 12,
      w: 5,
      alpha: 0.4,
      seed: 11,
      over: 1,
      keep: (x, y) => y < 380 + (fractal(12, x, y, 0.01, 0.01, 2) - 0.5) * 200,
    });
  });
  // the sun, where the light comes from
  wax(g, () => {
    const sun = blob(150, 140, 62, 60, 20, 0.05, 16);
    fillShape(g, sun, PAPER);
    scribble(g, sun, K.yellow, { angle: 0.9, gap: 4.5, w: 5.5, alpha: 0.95, seed: 21 });
    scribble(g, sun, K.orange, {
      angle: -0.7,
      gap: 7,
      w: 4,
      alpha: 0.45,
      seed: 22,
      keep: (x, y) => x - 150 + (y - 140) > 20,
    });
    line(
      g,
      sun.filter((_, i) => i % 3 === 0),
      K.orange,
      1.8,
      23,
      0.9,
    ...
    for (let i = 0; i < 12; i++) {
      ...
      line(
        g,
        [
          [150 + Math.cos(a) * r0, 140 + Math.sin(a) * r0],
          [150 + (Math.cos(a) * (r0 + r1)) / 2, 140 + (Math.sin(a) * (r0 + r1)) / 2 + 2],
          [150 + Math.cos(a) * r1, 140 + Math.sin(a) * r1],
        ],
        K.yellow,
        2.4,
        24 + i,
  ...
  const cloud = (cx: number, cy: number, s: number, seed: number) => {
    ...
    for (let i = 0; i < 6; i++)
      puffs.push(
        blob(
          cx + (i - 2.5) * 32 * s + (r() - 0.5) * 10,
          cy - Math.sin((i / 5) * Math.PI) * 28 * s + (r() - 0.5) * 8,
          (34 + r() * 16) * s,
          (28 + r() * 10) * s,
          seed + i,
          0.08,
    ...
    wax(g, () => {
      ...
      puffs.forEach((p, i) => {
        ...
        const runs: P[][] = [];
        p.forEach((q) => {
          const hidden = puffs.some((o, j) => j !== i && inside(o, q[0], q[1]));
          if (!hidden) run.push(q);
          else if (run.length) {
            runs.push(run);
            run = [];
          }
        ...
        runs
          .filter((r2) => r2.length > 3)
          .forEach((r2, k) =>
            line(
              g,
              r2.filter((_, m) => m % 3 === 0 || m === r2.length - 1),
              "#7fa9d8",
              1.5,
              seed + 20 + i * 5 + k,
              0.8,
  ...
  cloud(250, 470, 1.0, 40);
  cloud(870, 150, 0.85, 60);
  cloud(930, 560, 0.7, 80);
  ...
  wax(g, () => {
    ...
    for (let k = 0; k < 6; k++) {
      const a = m(-Math.PI / 2 + (k / 6) * Math.PI),
        b2 = m(-Math.PI / 2 + ((k + 1) / 6) * Math.PI),
        gore = [...a, ...[...b2].reverse()];
      fillShape(g, gore, PAPER);
      scribble(g, gore, [K.pink, K.yellow, K.sky][k % 3], { angle: 1.3, gap: 4, w: 3.6, alpha: 0.75, seed: 90 + k });
      env.push(...a);
    }
    line(
      g,
      m(-Math.PI / 2).filter((_, i) => i % 3 === 0),
      "#5a4a6a",
      1.1,
      97,
      0.8,
...
const land = (g: Gfx) => {
  ...
  wax(g, () => {
    fillShape(g, back, PAPER);
    scribble(g, back, "#8ccf5a", { angle: 0.15, gap: 5.5, w: 6, alpha: 0.9, seed: 400, over: 1 });
    scribble(g, back, K.green, {
      angle: -0.4,
      gap: 8,
      w: 5,
      alpha: 0.5,
      seed: 401,
      over: 1,
      keep: (x, y) => y > 890 + Math.sin(x * 0.01) * 12,
    });
    line(
      g,
      back.filter((_, i) => i % 4 === 0 && back[i][1] < 1000),
      "#2f7a33",
      1.8,
      402,
      0.85,
    ...
    [
      ...
      fillShape(g, c, PAPER);
      scribble(g, c, K.green, { angle: 1.1, gap: 4, w: 4.5, alpha: 0.9, seed: 430 + i });
      scribble(g, c, K.leaf, { angle: 0.3, gap: 5, w: 3.6, alpha: 0.7, seed: 440 + i, keep: (xx) => xx > x + 2 });
      line(
        g,
        c.filter((_, k) => k % 3 === 0),
        "#1f5a28",
    ...
    fillShape(g, front, PAPER);
    scribble(g, front, K.green, { angle: -0.12, gap: 5, w: 6.5, alpha: 0.92, seed: 460, over: 1 });
    scribble(g, front, K.leaf, { angle: 0.5, gap: 7, w: 5, alpha: 0.55, seed: 461, over: 1, keep: (x, y) => y > 985 });
    ...
    const pw: P[] = [
      ...path.map(([x, y], i) => [x - 22 + i * 0.35, y] as P),
      ...path.map(([x, y], i) => [x + 22 - i * 0.35, y] as P).reverse(),
    ];
    scribble(g, pw, "#f3c77a", { angle: 0.2, gap: 4, w: 4.5, alpha: 0.9, seed: 462, over: 1 });
    line(
      g,
      front.filter((_, i) => i % 4 === 0 && front[i][1] < 1060),
      "#1f5a28",
      2,
      463,
      0.9,
