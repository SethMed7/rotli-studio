...
import { blob, clamp, fillShape, hatchRuns, inside, lerpP, smooth } from "../styles/gallery";
...
const B0 = 34,
  B1 = 1046,
  LEDGE = 958; // the board's inner edges
...
const scumble = (
  ...
  region: P[],
  color: string,
  o: { angle: number; gap: number; w: number; alpha: number; seed: number; keep?: (x: number, y: number) => boolean },
) => {
  const { angle, gap, w, alpha, seed, keep = () => true } = o,
    r = rng(seed);
  let x0 = 1e9,
    y0 = 1e9,
    x1 = -1e9,
  ...
  const c = g.cur;
  g.touch(x0 - 12, y0 - 12, x1 + 12, y1 + 12);
  c.save();
  c.lineCap = "round";
  c.lineJoin = "round";
  c.strokeStyle = color;
  c.lineWidth = dust ? w * 2.2 : w;
  hatchRuns({ x0, y0, x1, y1 }, angle, gap, (x, y) => inside(region, x, y) && keep(x, y), 4, seed).forEach((run) => {
    c.globalAlpha = alpha * (0.86 + r() * 0.2);
    c.beginPath(); // a steady hand: grain comes from the board, not from stripes of pressure
    run
      .filter((_, k, a) => k % 2 === 0 || k === a.length - 1)
      .forEach(([x, y], k) => (k ? c.lineTo(x + (r() - 0.5) * 1.5, y + (r() - 0.5) * 1.5) : c.moveTo(x, y)));
    c.stroke();
  });
  c.restore();
};
...
const moon = (g: Gfx, cx: number, cy: number, r: number, lit: number, waxing: boolean, seed: number) => {
  const s = waxing ? 1 : -1,
    tx = r * (1 - 2 * lit),
    N = 40;
  const disk: P[] = Array.from({ length: 72 }, (_, i) => [
    cx + Math.cos((i / 72) * Math.PI * 2) * r,
    cy + Math.sin((i / 72) * Math.PI * 2) * r,
  ]);
  const limb: P[] = [],
    term: P[] = [];
  for (let i = 0; i <= N; i++) {
    const a = -Math.PI / 2 + (i / N) * Math.PI;
    limb.push([cx + s * Math.cos(a) * r, cy + Math.sin(a) * r]);
  }
  for (let i = 0; i <= N; i++) {
    const a = Math.PI / 2 - (i / N) * Math.PI;
    term.push([cx + s * Math.cos(a) * tx, cy + Math.sin(a) * r]);
  }
  const litShape = [...limb, ...term];
  const mare = (x: number, y: number) =>
    fractal(seed, ((x - cx) * 60) / r, ((y - cy) * 60) / r, 0.03, 0.03, 3) +
      (fractal(seed + 3, x, y, 0.45, 0.45, 2) - 0.5) * 0.16 <
    0.52 + (lit > 0.95 ? 0.06 : 0); // where the chalk skipped: the dark seas, their edges crumbling
  ...
  chalk(g, (dust) => {
    ...
    scumble(g, dust, litShape, lc, {
      angle: 0.75 + (seed % 3) * 0.1,
      gap: 3.2,
      w: 4.2,
      alpha: 0.42,
      seed: seed + 8,
      keep: (x, y) => !mare(x, y),
    }); // the highlands built up across it: the seas are just less chalk
    scumble(g, dust, litShape, CHALK, {
      angle: 0.5,
      gap: 5,
      w: 3.6,
      alpha: 0.45,
      seed: seed + 3,
      keep: (x, y) => Math.hypot(x - cx, y - cy) > r * 0.62 && (x - cx) * s > -r * 0.2,
    }); // the limb, gone over again
    if (!dust) {
      const cr = rng(seed + 4);
      for (let k = 0; k < 5; k++) {
        const a = cr() * Math.PI * 2,
          ...
          px = cx + Math.cos(a) * d,
          py = cy + Math.sin(a) * d;
        if (!inside(litShape, px, py)) continue;
        const rr = 3 + cr() * r * 0.09;
        line(
          g,
          dust,
          Array.from(
            { length: 9 },
            (_, i) => [px + Math.cos((i / 8) * Math.PI * 2) * rr, py + Math.sin((i / 8) * Math.PI * 2) * rr * 0.9] as P,
          ),
          0.9,
          CHALK,
          seed + 10 + k,
    ...
    for (let i = 0; i <= N; i++) {
      const a = Math.PI / 2 + (i / N) * Math.PI;
      back.push([cx + s * Math.cos(a) * r, cy + Math.sin(a) * r]);
    }
    line(
      g,
      dust,
      back.filter((_, i) => i % 3 === 0 || i === N),
      1.3,
      CHALK,
      seed + 6,
      0.45,
    ); // the dark limb, barely
    if (lit < 0.97)
      line(
        g,
        dust,
        term.filter((_, i) => i % 4 === 0 || i === N),
        1.1,
        CHALK,
        seed + 7,
        0.5,
...
const board = (g: Gfx, ctx: Ctx) => {
  ...
  gr.addColorStop(0, "#35493f");
  gr.addColorStop(0.62, "#27352f");
  gr.addColorStop(1, "#1b2622");
  ctx.fillStyle = gr;
  ...
  g.group(
    ...
    () => {
      ...
      for (let i = 0; i < 9; i++) {
        ...
        for (let k = 0; k <= 12; k++) {
          const a = a0 + (k / 12) * 2.2;
          pts.push([cx + Math.cos(a) * rr, cy + Math.sin(a) * rr * 0.55]);
        }
        g.pen(pts, {
          w: 40 + r() * 30,
          color: CHALK,
  ...
  g.group(
    "plain",
    () => {
      letter(g, "HOMEWORK P 42", 140, 780, { cap: 28, color: CHALK, seed: 40, opacity: 0.12 });
      letter(g, "NEWTON", 700, 796, { cap: 26, color: CHALK, seed: 41, opacity: 0.1 });
    },
    { blur: 2.4 },
  );
};
...
const frame = (g: Gfx) => {
  g.group("plain", () => {
    ...
    fillShape(
      ...
      "#8c6139",
    ); // the ledge's top face, catching the light
    for (let i = 0; i < 26; i++) {
      const y = r() * 1080,
        x = r() < 0.5 ? r() * B0 : B1 + r() * B0;
      g.pen(
        [
          [x, y],
          [x + (r() - 0.5) * 3, y + 40 + r() * 60],
        ],
        { w: 0.9, color: dark, seed: 71 + i, wobble: 0.6, boil: 0, opacity: 0.45, retrace: false },
      );
    } // grain on the sides
    for (let i = 0; i < 16; i++) {
      const x = r() * 1080,
        y = r() < 0.5 ? r() * B0 : LEDGE + 30 + r() * 80;
      g.pen(
        [
          [x, y],
          [x + 60 + r() * 90, y + (r() - 0.5) * 3],
        ],
        { w: 0.9, color: dark, seed: 100 + i, wobble: 0.6, boil: 0, opacity: 0.45, retrace: false },
      );
    }
    ...
    for (let i = 0; i < 140; i++) {
      const x = B0 + r() * (B1 - B0),
        y = LEDGE + 2 + r() * 20;
      fillShape(g, blob(x, y, 0.8 + r() * 1.8, 0.6 + r(), 110 + i, 0.3, 6), CHALK, 0.25 + r() * 0.45);
    }
    const st: P[] = [
      [690, LEDGE + 2],
      [790, LEDGE - 6],
    ...
    for (let i = 0; i < 40; i++) {
      const x = 260 + r() * 130,
        y = LEDGE + 6 + r() * 12;
      fillShape(g, blob(x, y, 1.2 + r() * 2, 1, 300 + i, 0.3, 6), CHALK, 0.35);
    }
  });
};
...
const lesson = (g: Gfx) => {
  const phases: { lit: number; waxing: boolean; label: [string, string] }[] = [
    { lit: 0.22, waxing: true, label: ["WAXING", "CRESCENT"] },
    { lit: 0.5, waxing: true, label: ["FIRST", "QUARTER"] },
    { lit: 0.8, waxing: true, label: ["WAXING", "GIBBOUS"] },
    { lit: 1, waxing: true, label: ["FULL", ""] },
    { lit: 0.8, waxing: false, label: ["WANING", "GIBBOUS"] },
    { lit: 0.5, waxing: false, label: ["LAST", "QUARTER"] },
    { lit: 0.22, waxing: false, label: ["WANING", "CRESCENT"] },
  ];
  const at = (i: number): [number, number, number] => {
    const f = i / 6,
      s = Math.sin(f * Math.PI);
    return [128 + i * 137, 612 - s * 222, 42 + s * 28];
  };
  ...
  chalk(
    ...
    (dust) => {
      ...
      for (let i = 0; i < 70; i++) {
        const x = 60 + r() * 960,
          ...
          ok =
            [0, 1, 2, 3, 4, 5, 6].every((k) => {
              const [mx, my, mr] = at(k);
              return (
                (Math.hypot(x - mx, y - my) > mr + 34 && Math.abs(y - (my + mr + 48)) > 32) || Math.abs(x - mx) > 70
              );
            }) && !(y < 180 && x > 170 && x < 910);
        if (!ok) continue;
        const big = r() < 0.12;
        ...
        } else fillShape(g, blob(x, y, 1 + r() * 1.6, 1 + r() * 1.4, 700 + i, 0.2, 6), CHALK, 0.5 + r() * 0.4);
      }
      ...
      line(g, dust, dip.slice(0, 5), 1, BLUE, 810, 0.6, 0.4);
      line(g, dust, [dip[4], dip[5], dip[6], dip[3]], 1, BLUE, 811, 0.6, 0.4);
    },
    0.18,
  );
  phases.forEach((p, i) => {
    const [x, y, r] = at(i);
    moon(g, x, y, r, p.lit, p.waxing, 1000 + i * 37);
  });
  chalk(
    g,
    (dust) => {
      // the title, underlined twice in yellow the way a teacher does when it matters
      letter(g, "PHASES OF THE MOON", 540, 86, {
        cap: 44,
        color: CHALK,
        seed: 20,
        align: "center",
      ...
      for (let k = 0; k <= 16; k++) {
        const f = k / 16,
          s = Math.sin(f * Math.PI);
        arc.push([150 + f * 780, 470 - s * 196]);
      }
      line(
        g,
        dust,
        smooth(
          arc.filter((_, k) => k % 2 === 0),
          false,
          3,
        ),
        1.6,
      ...
      const tip = arc[16],
        pre = arc[15],
        a = Math.atan2(tip[1] - pre[1], tip[0] - pre[0]);
      line(
        g,
        dust,
        [
          [tip[0] - Math.cos(a - 0.5) * 20, tip[1] - Math.sin(a - 0.5) * 20],
          tip,
          [tip[0] - Math.cos(a + 0.5) * 20, tip[1] - Math.sin(a + 0.5) * 20],
        ],
        1.8,
      ...
      line(
        ...
        Array.from({ length: 11 }, (_, k) => {
          const a2 = -0.3 + (k / 10) * Math.PI * 2.1;
          return [870 + Math.cos(a2) * 92, 858 + Math.sin(a2) * 42] as P;
        }),
        1.4,
        PINK,
        72,
