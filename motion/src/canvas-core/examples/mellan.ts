...
import { clamp } from "../styles/gallery";
...
const W = 1080,
  C: P = [540, 540],
  PLATE = 46; // the copper plate's edge; nothing outside it prints
const PITCH = 6.2; // distance between neighbouring turns, px
const MOON_R = 380,
  MOON_C: P = [548, 526];
const LIGHT = (() => {
  const v = [-0.66, -0.52, 0.54];
...
export const tone = (x: number, y: number): number => {
  ...
  for (const c of craters) {
    const du = (u - c.u) / c.r,
      dv = (v - c.v) / c.r,
      q = Math.sqrt(du * du + dv * dv);
    if (c.rays && q < 9) {
      const ang = Math.atan2(dv, du),
        ray = Math.pow(Math.abs(Math.sin(ang * 5.5 + 0.7)), 18) + Math.pow(Math.abs(Math.sin(ang * 3.2 + 2)), 30);
      alb += 0.22 * ray * Math.exp(-q * 0.28) * (q > 1.3 ? 1 : 0);
    }
    if (q > 1.4) continue;
    const toward = q > 1e-6 ? (du * L2[0] + dv * L2[1]) / q : 0; // +1 on the side of the crater nearest the light
    if (q < 1) {
      const crescent = smoothstep(-0.2, 0.5, toward) * smoothstep(0.2, 0.9, q + 0.25);
      shade *= 1 - 0.85 * c.depth * low * crescent;
      shade *= 1 + 0.3 * c.depth * smoothstep(0.1, 0.7, -toward) * low;
      alb -= (c.r > 0.15 ? 0.14 : 0.05) * c.depth;
    } else {
      const rim = 1 - smoothstep(1, 1.4, q);
      alb += 0.12 * c.depth * rim * smoothstep(-0.2, 0.6, toward);
      shade *= 1 - 0.45 * c.depth * rim * smoothstep(0, 0.7, -toward) * low;
    }
  }
...
const walk = (maxStep = Infinity): Ribbon => {
  ...
  let th = 0.6;
  while (th < thMax) {
    const step = 1.4 / Math.max(a * th, 1.4),
      m = Math.max(1, Math.ceil(step / maxStep)); // ~1.4 px of line per step
    for (let j = 0; j < m; j++) {
      ...
      const dark = 1 - tone(x, y),
        grow = Math.min(1, r / 40); // the first turns open gently from a point
      const w = (PITCH * (0.05 + 0.86 * dark) * grow) / 2;
      const tx = -Math.sin(t) * r + Math.cos(t) * a,
        ty = Math.cos(t) * r + Math.sin(t) * a,
        tl = Math.hypot(tx, ty),
        nx = ty / tl,
        ny = -tx / tl;
      ...
      laid += onPlate ? (2 * w * 1.4) / m : 0;
      ink.push(laid);
      ths.push(t); // ink that lands OFF the plate is cut away by the clip: it must not cost the film any time
    }
    th += step;
  }
...
export const stillOutline = (rb: Ribbon): P[] => [
  ...rb.base.map((i) => rb.L[i]),
  ...rb.base.map((i) => rb.R[i]).reverse(),
];
export const spiral = (progress = 1, cache?: Map<string, unknown>): P[] => {
  ...
  const n = rb.ink.length,
    want = rb.ink[n - 1] * clamp(progress);
  let k = n;
  if (progress < 1) {
    let lo = 0,
      hi = n - 1;
...
export const printPlate = (ctx: Ctx, env: Env, rib: P[] | null, zoom = 1) => {
  const s = env.scale * zoom,
    o = (1 - zoom) * C[0] * env.scale;
  ctx.setTransform(s, 0, 0, s, o, o);
  ctx.fillStyle = PAPER;
  ctx.fillRect(0, 0, W, W);
  ctx.save();
  ctx.beginPath();
  ctx.rect(PLATE, PLATE, W - 2 * PLATE, W - 2 * PLATE);
  ctx.clip(); // the plate: the spiral prints only inside it
  if (rib) {
    ctx.fillStyle = INKC;
    ctx.beginPath();
  ...
  ctx.lineWidth = 2;
  ctx.strokeRect(40, 40, W - 80, W - 80);
  ctx.strokeStyle = "rgba(255,252,240,0.7)";
  ctx.lineWidth = 1.2;
  ctx.strokeRect(38.5, 38.5, W - 80, W - 80);
};
export const drawMellan = (ctx: Ctx, frame: number, env: Env, progress = 1) => {
  void frame;
  printPlate(ctx, env, spiral(progress, env.cache));
};
