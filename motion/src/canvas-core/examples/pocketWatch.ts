...
import { bounds, clamp, fillShape, hatchRuns, inside, lerp, lerpP, smooth } from "../styles/gallery";
...
const W = ([x, y]: P): P => {
  const yy = y * SQ,
    c = Math.cos(ROT),
    s = Math.sin(ROT);
  return [C[0] + x * c - yy * s, C[1] + x * s + yy * c];
};
const polar = (r: number, a: number): P => [Math.sin(a) * r, -Math.cos(a) * r]; // a = 0 at twelve, clockwise
const ring = (r: number, n = 90, c: P = [0, 0]): P[] =>
  Array.from({ length: n }, (_, i) =>
    W([c[0] + polar(r, (i / n) * Math.PI * 2)[0], c[1] + polar(r, (i / n) * Math.PI * 2)[1]]),
  );
...
const stroke = (
  g: Gfx,
  pts: P[],
  o: { w?: number; op?: number; seed: number; wob?: number; blob?: boolean; retrace?: boolean },
) => {
  ...
  if (blob) {
    // a firm stroke leaves a bead of ink where the ball stops, a smaller one where it started
    const e = pts[pts.length - 1],
      s0 = pts[0],
      c = g.cur,
      r = rng(seed * 7);
    g.touch(e[0] - 4, e[1] - 4, e[0] + 4, e[1] + 4);
    g.touch(s0[0] - 3, s0[1] - 3, s0[0] + 3, s0[1] + 3);
    c.fillStyle = BIRO;
    c.globalAlpha = 0.8;
    c.beginPath();
    c.arc(e[0] + (r() - 0.5), e[1] + (r() - 0.5), 1.2 + r() * 0.8, 0, Math.PI * 2);
    c.fill();
    c.globalAlpha = 0.5;
    c.beginPath();
    c.arc(s0[0], s0[1], 0.8 + r() * 0.5, 0, Math.PI * 2);
    c.fill();
    c.globalAlpha = 1;
  }
};
...
const hatch = (
  ...
  hatchRuns(b, angle, gap, (x, y) => inside(region, x, y) && keep(x, y), 2.5, seed).forEach((run, k) => {
    ...
    while (i < run.length - 1) {
      const n = Math.min(run.length - 1 - i, 8 + Math.floor(r() * 22)),
        seg = run.slice(i, i + n + 1);
      if (seg.length > 1) {
        const a = seg[0],
          z = seg[seg.length - 1],
          m = lerpP(a, z, 0.5),
          bow = (r() - 0.5) * 1.6;
        stroke(g, [a, [m[0] + bow, m[1] - bow], z], {
          w: 0.82,
          op: op * (0.75 + r() * 0.35),
          seed: seed * 1000 + k * 37 + i,
          wob: 0.25,
        });
      }
      i += n + (r() < 0.25 ? 1 : 0);
...
const bezelTone = (x: number, y: number) => {
  const dx = x - C[0],
    dy = y - C[1],
    l = Math.hypot(dx, dy) || 1;
  return 0.18 + 0.82 * Math.pow(lit(dx / l, dy / l), 1.6) + ragged(x, y, 0.16);
};
const dialTone = (x: number, y: number) => {
  const dx = x - C[0],
    dy = y - C[1],
    l = Math.hypot(dx, dy) || 1,
    edge = clamp((l - R_BEZ * 0.72) / (R_BEZ * 0.26));
...
const numeral = (g: Gfx, txt: string, a: number, h: number, seed: number) => {
  ...
  const glyphs: Record<string, { w: number; strokes: [P, P, boolean][] }> = {
    ...
    V: {
      ...
      strokes: [
        [[-0.31, -0.5], [0, 0.5], true],
        [[0.31, -0.5], [0, 0.5], false],
      ],
    },
    X: {
      w: 0.62,
      strokes: [
        [[-0.31, -0.5], [0.31, 0.5], true],
        [[0.31, -0.5], [-0.31, 0.5], false],
      ],
    },
  };
  const gap = 0.1,
    total = [...txt].reduce((a2, ch) => a2 + glyphs[ch].w, 0) + gap * (txt.length - 1);
  const base = polar(R_NUM, a),
    up: P = [Math.sin(a), -Math.cos(a)],
    side: P = [Math.cos(a), Math.sin(a)];
  const at = (u: number, v: number): P =>
    W([base[0] + side[0] * u * h - up[0] * v * h, base[1] + side[1] * u * h - up[1] * v * h]);
  let x = -total / 2,
    k = 0;
  for (const ch of txt) {
    const gl = glyphs[ch],
    ...
    gl.strokes.forEach(([p, q, thick]) => {
      const A = at(cx + p[0], p[1]),
        B = at(cx + q[0], q[1]);
      stroke(g, [A, lerpP(A, B, 0.5), B], { w: 1.05, op: 0.92, seed: seed + k++, wob: 0.15 });
      if (thick) {
        const o = 0.035;
        stroke(g, [at(cx + p[0] + o, p[1]), at(cx + (p[0] + q[0]) / 2 + o, 0), at(cx + q[0] + o, q[1])], {
          w: 1.05,
          op: 0.9,
          seed: seed + k++,
          wob: 0.15,
    ...
    [-0.5, 0.5].forEach((v) => {
      const sx = gl.w / 2 + 0.07;
      stroke(g, [at(cx - sx, v), at(cx + sx, v)], { w: 0.9, op: 0.85, seed: seed + k++, wob: 0.1 });
    });
    x += gl.w + gap;
  }
};
...
const breguet = (
  ...
  const d: P = [Math.sin(a), -Math.cos(a)],
    n: P = [Math.cos(a), Math.sin(a)],
    p = (t: number, s: number): P => {
      const q = W([d[0] * t + n[0] * s, d[1] * t + n[1] * s]);
  ...
  for (let i = 0; i < 28; i++) {
    ...
    moon.push(p(m + Math.cos(t) * moonR, Math.sin(t) * moonR));
    hole.push(p(m + moonR * 0.12 + Math.cos(t) * moonR * 0.56, Math.sin(t) * moonR * 0.56));
  }
  const c = g.cur,
    bb = bounds(moon);
  g.touch(bb.x0 - 2, bb.y0 - 2, bb.x1 + 2, bb.y1 + 2);
  c.fillStyle = BIRO;
  c.globalAlpha = 0.9 * alpha;
  c.beginPath();
  moon.forEach(([x, y], i) => (i ? c.lineTo(x, y) : c.moveTo(x, y)));
  c.closePath();
  hole.forEach(([x, y], i) => (i ? c.lineTo(x, y) : c.moveTo(x, y)));
  c.closePath();
  c.fill("evenodd");
  c.globalAlpha = 1;
...
const chain = (g: Gfx, start: P, seed: number) => {
  ...
  const path = smooth(
    [
      start,
      [start[0] + 60, start[1] - 96],
      [start[0] + 190, start[1] - 150],
      [start[0] + 330, start[1] - 112],
      [start[0] + 430, start[1] - 20],
      [start[0] + 520, start[1] + 120],
      [1120, start[1] + 190],
    ],
    false,
    20,
  ...
  while (d < cum[cum.length - 1] - 20) {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      o.push([
        p[0] + t[0] * Math.cos(a) * L + n[0] * Math.sin(a) * Wd,
        p[1] + t[1] * Math.cos(a) * L + n[1] * Math.sin(a) * Wd,
      ]);
      iN.push([
        p[0] + t[0] * Math.cos(a) * L * 0.6 + n[0] * Math.sin(a) * Wd * 0.34,
        p[1] + t[1] * Math.cos(a) * L * 0.6 + n[1] * Math.sin(a) * Wd * 0.34,
      ]);
    }
    // its shadow on the table, down and right
    hatch(
      g,
      o.map(([x, y]) => [x + 5, y + 7] as P),
      -0.95,
      2.6,
      () => true,
      seed + k * 3,
    ...
    hatch(
      ...
      0.6,
      2.8,
      (x, y) => (x - p[0]) * LIGHT[0] + (y - p[1]) * LIGHT[1] < -1 && !(face && inside(iN, x, y)),
      seed + k * 3 + 1,
      0.72,
    );
    stroke(g, o.slice(0, 13), { w: 1.05, op: 0.9, seed: seed + k * 5 + 2, wob: 0.2, retrace: true });
    stroke(g, [...o.slice(12), o[0]], {
      w: 1.05,
      op: 0.9,
      seed: seed + k * 5 + 3,
      wob: 0.2,
...
export const drawPocketWatch = (ctx: Ctx, _frame: number, env: Env) => {
  ...
  const caseF = ring(R_CASE, 120),
    caseB = caseF.map(([x, y]) => [x + TH[0], y + TH[1]] as P);
  ...
  g.group("plain", () => {
    ...
    stroke(
      g,
      ring(R_CASE + 14, 40).map(([x, y]) => [x - 4, y + 3] as P),
      { w: 0.8, op: 0.13, seed: 3, wob: 1.2 },
    );
    ...
    fillShape(g, caseB, PAPER);
    fillShape(g, caseF, PAPER);
    const band: P[] = [...caseF, ...caseB.slice().reverse()];
    void band;
    for (let i = 0; i < 240; i++) {
      const a = (i / 240) * Math.PI * 2,
        f = W(polar(R_CASE, a)),
        b2: P = [f[0] + TH[0], f[1] + TH[1]],
        dx = f[0] - C[0],
        dy = f[1] - C[1],
        l = Math.hypot(dx, dy);
      ...
      stroke(g, [lerpP(f, b2, 0.05), lerpP(f, b2, 1.02)], {
        w: 0.85,
        op: 0.55 + shade * 0.35,
        seed: 400 + i,
        wob: 0.1,
    ...
    {
      const n = caseB.length,
        vis = (i: number) => {
          const f = caseF[i],
            dx = f[0] - C[0],
            dy = f[1] - C[1],
            l = Math.hypot(dx, dy);
          return (dx / l) * TH[0] + (dy / l) * TH[1] > 2;
        };
      let s0 = 0;
      while (vis(s0) || !vis((s0 + 1) % n)) s0 = (s0 + 1) % n; // the first index where the visible arc begins
      const arc: P[] = [];
      for (let k = 1; k <= n && vis((s0 + k) % n); k++) arc.push(caseB[(s0 + k) % n]);
    ...
    const inBez = (x: number, y: number) => {
      const q = Math.hypot(x - C[0], y - C[1]);
      return q > R_BEZ * 0.93 && q < R_CASE * 1.0;
    };
    ...
    for (let i = 0; i < 180; i++) {
      const a = (i / 180) * Math.PI * 2,
        p0 = W(polar(R_CASE, a)),
        p1 = W(polar(R_CASE - 11, a));
      stroke(g, [p0, p1], { w: 0.75, op: 0.35 + 0.5 * (1 - bezelTone(p0[0], p0[1])), seed: 500 + i, wob: 0.05 });
    }
    stroke(g, caseF.slice(0, 61), { w: 1.25, op: 0.9, seed: 510, wob: 0.3, retrace: true, blob: true });
    stroke(g, [...caseF.slice(60), caseF[0], caseF[1]], {
      w: 1.25,
      op: 0.9,
      seed: 511,
      wob: 0.3,
    ...
    stroke(g, [...ring(R_BEZ, 110), ring(R_BEZ, 110)[0], ring(R_BEZ, 110)[1]], {
      w: 1.15,
      op: 0.9,
      seed: 513,
      wob: 0.2,
    ...
    [R_TRACK, R_TRACK2].forEach((rr, k) =>
      stroke(g, [...ring(rr, 110), ring(rr, 110)[0], ring(rr, 110)[1]], {
        w: 0.95,
        op: 0.88,
        seed: 130 + k,
        wob: 0.15,
    ...
    for (let i = 0; i < 60; i++) {
      const a = (i / 60) * Math.PI * 2,
        five = i % 5 === 0;
      stroke(g, [W(polar(R_TRACK - 1, a)), W(polar(five ? R_TRACK2 - 6 : R_TRACK2 + 1, a))], {
        w: five ? 1.3 : 0.85,
        op: 0.9,
        seed: 140 + i,
        wob: 0.04,
    ...
    labels.forEach((t, i) => {
      if (i === 6) return;
      numeral(g, t, (i / 12) * Math.PI * 2, 34, 1000 + i * 20);
    }); // six is taken by the seconds dial
    ...
    hatch(
      ...
      (x, y) => {
        const c0 = W(SUB),
          ...
          dy = y - c0[1],
          l = Math.hypot(dx, dy) || 1;
        return l > R_SUB * 0.72 && (dx / l) * LIGHT[0] + (dy / l) * LIGHT[1] > 0.1;
      },
      150,
      0.7,
    );
    ...
    for (let i = 0; i < 60; i++) {
      const a = (i / 60) * Math.PI * 2,
        c0 = SUB,
        p0 = polar(R_SUB - 1, a),
        p1 = polar(i % 5 ? R_SUB - 6 : R_SUB - 10, a);
      stroke(g, [W([c0[0] + p0[0], c0[1] + p0[1]]), W([c0[0] + p1[0], c0[1] + p1[1]])], {
        w: i % 5 ? 0.6 : 0.9,
        op: 0.85,
        seed: 160 + i,
        wob: 0.02,
    ...
    const sa = 2.35,
      s0 = W(SUB),
      s1 = W([SUB[0] + polar(R_SUB - 8, sa)[0], SUB[1] + polar(R_SUB - 8, sa)[1]]),
      s2 = W([SUB[0] - polar(12, sa)[0], SUB[1] - polar(12, sa)[1]]);
    ...
    const hourA = ((10 + 8 / 60) / 12) * Math.PI * 2,
      minA = (8 / 60) * Math.PI * 2;
    breguet(g, hourA, 138, 15, 0.7, 5.2, 180, 0.16, [5, 6]);
    breguet(g, minA, 206, 13, 0.76, 4.2, 190, 0.16, [6, 7]);
    breguet(g, hourA, 138, 15, 0.7, 5.2, 180);
    breguet(g, minA, 206, 13, 0.76, 4.2, 190);
    fillShape(g, ring(11, 16), BIRO, 0.95);
    fillShape(g, ring(4.5, 12), PAPER);
    fillShape(g, ring(2, 8), BIRO, 0.9);
    ...
    hatch(g, pend, 0.6, 3, (x, _y) => x - W([0, -R_CASE - 10])[0] > -4, 200, 0.7);
    ...
    for (let i = -18; i <= 18; i += 3.2)
      stroke(g, [W([i, -R_CASE - 28]), W([i, -R_CASE - 62])], {
        w: 0.8,
        op: i > 2 ? 0.9 : 0.55,
        seed: 210 + Math.round(i * 10),
        wob: 0.05,
      }); // the knurling
    stroke(g, [...crown, crown[0]], { w: 1.15, op: 0.92, seed: 220, wob: 0.12, retrace: true });
    const bowC: P = [0, -R_CASE - 78],
      bowO = ring(52, 70, bowC).filter((_, i) => i < 30 || i > 40),
      bowI = ring(41, 70, bowC).filter((_, i) => i < 30 || i > 40);
    hatch(
      g,
      [...ring(52, 70, bowC), ...ring(41, 70, bowC).reverse()],
      -0.9,
      3,
      (x, y) => {
        const c0 = W(bowC);
        return x - c0[0] + (y - c0[1]) > -6;
      },
      230,
      0.7,
    );
