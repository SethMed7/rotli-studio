...
import { TAIL, WORD, inkStroke, outlineOf, place, type Nib } from "../styles/lettering";
import { trace } from "../styles/gallery";
...
const card = (ctx: Ctx, cx: number, cy: number, w: number, h: number, deg: number, fill: string, lift = 1) => {
  ...
  ctx.fillStyle = `rgba(70,50,30,${0.07 * lift})`;
  ctx.fillRect(-w / 2 + 7, -h / 2 + 10, w, h);
  ctx.fillStyle = `rgba(70,50,30,${0.09 * lift})`;
...
export const drawSocialCard = (ctx: Ctx, _frame: number, env: Env) => {
  ...
  g.group("ink", () => {
    [1.95, 1, 0].forEach((y, i) =>
      g.pen([place(NIB, [-0.6, y]), place(NIB, [8.9 - y * 0.3, y])], {
        w: 0.45,
        color: LEAD,
        opacity: i === 2 ? 0.5 : 0.3,
        wobble: 0.2,
    ...
    for (let x = -0.3; x < 8.7; x += 0.66)
      g.pen([place(NIB, [x, -0.3]), place(NIB, [x, 2.1])], {
        w: 0.35,
        color: LEAD,
        opacity: 0.18,
        wobble: 0.1,
  ...
  g.group(
    ...
    () => {
      const c = g.cur;
      c.fillStyle = IRON;
      g.touch(40, 60, 700, 300);
      STROKES.forEach((k) => {
        trace(c, outlineOf(k));
        c.fill();
      });
      const d = place(NIB, [2.2, 1.42]);
      c.beginPath();
      c.ellipse(d[0], d[1], 3.8, 3.0, -0.5, 0, Math.PI * 2);
      c.fill();
    },
    { textures: ["draftTooth"] },
  );
  ...
  g.group("ink", () =>
    g.pen(line([64, 458], [470, 452], 3), { w: 1.2, color: IRON, opacity: 0.55, wobble: 0.8, boil: 0, seed: 7 }),
  );
  letter(g, "ILLUSTRATIONS / LOOPS / STICKERS / FILMS", 64, 482, {
    cap: 15,
    color: IRON,
    seed: 5,
    w: 1.6,
  ...
  const L = env.canvas(Math.round(1080 * s), Math.round(1080 * s)),
    S = env.canvas(Math.round(SW * s), Math.round(SH * s));
  PRINTS.forEach((p) => {
    ...
    letter(sg, p.label, SW / 2, M + 1080 + 58, {
      cap: 64,
      color: MUTED,
      seed: 30 + Math.round(p.at[0]),
      w: 6,
      align: "center",
      opacity: 0.95,
    });
    ...
    ctx.save();
    ctx.translate(p.at[0], p.at[1]);
    ctx.rotate((p.turn * Math.PI) / 180);
  ...
  g.group("ink", () => {
    g.pen(
      ...
      { w: 1.6, color: IRON, opacity: 0.85, wobble: 0.5, boil: 0, seed: 71 },
    );
    g.pen(
      [
        [662, 456],
      ...
      { w: 1.6, color: IRON, opacity: 0.85, wobble: 0.3, boil: 0, seed: 72, retrace: false },
    );
  });
  ...
  g.group("ink", () => {
    const star = (cx: number, cy: number, k: number, seed: number) => {
      const pts: P[] = [];
      for (let i = 0; i <= 10; i++) {
      ...
      g.pen(pts, { w: 1.1, color: RED, opacity: 0.7, wobble: 0.4, boil: 0, seed });
    };
    star(604, 64, 12, 81);
  });
