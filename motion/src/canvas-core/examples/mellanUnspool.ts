...
const V0 = 9,
  K = 1.3; // view half-width = V0 + K * tip radius (world px)
const HALF = 540,
  KNEE = 130; // the whole plate, and how softly the camera lands on it
...
const clock = (() => {
  // theta at the tip for each frame, frame 0 = nothing
  const th0 = 0.6,
    th1 = R_LAST / SPIRAL_A,
    out = [0];
  let acc = 0;
...
const view = (r: number) => {
  ...
  const d = raw - (HALF - KNEE);
  return raw - (d * d) / (4 * KNEE);
};
  ...
  shots: [
    {
      ...
      draw: (ctx, f, env) => {
        const rb = ribbon(env.cache, FINE),
          th = clock[f],
          n = rb.th.length;
