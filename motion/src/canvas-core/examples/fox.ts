...
import { blob, bounds, clipped, fillShape, inside, lerpP, mix, resample, smooth } from "../styles/gallery";
...
const ringLen = (s: P[]) =>
  s.reduce((a, p, i) => a + Math.hypot(p[0] - s[(i + 1) % s.length][0], p[1] - s[(i + 1) % s.length][1]), 0);
...
const tear = (shape: P[], amp: number, seed: number, grow = 0): P[] => {
  const closed = [...shape, shape[0]],
    n = Math.max(24, Math.round(ringLen(shape) / 2.6)),
    s = resample(closed, n).slice(0, -1),
    nr = normals(s),
    r = rng(seed);
  return s.map(([x, y], i) => {
    const d = (fractal(seed, x, y, 0.09, 0.09, 3) - 0.5) * amp * 2.2 + (r() - 0.5) * amp * 0.6 + grow;
    return [x + nr[i][0] * d, y + nr[i][1] * d];
  });
};
const cut = (shape: P[], seed: number): P[] => {
  const s = resample([...shape, shape[0]], Math.max(24, Math.round(ringLen(shape) / 4))).slice(0, -1),
    r = rng(seed);
  return s.map(([x, y]) => [x + (r() - 0.5) * 0.7, y + (r() - 0.5) * 0.7]);
};
  ...
  clipped(g, shape, () => {
    for (let i = 0; i < 5; i++)
      fillShape(
        ...
        r() < 0.5 ? mix(color, "#ffffff", 0.18) : mix(color, "#000000", 0.12),
        0.25,
      );
    c.lineCap = "round";
    ...
    for (let i = 0; i < n; i++) {
      const x = b.x0 + r() * (b.x1 - b.x0),
        ...
        a = r() * Math.PI,
        l = 3 + r() * 9;
      c.strokeStyle = r() < 0.55 ? mix(color, "#fff8e8", 0.45) : mix(color, "#1a0e06", 0.3);
      c.globalAlpha = 0.25 + r() * 0.3;
      c.lineWidth = 0.6 + r() * 0.7;
      c.beginPath();
      c.moveTo(x, y);
      c.quadraticCurveTo(
        x + Math.cos(a) * l * 0.5 + (r() - 0.5) * 3,
        y + Math.sin(a) * l * 0.5 + (r() - 0.5) * 3,
        x + Math.cos(a) * l,
        y + Math.sin(a) * l,
      );
      c.stroke();
  ...
  g.group(
    ...
    () =>
      pieces.forEach((p, i) => {
        ...
        fillShape(
          g,
          shapes[i].map(([x, y]) => [x + 3 * l, y + 5 * l] as P),
          SHADOW,
          0.5,
        );
      }),
...
const sky = (g: Gfx) => {
  ...
  layer(g, [
    {
      shape: Array.from(
        { length: 40 },
        (_, i) => [214 + Math.cos((i / 40) * Math.PI * 2) * 84, 206 + Math.sin((i / 40) * Math.PI * 2) * 84] as P,
      ),
      color: "#f3e3bd",
      seed: 5,
      lift: 0.6,
  ...
  layer(g, [
    { shape: blob(190, 190, 26, 18, 6, 0.2, 10, 0.4), color: "#e2cfa3", torn: 1.2, seed: 6, lift: 0.3 },
    { shape: blob(238, 232, 20, 14, 7, 0.25, 10, -0.3), color: "#e2cfa3", torn: 1.2, seed: 7, lift: 0.3 },
  ]);
  ...
  layer(
    ...
    [
      ...
      shape: Array.from({ length: 10 }, (_, k) => {
        const a = -Math.PI / 2 + (k / 10) * Math.PI * 2,
          rr = k % 2 ? 4.5 : 11;
        return [x + Math.cos(a) * rr, y + Math.sin(a) * rr] as P;
      }),
      color: "#f2cf6e",
      seed: 20 + i,
      lift: 0.3,
...
const figure = (g: Gfx) => {
  ...
  const shoulder: P[] = P0([
    ...
    [612, 650],
    [572, 628],
  ]); // light across the shoulder
  // slender forelegs with black stockings only below the wrist, and neat paws
  const leg = (x0: number, y0: number, x1: number, y1: number, w0: number, w1: number): P[] =>
    P0(
      [
        [x0 - w0, y0],
        [x0 + w0, y0],
        [(x0 + x1) / 2 + w0 * 0.8, (y0 + y1) / 2],
        [x1 + w1, y1],
        [x1 - w1, y1],
        [(x0 + x1) / 2 - w0 * 0.85, (y0 + y1) / 2],
      ],
      4,
    );
  const legN = leg(512, 700, 516, 896, 15, 11),
    legF = leg(566, 716, 570, 892, 13, 10);
  const stock = (x1: number, y1: number, w: number, top: number): P[] =>
    P0(
      [
        [x1 - w - 2, top],
        [x1 + w + 2, top + 3],
  ...
  const jaw: P[] = P0([
    ...
    [416, 542],
    [378, 516],
  ]); // the white of the jaw flares back into a pointed cheek tuft
  const brow: P[] = P0([
    [420, 452],
    [470, 422],
    [530, 416],
  ...
  layer(g, [
    { shape: body, color: RUST, torn: 2.2, seed: 310, fibre: 1.2, lift: 1.3 },
    { shape: legF, color: RUST_D, torn: 1.4, seed: 311, lift: 0.9 },
  ]);
  layer(g, [
    { shape: haunch, color: RUST, torn: 2.4, seed: 320, lift: 1.2 },
    { shape: stock(570, 892, 10, 820), color: "#2e211a", torn: 1.1, seed: 322, lift: 0.4 },
  ]);
  layer(g, [
    { shape: haunchShade, color: RUST_D, torn: 2.2, seed: 321, lift: 0.4 },
    { shape: shoulder, color: RUST_L, torn: 2, seed: 323, lift: 0.4 },
  ]);
  ...
  layer(g, [
    { shape: blob(517, 898, 17, 8, 390, 0.12, 12), color: BLACK, torn: 1, seed: 390, lift: 0.4 },
    { shape: blob(572, 896, 15, 7, 391, 0.12, 12), color: "#2e211a", torn: 1, seed: 391, lift: 0.4 },
  ]);
  layer(g, [{ shape: tailFront, color: RUST, torn: 2.6, seed: 303, fibre: 1.2, lift: 1.3 }]);
  layer(g, [
    { shape: tailShade, color: RUST_D, torn: 2.2, seed: 301, lift: 0.5 },
    { shape: tailTip, color: CREAM, torn: 3.2, seed: 302, lift: 0.7 },
  ]);
  layer(g, [
    { shape: earF, color: BLACK, torn: 1.6, seed: 340, lift: 0.8 },
    {
      shape: [
        [534, 420],
        [566, 432],
  ...
  layer(g, [
    { shape: jaw, color: CREAM, torn: 3, seed: 361, lift: 0.6 },
    { shape: brow, color: RUST_L, torn: 1.8, seed: 360, lift: 0.4 },
    { shape: earN, color: RUST, torn: 1.6, seed: 362, lift: 0.9 },
  ]);
  layer(g, [
    { shape: earNin, color: CREAM, torn: 1.8, seed: 370, lift: 0.4 },
    {
      shape: [
        [480, 350],
        [498, 350],
  ...
  layer(g, [
    { shape: blob(372, 494, 13, 10, 380, 0.1, 10, 0.3), color: BLACK, seed: 380, lift: 0.4 },
    {
      shape: P0(
        [
          [382, 508],
  ...
  layer(g, [
    ...
    { shape: blob(447, 467, 2.4, 2, 384, 0.1, 8), color: "#ffffff", seed: 384, lift: 0.1 },
  ]);
};
...
const foreground = (g: Gfx) => {
  ...
  const leaf = (cx: number, cy: number, s: number, a: number, color: string, seed: number): Piece => {
    ...
    return {
      shape: pts.map(
        ([x, y]) => [cx + (x * Math.cos(a) - y * Math.sin(a)) * s, cy + (x * Math.sin(a) + y * Math.cos(a)) * s] as P,
      ),
      color,
      seed,
      lift: 0.6,
  ...
  const r = rng(700);
  layer(g, [
    leaf(250, 960, 34, 0.4, "#b6352a", 701),
    leaf(760, 972, 28, -0.8, "#e0962f", 702),
    leaf(890, 1010, 38, 1.2, "#8f2e1f", 703),
    leaf(140, 1030, 30, -0.3, "#e0962f", 704),
    leaf(640, 1040, 24, 2.2, "#b6352a", 705),
    leaf(990, 900, 22, 0.9, "#c96a2a", 706),
  ]);
  const blades: Piece[] = [];
  for (let i = 0; i < 26; i++) {
    const x = 300 + r() * 560,
