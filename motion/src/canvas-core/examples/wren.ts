...
import { blob, bounds, clamp, clipped, fillShape, lerpP, mix, smooth } from "../styles/gallery";
...
const wet = (
  ...
  pts: P[],
  color: string,
  o: { alpha: number; seed: number; pool?: number; bloom?: P | null; bloomR?: number },
) => {
  const { alpha, seed, pool = 0.55, bloom = null, bloomR = 22 } = o,
    b = bounds(pts);
  g.group("paint", () => {
    g.wash(pts, color, { alpha, seed, dx: 7, dy: 6, shrink: 1.07, rim: true });
    clipped(g, pts, () => {
      const c = g.cur,
        gr = c.createLinearGradient(0, b.y0 + (b.y1 - b.y0) * 0.45, 0, b.y1 + 2);
      gr.addColorStop(0, color + "00");
      gr.addColorStop(
        0.75,
        color +
          Math.round(255 * alpha * pool * 0.6)
            .toString(16)
            .padStart(2, "0"),
      );
      gr.addColorStop(
        1,
        color +
          Math.round(255 * clamp(alpha * pool * 1.3))
            .toString(16)
            .padStart(2, "0"),
      );
      c.fillStyle = gr;
      c.fillRect(b.x0 - 20, b.y0, b.x1 - b.x0 + 40, b.y1 - b.y0 + 20);
    });
    if (bloom) {
      // a backrun: a pale cauliflower where a wetter drop pushed the pigment out to a dark crinkled rim
      const bl = blob(bloom[0], bloom[1], bloomR, bloomR * 0.8, seed + 7, 0.32, 18);
      fillShape(g, bl, PAPER, 0.42);
      const c = g.cur;
      c.strokeStyle = mix(color, INK, 0.25);
      c.globalAlpha = alpha * 0.9;
      c.lineWidth = 1.6;
      c.beginPath();
      bl.forEach(([x, y], i) => (i ? c.lineTo(x, y) : c.moveTo(x, y)));
      c.closePath();
      c.stroke();
      c.globalAlpha = 1;
    }
...
const pen = (g: Gfx, pts: P[], w: number, seed: number, op = 0.95, wob = 0.9) =>
...
const twig = (g: Gfx) => {
  const main = tube(twigLine, 17, 6, true),
    sb = tube(side, 7, 3, true),
    lb = tube(lower, 7, 3, true);
  wet(g, main, UMBER, { alpha: 0.62, seed: 30, pool: 0.8 });
  wet(g, sb, UMBER, { alpha: 0.55, seed: 31 });
  wet(g, lb, UMBER, { alpha: 0.55, seed: 32 });
  ...
  const leaf = (base: P, ang: number, len: number, seed: number) => {
    const r = rng(seed),
      d: P = [Math.cos(ang), Math.sin(ang)],
      n: P = [-d[1], d[0]],
      at = (t: number, s: number): P => [
        base[0] + d[0] * t * len + n[0] * s * len,
        base[1] + d[1] * t * len + n[1] * s * len,
      ];
    const edge = [
      at(0, 0),
      at(0.2, 0.14),
    ...
    return {
      shape,
      mid: [at(0, 0), at(0.55, 0.01 * (r() - 0.5)), at(0.95, 0)] as P[],
      veins: [0.3, 0.5, 0.7].map((t, k) => [at(t, 0), at(t + 0.12, (k % 2 ? 1 : -1) * 0.22)] as P[]),
    };
  };
  const leaves = [
    leaf([884, 540], -1.9, 96, 40),
    leaf([860, 580], -0.25, 110, 41),
    leaf([930, 506], -0.75, 88, 42),
    leaf([1010, 612], -1.1, 80, 43),
    leaf([236, 800], 2.2, 84, 44),
  ];
  ...
  haws.forEach(([x, y], i) =>
    wet(g, blob(x, y, 12, 12.5, 60 + i, 0.06, 12), HAW, { alpha: 0.85, seed: 60 + i, pool: 0.9 }),
  );
  g.group("plain", () =>
    haws.forEach(([x, y], i) => fillShape(g, blob(x - 4, y - 5, 3.4, 2.6, 70 + i, 0.1, 8), PAPER, 0.9)),
  );
  inked(g, () => {
    // the bark: the top edge left open where the light hits, the underside pressed firmly
    const top = twigLine.map(([x, y], i) => [x, y - lerpP([17, 0], [6, 0], i / (twigLine.length - 1))[0]] as P),
      bot = twigLine.map(([x, y], i) => [x, y + lerpP([17, 0], [6, 0], i / (twigLine.length - 1))[0]] as P);
    pen(g, top.slice(0, 3), 1.7, 80, 0.8);
    pen(g, top.slice(3), 1.5, 81, 0.75);
    pen(g, bot.slice(0, 4), 2.6, 82);
    pen(g, bot.slice(3), 2.2, 83);
    pen(
      g,
      side.map(([x, y]) => [x + 4, y + 3] as P),
      1.9,
      84,
    );
    pen(
      g,
      lower.map(([x, y]) => [x + 4, y] as P),
      1.9,
      85,
    );
    ...
    for (let i = 0; i < 16; i++) {
      const t = r(),
        p = lerpP(twigLine[Math.floor(t * 4)], twigLine[Math.floor(t * 4) + 1], (t * 4) % 1);
      pen(
        g,
        [
          [p[0] - 5, p[1] - 2 + r() * 6],
    ...
    leaves.forEach((l, i) => {
      pen(
        g,
        l.shape.filter((_, k) => k % 2 === 0),
        1.3,
        120 + i,
        0.8,
        0.7,
      );
      pen(g, l.mid, 1.1, 130 + i, 0.7, 0.3);
      l.veins.forEach((v, k) => pen(g, v, 0.7, 140 + i * 5 + k, 0.55, 0.2));
    });
    haws.forEach(([x, y], i) => {
      const bl = blob(x, y, 12, 12.5, 60 + i, 0.06, 12);
      pen(g, bl.slice(Math.floor(bl.length * 0.35)).concat(bl.slice(0, 2)), 1.3, 160 + i, 0.85, 0.4);
      fillShape(g, blob(x + 1, y + 11, 2.2, 1.8, 170 + i, 0.1, 6), INK, 0.8);
    });
    pen(
      g,
      [
...
const bird = (g: Gfx) => {
  const body = smooth(BODY, true, 8),
    tail = smooth(TAIL, true, 8),
    wing = smooth(WING, true, 8),
    belly = smooth(BELLY, true, 8);
  ...
  wet(
    ...
    DARKB,
    { alpha: 0.55, seed: 206, pool: 0.2 },
  );

  inked(g, () => {
    // the contour: open where the light eats the edge (crown, back), pressed at the belly and under the tail
    const b = smooth(BODY, true, 6),
      n = b.length;
    pen(g, b.slice(0, Math.round(n * 0.2)), 2.4, 300, 0.9); // crown to nape, light
    ...
    pen(
      g,
      b.slice(Math.round(n * 0.52), Math.round(n * 0.72)).map(([x, y]) => [x + 3, y + 4] as P),
      2.2,
      304,
      0.55,
      1.6,
    ); // a second searching stroke under the belly
    ...
    } // loose feather flicks on the breast
    const t = smooth(TAIL, true, 6),
      m = t.length;
    pen(g, t.slice(0, Math.round(m * 0.55)), 2.4, 310, 0.88);
    pen(g, t.slice(Math.round(m * 0.5), m - 2), 3.6, 311, 0.93);
    for (let i = 0; i < 9; i++) {
      const f = 0.12 + i * 0.1,
        a = lerpP(TAIL[1], TAIL[4], f),
        c = lerpP(TAIL[9], TAIL[6], f);
      pen(
        g,
        [lerpP(a, c, 0.06), lerpP(lerpP(a, c, 0.5), [a[0] - 3, a[1] + 3], 0.3), lerpP(a, c, 0.94)],
        1.5,
        320 + i,
        0.8,
        0.4,
    ...
    const w2 = smooth(WING, true, 6),
      q = w2.length;
    pen(g, w2.slice(Math.round(q * 0.12), Math.round(q * 0.64)), 3.4, 330, 0.93);
    pen(g, w2.slice(Math.round(q * 0.6), Math.round(q * 0.9)), 2.2, 331, 0.8);
    for (let i = 0; i < 7; i++) {
      const x = 548 + i * 9,
        y = 526 + i * 7;
      pen(
    ...
    for (let i = 0; i < sp.length; i++) {
      const [x, y] = sp[i];
      fillShape(g, blob(x, y, 3.2, 2.2, 350 + i, 0.2, 8), PAPER, 0.95);
      pen(
        g,
        [
          [x - 5, y + 3],
    ...
    pen(
      ...
      0.5,
      0.4,
    ); // throat feathers parting as it sings
    // the eye: a bead with a catchlight, half-lidded by the eyestripe
    fillShape(g, blob(432, 461, 7.2, 7, 380, 0.05, 10), INK);
    fillShape(g, blob(429.5, 458.5, 2.2, 2, 381, 0.05, 8), PAPER);
    ...
    [
      ...
      pen(
        ...
        [
          [x0, y0],
          [lerpP([x0, y0], [x1, y1], 0.5)[0] + 2, lerpP([x0, y0], [x1, y1], 0.5)[1]],
          [x1, y1],
        ],
        2.6,
        390 + i,
      ...
      pen(
        ...
        0.9,
        0.2,
      ); // the hind toe grips from behind
    });
  });
};
  ...
  g.group("plain", () => {
    ...
    for (let i = 0; i < 22; i++) {
      const x = 160 + r() * 820,
        y = 280 + r() * 640,
        big = r() < 0.25;
      if (Math.hypot(x - 510, y - 530) < 170) continue;
      fillShape(g, blob(x, y, big ? 3.6 : 1.4 + r(), big ? 3 : 1.2 + r() * 0.8, 20 + i, 0.3, 8), INK, 0.7 + r() * 0.3);
    }
  });
};
