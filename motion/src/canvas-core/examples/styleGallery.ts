...
const TILE = 1000,
  ...
  COLS = 3,
  ROWS = 3;
const GW = COLS * TILE + (COLS + 1) * GUT,
  GH = HEAD + ROWS * (TILE + LAB) + (ROWS + 1) * GUT;
const SHEET = "#eee8dc",
  INKC = "#2a2521",
  MUTED = "#8a7f72";
...
const PLATES: { draw: (ctx: Ctx, f: number, env: Env) => void; style: string; subject: string }[] = [
  ...
  {
    draw: (ctx, f, env) => drawMellan(ctx, f, env, 1),
    style: "SINGLE-LINE ENGRAVING",
    subject: "THE MOON, ONE UNBROKEN SPIRAL",
  },
];
...
export const drawStyleGallery = (ctx: Ctx, _frame: number, env: Env) => {
  ...
  letter(g, "ANIDOODLE  /  NINE STYLES TO CHOOSE FROM", GUT, 52, {
    cap: 46,
    color: INKC,
    seed: 1,
    w: 3.2,
  ...
  letter(g, "EVERY MARK DRAWN IN CODE. EVERY PLATE REBUILDS PIXEL FOR PIXEL FROM ITS OWN SOURCE.", GUT, 118, {
    cap: 19,
    color: MUTED,
    seed: 2,
    w: 1.7,
