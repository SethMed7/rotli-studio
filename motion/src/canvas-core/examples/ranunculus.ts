...
const petal = (g: Gfx, radius: number, angle: number, width: number, depth: number, seed: number, tier: number) => {
  const r = rng(seed),
    ...
    cy = 391 + (r() - 0.5) * 12;
  const place = ([x, y]: P): P => [
    cx + Math.cos(angle) * y - Math.sin(angle) * x,
    cy + (Math.sin(angle) * y + Math.cos(angle) * x) * 0.87,
  ];
  const lip = arc(0, radius - depth * 0.28, width, depth * 0.45, Math.PI, 0, 9).map(([x, y], i): P => [
    x,
    y + Math.sin(i * 1.8 + seed) * depth * 0.045,
  ]);
  const shape = [
    ...lip,
    [width * 0.73, radius - depth * 0.61],
    [width * 0.15, radius - depth],
    [-width * 0.63, radius - depth * 0.7],
  ] as P[];
  const colors = ["#f7ddd2", "#f4cbbb", "#efb8a8", "#e8a491", "#df907f", "#d88071", "#c77364"];
  g.group("paint", () => {
    g.form(shape.map(place), colors[tier], tier < 3 ? "#ce8c8a" : "#ac6667", {
      seed,
      light: [-3.2, -4.6],
      alpha: 0.94,
    });
    g.wash(
      [...lip.slice(1, 8), [0, radius - depth * 0.17]].map((p) => place(p as P)),
      "#fff2dc",
      { seed: seed + 1, alpha: 0.28, dx: -1, dy: -1, shrink: 0.91, rim: false },
    );
  });
  g.group("ink", () => {
    g.pen(lip.map(place), {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      g.pen(line([x * 0.45, radius - depth * 0.69], [x, radius - depth * 0.06], (r() - 0.5) * 7).map(place), {
        seed: seed + 10 + j,
        w: 0.48,
        opacity: 0.17,
        color: "#986b69",
...
const leaf = (g: Gfx, origin: P, direction: number, size: number, seed: number) => {
  const place = ([x, y]: P): P => [
    origin[0] + (x * Math.cos(direction) - y * Math.sin(direction)) * size,
    origin[1] + (x * Math.sin(direction) + y * Math.cos(direction)) * size,
  ];
  ...
  g.group("ink", () => {
    ...
    [
      ...
      g.pen(line([x * 0.56, 0], [x, y], 3).map(place), {
        seed: seed + 3 + i,
        w: 0.55,
        opacity: 0.35,
        boil: 0,
...
export const drawRanunculus = (ctx: Ctx, _frame: number, env: Env) => {
  ...
  rings.forEach((ring, tier) => {
    ...
    for (let i = 0; i < ring.count; i++) {
      ...
      petal(
        g,
        ring.radius * (0.95 + r() * 0.1),
        a,
        ring.width * (0.88 + r() * 0.24),
        ring.depth * (0.92 + r() * 0.16),
        1000 + tier * 500 + i * 25,
        tier,
      );
    }
