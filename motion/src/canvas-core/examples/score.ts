...
export const fixturesScore =
  (fps: number, bpm: number, frames: number) =>
  (sr: number): [Float32Array, Float32Array] => {
    ...
    const note = (t0: number, hz: number, dur: number, gain: number, pan: number, bell = false) => {
      ...
      for (let i = 0; i < len && i0 + i < n; i++) {
        const t = i / sr,
          env = Math.exp(-t * (bell ? 2.2 : 7)) * Math.min(1, t * 400),
          v =
            (Math.sin(2 * Math.PI * hz * t) +
              (bell
                ? 0.5 * Math.sin(2 * Math.PI * hz * 2.76 * t) + 0.25 * Math.sin(2 * Math.PI * hz * 5.4 * t)
    ...
    for (let b = 0; b * beat < frames / fps; b++) {
      note(b * beat, scale[b % scale.length], 0.5, 0.22, 0.35 + 0.3 * (b % 2));
      if (b % 2 === 0) note(b * beat, scale[b % scale.length] / 2, 0.7, 0.16, 0.5);
    }
