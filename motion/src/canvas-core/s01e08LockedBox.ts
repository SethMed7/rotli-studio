...
const fade = (ctx: Ctx, a: number, fn: () => void) => {
  if (a <= 0) return;
  ctx.save();
...
const onDevice = (ctx: Ctx, cx: number, cy: number, s: number, look = 0) => {
  ...
  ctx.beginPath();
  ctx.arc(-24 + look * 8, -8, 8, 0, 6.29);
  ctx.arc(24 + look * 8, -8, 8, 0, 6.29);
  ctx.fill();
  ctx.strokeStyle = C.night;
  ctx.lineWidth = 4;
  ctx.beginPath();
  ctx.arc(look * 6, 8, 16, 0.2, Math.PI - 0.2);
  ctx.stroke();
  ctx.restore();
};
...
const mark = (ctx: Ctx, cx: number, cy: number, ok: boolean, k = 1, r = 22) => {
  ...
  ctx.translate(cx, cy);
  ctx.scale(k, k);
  ctx.fillStyle = ok ? C.oliveBright : C.badge;
  ctx.beginPath();
  ctx.arc(0, 0, r, 0, 6.29);
  ctx.fill();
  ctx.strokeStyle = C.night;
  ...
  if (ok) {
    ctx.moveTo(-q * 1.1, 0);
    ctx.lineTo(-q * 0.3, q * 0.9);
    ctx.lineTo(q * 1.2, -q * 0.9);
  } else {
    ctx.moveTo(-q, -q);
    ctx.lineTo(q, q);
    ctx.moveTo(q, -q);
...
const burst = (ctx: Ctx, x: number, y: number, k: number) => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    ctx.lineTo(x + Math.cos(a) * (44 + 34 * k), y + Math.sin(a) * (44 + 34 * k));
    ctx.stroke();
  }
  ctx.restore();
};
...
const noEdit = (ctx: Ctx, cx: number, cy: number, s: number, col: string) => {
  ...
  ctx.closePath();
  ctx.fill();
  ctx.fillRect(-3.5 * u, -3.2 * u, 12 * u, 6.4 * u);
  ctx.fillRect(9.5 * u, -3.2 * u, 3 * u, 6.4 * u);
  ctx.restore();
...
const pencil = (ctx: Ctx, x: number, y: number, f: number) => {
  ctx.save();
  ctx.translate(x, y);
  const o = { w: 3.5, color: C.nightText, frame: f, wob: 0.4, boil: 0.3 };
  ink(
    ctx,
    [
      [0, 0],
...
const arrow = (ctx: Ctx, f: number, x0: number, y0: number, x1: number, y1: number, k: number, seed: number) => {
  ...
  ink(
    ...
    [
      [x0, y0],
      [lerp(x0, x1, 0.5), lerp(y0, y1, 0.5) - 10],
      [x1, y1],
    ],
    { w: 5, color: C.oliveBright, seed, frame: f, progress: k, dash: [12, 9], wob: 0.4 },
  );
  if (k >= 1) {
    const a = Math.atan2(y1 - y0 + 10, x1 - x0);
    ctx.save();
    ctx.translate(x1, y1);
    ctx.rotate(a);
...
const knockMarks = (ctx: Ctx, x: number, y: number, k: number, f: number) => {
  ...
  [0, 1].forEach((i) =>
    ink(
      ...
      { w: 4, color: C.nightText, seed: 400 + i, frame: f, alpha: a, wob: 0.3 },
    ),
  );
};
...
const drips = (ctx: Ctx, f: number, x0: number, x1: number, y: number, n = 9) => {
  ctx.save();
  ctx.strokeStyle = C.nightMuted;
  ctx.lineWidth = 3;
  ctx.lineCap = "round";
  for (let i = 0; i < n; i++) {
    const x = x0 + ((x1 - x0) * (i + 0.5)) / n,
      ph = (f * 2.2 + i * 37) % 70;
    ctx.globalAlpha = 0.6 * (1 - ph / 70);
    ctx.beginPath();
    ctx.moveTo(x, y + 6 + ph * 1.6);
    ctx.lineTo(x, y + 20 + ph * 1.6);
    ctx.stroke();
  }
  ctx.restore();
};
...
const windowShot = (ctx: Ctx, s: S, inside: () => void) => {
  const f = s.f,
    o = { w: 4, color: LINE(), frame: f, wob: 0.8 },
    x1 = WN.x + WN.w,
    y1 = WN.y + WN.h;
  ...
  [
    ...
    ink(
      ...
      { w: 3, color: SOFT(), seed: 10 + i, frame: f, alpha: 0.5 },
    ),
  );
...
const cold: Scene = {
  ...
  draw: (ctx, env, s) => {
    ...
    windowShot(ctx, s, () => {
      ...
      card(ctx, {
        x: 1190,
        y: 620 + Math.sin(s.f / 17) * 6,
        rot: -0.07 + Math.sin(s.f / 23) * 0.03,
        scale: 1.4,
        title: "Passport",
        lines: 2,
        seed: 71,
...
const chapter: Scene = {
  ...
  len: 120,
  draw: (ctx, env, s) =>
    chapterCard(ctx, env, s, { no: 8, title: ["The locked box."], pose: "stays_local", line: true }),
};

// the house, seen from the rain: the window shows the room, the door faces the weather
const HS = { x0: 260, x1: 1260, top: 420, gy: 840 },
  HW = { x: 340, y: 480, w: 660, h: 290 },
  DOOR = { x: 1110, y: 600, w: 120 };
const KNOCK = [
  { t0: 96, hit: 128, y: 650 },
...
const house = (ctx: Ctx, s: S, inside: () => void) => {
  ...
  ink(
    ...
    { ...o, seed: 33, wob: 0.4 },
  ); // the chimney, standing on the roof
  ...
  ink(
    ctx,
    [
      [HW.x - 30, HW.y + HW.h + 12],
      [HW.x + HW.w + 30, HW.y + HW.h + 12],
    ],
    { ...o, w: 6, seed: 35 },
  );
  ...
  ctx.fillStyle = C.nightText;
  ctx.beginPath();
  ctx.arc(DOOR.x + 24, 730, 6, 0, 6.29);
  ctx.fill();
};
const secure: Scene = {
  id: "secure",
  ...
  draw: (ctx, env, s) => {
    const pose = poseOf(s.l, [
      [0, "notes"],
      [SECURE_AT, "stays_local"],
      [KNOCK[0].hit, "listening"],
      [refuseAt(KNOCK[2]) + 30, "stays_local"],
      [350, "celebrating"],
    ]);
    const read = easeOut(s.t(READ_AT, READ_AT + 14)),
      chipK = pop(s.t(CHIP_AT, CHIP_AT + 14));
    house(ctx, s, () => {
      lq(ctx, s, pose, 460, 860, 380);
      card(ctx, {
        x: 680,
        y: 604 + Math.sin(s.f / 19) * 4,
        rot: -0.05,
        scale: 1.1,
        title: "Passport",
        lines: 2,
      ...
      if (s.l >= SECURE_AT && s.l < SECURE_AT + 16)
        sparkle(ctx, 770, 660, 20 * (1 - s.t(SECURE_AT, SECURE_AT + 16)), C.oliveBright, s.l / 4);
      const pk = pop(s.t(SECURE_AT + 10, SECURE_AT + 22));
      if (pk > 0) {
        ctx.save();
        ctx.translate(575, 716);
        ctx.scale(pk, pk);
    ...
    text(ctx, "on-device model", 910, 812, {
      size: 26,
      weight: 600,
      align: "center",
      color: C.oliveBright,
      alpha: s.t(CHIP_AT + 8, CHIP_AT + 20),
    });
    // the door's lock: it arrives with the secure mark and jolts at each knock
    const jolt = KNOCK.reduce(
      (a, k) =>
        Math.max(
          a,
          [0, 8, 16].reduce(
            (b, d) => Math.max(b, s.l >= k.hit + d && s.l < k.hit + d + 6 ? 1 - (s.l - k.hit - d) / 6 : 0),
            0,
          ),
        ),
      0,
    );
    const lk = backOut(s.t(SECURE_AT, SECURE_AT + 14));
    if (lk > 0) lockIcon(ctx, DOOR.x + DOOR.w / 2 + jolt * 3, 680, 30 * lk, C.oliveBright);
    // remote models: each cloud floats to the door, knocks three times, and is turned away
    KNOCK.forEach((k, i) => {
      if (s.l < k.t0) return;
      const ref = refuseAt(k),
        X = 1390;
      let x: number,
        y: number,
        a = 1;
      if (s.l < k.hit) {
        const e = easeOut(s.t(k.t0, k.hit));
        x = lerp(2140, X, e);
        y = lerp(k.y - 220, k.y, e);
      } else if (s.l < ref) {
        const d = (s.l - k.hit) % 8;
        x = X - (s.l - k.hit < 24 ? Math.sin((d / 8) * Math.PI) * 26 : 0);
        y = k.y;
      } else {
        const e = easeIn(s.t(ref + 10, ref + 70));
        x = X + 30 * easeOut(s.t(ref, ref + 10)) + 560 * e;
        y = k.y + Math.sin((s.l - ref) / 7) * 8 - 90 * e;
        a = 1 - e;
      }
      fade(ctx, a, () =>
        cloud(ctx, x, y, 0.9, { frame: s.f, seed: 60 + i, face: s.l >= ref && s.l < ref + 50 ? "dizzy" : "curious" }),
      );
      ...
      if (i === 0)
        text(ctx, "remote model", x, y + 96, {
          size: 26,
          weight: 600,
          align: "center",
          color: C.nightMuted,
          alpha: s.t(k.t0 + 14, k.t0 + 26) * a,
        });
    });
    lowerThird(
      ctx,
      ...
      24,
      228,
      "Mark a note secure and Rotli moves it to wiki/_secure/",
      "and keeps it away from remote models and web lookups.",
      true,
    );
    lowerThird(
      ctx,
      ...
      236,
      s.len,
      "Remote models never see secure content;",
...
const WIN = { x: 90, y: 64, w: 1160, h: 740 },
  SIDE = 300,
  EX = WIN.x + SIDE + 64,
  PCX = 670,
  PCY = 430;
...
const onScreen = (s: S, x: number, y: number): P => {
  const k = zoom(s);
  return [PCX + (x - PCX) * k, PCY + (y - PCY) * k];
};
const R = () => T.roles;
/** the sidebar row marks: a lock on secure notes, a no-edit pencil on locked ones */
const rowMark = (ctx: Ctx, i: number, kind: "secure" | "locked", k = 1) => {
  if (k <= 0) return;
  const x = WIN.x + SIDE - 36,
    y = WIN.y + 192 + i * 46 - 8;
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(k, k);
  if (kind === "secure") lockIcon(ctx, 0, 0, 20, R().accent);
  else noEdit(ctx, 0, -2, 22, R().accent);
  ctx.restore();
};
...
const lockToggle = (ctx: Ctx, on: boolean) => {
  const b = LOCK_BTN;
  fillRR(ctx, b.x, b.y, b.w, b.h, 12, on ? R().accent : R().surface, on ? R().accent : R()["text-muted"], 2);
  noEdit(ctx, b.x + 32, b.y + 24, 22, on ? R()["on-accent"] : R()["text-muted"]);
  text(ctx, "Lock", b.x + 60, b.y + 34, { size: 26, weight: 600, color: on ? R()["on-accent"] : R().text });
};
...
const KEY = "sk-test-XXXXXXXXXXXX",
  PRE = "Booking key: ",
  PASTE = 88,
  CAUGHT = 112,
  SH = { cloud: 176, hit: 206, chip: 236 };
const shapes: Scene = {
  id: "shapes",
  len: 330,
  draw: (ctx, env, s) => {
    const caught = s.l >= CAUGHT,
      ck = pop(s.t(CAUGHT, CAUGHT + 12));
    ctx.save();
    push(ctx, PCX, PCY, zoom(s));
    appFrame(ctx, T, WIN, { notes: NOTES, active: 2, side: SIDE });
    rowMark(ctx, 1, "secure");
    rowMark(ctx, 2, "secure", ck);
    heading(ctx, T, EX, 222, "Ferry booking", 50);
    if (ck > 0) {
      ctx.save();
      ctx.translate(EX + measure(ctx, "Ferry booking", 50, 600) + 40, 206);
      ctx.scale(ck, ck);
      lockIcon(ctx, 0, 0, 30, R().accent);
      ctx.restore();
    }
    fade(ctx, ck, () =>
      chip(ctx, EX, 250, "wiki/_secure/", {
        size: 26,
        mono: true,
        bg: R()["surface-2"],
        fg: R().text,
    ...
    para(ctx, T, EX, 360, "Morning ferry, one seat");
    para(ctx, T, EX, 416, "Two bikes on the deck");
    const y = 472,
      kx = EX + measure(ctx, PRE, 30) + 14,
      kw = measure(ctx, KEY, 30, 500, FONT.mono);
    if (s.l >= 40) para(ctx, T, EX, y, PRE);
    if (caught) {
      const e = easeOut(s.t(CAUGHT, CAUGHT + 8));
      fillRR(ctx, kx - 8, y - 32, (kw + 16) * e, 44, 8, R().tint, R().accent, 2);
    }
    if (s.l >= PASTE) text(ctx, KEY, kx, y, { size: 30, font: FONT.mono, color: R().text });
    if (s.l >= 60 && s.l < CAUGHT && Math.floor(s.f / 8) % 2 === 0) {
      ctx.fillStyle = R().accent;
      ctx.fillRect((s.l >= PASTE ? kx + kw : kx) + 2, y - 26, 3, 32);
    }
    // the pointer places the caret at the end of the line, then rests out of the way
    const k1 = easeInOut(s.t(20, 56)),
      k2 = easeInOut(s.t(130, 160));
    let px = lerp(1150, kx + 6, k1),
      py = lerp(700, y - 10, k1);
    px = lerp(px, 1080, k2);
    py = lerp(py, 690, k2);
    px += Math.sin(s.f / 19) * 6 * s.t(160, 190);
    pointer(ctx, px, py, press(s.l, [58]));
    ctx.restore();
    // ⌘V, pressed
    const kp = s.l >= PASTE - 8 && s.l < PASTE + 8 ? 1 - Math.abs(s.l - PASTE) / 8 : 0;
    fade(ctx, s.t(62, 72) * (1 - s.t(120, 132)), () => keys(ctx, 1440, 140, ["⌘", "V"], kp, 64));
    ...
    if (s.l >= SH.cloud) {
      ...
      let x: number,
        ...
        a = 1;
      if (s.l < ref) {
        const e = easeIn(s.t(SH.cloud, ref));
        x = lerp(2120, ex + 110, e);
        y2 = lerp(ey - 160, ey, e);
      } else {
        const e = easeOut(s.t(ref, ref + 30));
        x = ex + 110 + 220 * e;
        y2 = ey + Math.sin((s.l - ref) / 7) * 8 + 40 * e;
        a = 1 - s.t(ref + 60, ref + 100);
      }
      burst(ctx, ex + 10, ey, s.t(ref, ref + 14));
      fade(ctx, a, () => {
        cloud(ctx, x, y2, 0.9, { frame: s.f, seed: 64, face: s.l >= ref && s.l < ref + 50 ? "dizzy" : "curious" });
        if (s.l >= ref) mark(ctx, x, y2 - 104, false, pop(s.t(ref, ref + 12)));
      });
    }
    const cp = pop(s.t(SH.chip, SH.chip + 14)),
      rd = easeOut(s.t(SH.chip + 14, SH.chip + 28));
    if (cp > 0) {
      onDevice(ctx, 1560, 760 + Math.sin(s.f / 12) * 3, 0.7 * cp, -rd);
      text(ctx, "on-device model", 1560, 870, {
        size: 26,
        weight: 600,
        align: "center",
        color: C.oliveBright,
        alpha: s.t(SH.chip + 8, SH.chip + 20),
      });
    }
    arrow(ctx, s.f, 1490, 720, ex + 16, 640, rd, 43);
    mark(ctx, 1620, 690, true, pop(s.t(SH.chip + 24, SH.chip + 36)), 18);
    ...
    lowerThird(
      ...
      CAUGHT - 6,
      s.len,
      "Rotli also recognizes common secret shapes",
      "(API keys, private keys, card and identity numbers) and treats them the same way.",
      true,
    );
  },
};
...
const lock: Scene = {
  ...
  len: 330,
  draw: (ctx, env, s) => {
    const on = s.l >= LK.click + 4,
      nk = pop(s.t(LK.click + 4, LK.click + 16));
    ctx.save();
    push(ctx, PCX, PCY, zoom(s));
    appFrame(ctx, T, WIN, { notes: NOTES, active: 0, side: SIDE });
    rowMark(ctx, 1, "secure");
    rowMark(ctx, 2, "secure");
    rowMark(ctx, 0, "locked", nk);
    lockToggle(ctx, on);
    heading(ctx, T, EX, 222, "Trip ideas", 50);
    if (nk > 0) {
      ctx.save();
      ctx.translate(EX + measure(ctx, "Trip ideas", 50, 600) + 44, 204);
      ctx.scale(nk, nk);
      noEdit(ctx, 0, 0, 30, R().accent);
      ctx.restore();
    }
    task(ctx, T, EX, 320, "x", "book the ferry");
    task(ctx, T, EX, 384, "/", "find the pink lake");
    task(ctx, T, EX, 448, " ", "pack a snorkel");
    para(ctx, T, EX, 540, "Sunset at the lighthouse", { muted: true });
    // the pointer clicks Lock, then drifts off the page
    const k1 = easeInOut(s.t(14, 52)),
      k2 = easeInOut(s.t(80, 110)),
      bx = LOCK_BTN.x + 90,
      by = LOCK_BTN.y + 30;
    let px = lerp(980, bx, k1),
      py = lerp(640, by, k1);
    px = lerp(px, 1000, k2);
    py = lerp(py, 660, k2);
    px += Math.sin(s.f / 17) * 8 * s.t(110, 140);
    pointer(ctx, px, py, press(s.l, [LK.click]));
    ctx.restore();
    // every model can still read it: the chip and a remote cloud, each with a tick
    const [ex] = onScreen(s, WIN.x + WIN.w, 0);
    const CH = { x: 1600, y: 300 },
      CL = { x: 1620, y: 620 };
    const cp = pop(s.t(LK.readChip - 16, LK.readChip - 4)),
      cl = easeOut(s.t(LK.readCloud - 30, LK.readCloud - 6));
    const bump = (e: readonly number[]) =>
      s.l >= e[1] && s.l < e[1] + 10 ? Math.sin(((s.l - e[1]) / 10) * Math.PI) : 0;
    if (cp > 0) {
      onDevice(
        ctx,
        CH.x + bump(LK.edit1) * 8,
        CH.y + Math.sin(s.f / 12) * 3,
        0.75 * cp,
        -easeOut(s.t(LK.readChip, LK.readChip + 12)),
      );
      text(ctx, "on-device model", CH.x, CH.y + 110, {
        size: 26,
        weight: 600,
        align: "center",
        color: C.oliveBright,
        alpha: s.t(LK.readChip - 8, LK.readChip + 4),
      });
    }
    if (cl > 0) {
      const cx = lerp(2140, CL.x, cl),
        cy = CL.y + Math.sin(s.f / 14) * 6;
      cloud(ctx, cx, cy, 0.9, {
        frame: s.f,
        seed: 66,
        face: s.l >= LK.edit2[1] && s.l < LK.edit2[1] + 40 ? "dizzy" : "curious",
      });
      text(ctx, "remote model", cx, cy + 96, {
        size: 26,
        weight: 600,
        align: "center",
        color: C.nightMuted,
        alpha: s.t(LK.readCloud - 10, LK.readCloud + 2),
      });
    }
    ...
    mark(ctx, CH.x + 66, CH.y - 70, true, pop(s.t(LK.readChip + 12, LK.readChip + 24)), 18);
    mark(ctx, CL.x + 80, CL.y - 80, true, pop(s.t(LK.readCloud + 12, LK.readCloud + 24)), 18);
    ...
    const edit = (e: readonly number[], fromX: number, fromY: number, toY: number, seedMark: number) => {
      if (s.l < e[0]) return;
      const back = easeOut(s.t(e[1], e[1] + 16)),
        gone = s.t(e[1] + 50, e[1] + 64);
      const x = s.l < e[1] ? lerp(fromX, ex + 14, easeIn(s.t(e[0], e[1]))) : ex + 14 + 70 * back,
        y = (s.l < e[1] ? lerp(fromY, toY, easeInOut(s.t(e[0], e[1]))) : toY) + Math.sin(s.l / 5) * 3 * back;
      fade(ctx, 1 - gone, () => {
        pencil(ctx, x, y, s.f);
        if (s.l >= e[1]) mark(ctx, x + 50, y + 46, false, pop(s.t(e[1], e[1] + 12)));
      });
      burst(ctx, ex + 8, toY, s.t(e[1], e[1] + 14));
      void seedMark;
    };
    edit(LK.edit1, CH.x - 110, CH.y + 30, 380, 1);
    edit(LK.edit2, CL.x - 130, CL.y + 40, 770, 2);
    ...
    lowerThird(
      ...
      24,
      s.len,
      "Want AI to stop touching a note? Lock it.",
      "Every model can still read it; none can edit it.",
      true,
    );
  },
};
...
const payoff: Scene = {
  ...
  draw: (ctx, env, s) => {
    ...
    const cx = lerp(2140, 1650, easeOut(s.t(20, 90))) + 420 * easeIn(s.t(150, 240)),
      cy = 560 + Math.sin(s.f / 14) * 8 - 60 * s.t(150, 240);
...
const end: Scene = {
  id: "end",
  len: 120,
  draw: (ctx, env, s) => storyEnd(ctx, env, s, { next: "Other Shores", pose: "waving", line: true }),
};

const SCENES = [cold, chapter, secure, shapes, lock, payoff, end];
const SE = sceneStart(SCENES, "secure"),
  SP = sceneStart(SCENES, "shapes"),
  LO = sceneStart(SCENES, "lock"),
  PA = sceneStart(SCENES, "payoff");
export const s01e08LockedBox: Film = story({
  id: "s01e08LockedBox",
  no: 8,
  ...
  score: {
    ...
    pops: [
      ...
      [SE + CHIP_AT, 81],
      [SE + READ_AT + 10, 88],
      ...KNOCK.flatMap((k) => [0, 8, 16].map((d) => [SE + k.hit + d, 72] as [number, number])),
      ...KNOCK.map((k) => [SE + refuseAt(k), 79] as [number, number]),
      [SP + PASTE, 84],
      [SP + CAUGHT, 88],
      [SP + SH.hit, 79],
      [SP + SH.chip + 24, 86],
      [LO + LK.click, 84],
      [LO + LK.readChip + 12, 86],
      [LO + LK.readCloud + 12, 88],
      [LO + LK.edit1[1], 79],
      [LO + LK.edit2[1], 79],
      [PA + 120, 91],
    ],
...
const CROP = {
  cold: { x: 420, y: 280, w: 1080, h: 640 },
  secure: { x: 240, y: 150, w: 1400, h: 800 },
  shapes: { x: 380, y: 125, w: 1320, h: 750 },
  lock: { x: 380, y: 30, w: 1440, h: 820 },
};
export const s01e08Derive: DeriveSpec = {
  no: 8,
  series: "season-one",
  line: true,
  label: "Rotli · Season One · 08",
  vertical: [
    {
      frame: 30,
      len: 180,
      crop: CROP.cold,
      title: "Rain at night.",
      sub: "Some things stay in the house.",
      hi: "night",
    },
    {
      frame: SE + 30,
      len: 240,
      crop: CROP.secure,
      title: "Mark a note secure.",
      sub: "Remote models never see secure content.",
      hi: "secure",
    },
    {
      frame: SP + 60,
      len: 210,
      crop: CROP.shapes,
      title: "Common secret shapes.",
      sub: "API keys, private keys, card and identity numbers.",
      hi: "secret",
    },
    {
      frame: LO + 40,
      ...
      crop: CROP.lock,
      title: "Lock it.",
      sub: "Every model can still read it; none can edit it.",
      hi: "Lock",
    },
  ],
  slides: [
