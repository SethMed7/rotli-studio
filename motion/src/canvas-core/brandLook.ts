...
export const brandLook: Film = {
  meta: { title: "brandLook", W: 1920, H: 1080, fps: 30, bpm: 120, durationFrames: 15, raster: "cpu" },
  ...
  shots: [
    {
      ...
      draw: (ctx, l, env) => {
        ...
        const size = 96,
          maxW = 980,
          words: string[] = [];
        for (const w of (BRAND.tagline || BRAND.product).split(/\s+/)) {
          const cur = words[words.length - 1];
          if (cur && measure(ctx, `${cur} ${w}`, size, 600, FONT.display, size * TRACK) <= maxW && !cur.endsWith("."))
            words[words.length - 1] = `${cur} ${w}`;
          else words.push(w);
        }
        words.splice(3);
        words.forEach((w, i) =>
          text(ctx, w, 110, 330 + i * 108, {
            size,
            weight: 600,
            font: FONT.display,
            color: C.cocoa,
        ...
        ink(
          ctx,
          [
            [110, 330 + (words.length - 1) * 108 + 24],
            [
              110 + Math.min(900, measure(ctx, last, size, 600, FONT.display, size * TRACK)),
              330 + (words.length - 1) * 108 + 20,
            ],
          ],
          { w: 9, color: C.clay, seed: 3, frame: l },
        );
        ...
        ctx.fillRect(1180, 0, 740, 620);
        const t = THEME_LIST[0].roles;
        fillRR(ctx, 1240, 70, 620, 330, 22, t.surface, t.border, 3);
        text(ctx, "A note, in this brand", 1280, 140, {
          size: 38,
          weight: 600,
          font: FONT.display,
          color: t.text,
        ...
        ["Plain text underneath", "The accent marks what matters", "Muted for the quiet parts"].forEach((s, i) => {
          fillRR(
            ctx,
            1280,
            180 + i * 62,
            ...
            30,
            8,
            i === 0 ? t.success : t.surface,
            i === 0 ? t.success : t["text-muted"],
            3,
          );
          text(ctx, s, 1328, 205 + i * 62, { size: 28, color: i === 2 ? t["text-muted"] : t.text });
        });
        ctx.fillStyle = t.accent;
        ctx.fillRect(1280, 372, 190, 4);
        [C.linen, C.surface2, C.peach, C.clay, C.cocoa, C.night].forEach((c, i) => {
          fillRR(ctx, 1240 + i * 104, 450, 88, 88, 44, c, C.cocoa, 3);
        });
        text(ctx, "ground · band · tint · accent · text · deep", 1240, 580, { size: 22, color: C.muted });
        ctx.fillStyle = C.night;
        ctx.fillRect(1180, 620, 740, 460);
