...
const WIN = { x: 100, y: 130, w: 1090, h: 700 },
  SIDE = 260,
  EX = WIN.x + SIDE + 64,
  COL = 1380,
  BX = 1600,
  CHIP = { x: COL, y: 262, s: 0.85 };
// cue table (frames)
const Q = {
  d: [105, 125, 145],
  strike: [150, 168, 186],
...
const camera = (ctx: Ctx, env: Env, l: number) => {
  ...
  for (let i = 0; i < CAM.length - 1; i++)
    if (l >= CAM[i][0] && l < CAM[i + 1][0]) {
      const a = CAM[i],
        b = CAM[i + 1],
        k = easeInOut(seg(l, a[0], b[0]));
      c = [lerp(a[1], b[1], k), lerp(a[2], b[2], k), lerp(a[3], b[3], k)];
      break;
    }
  const [s, tx, ty] = c;
...
const mark = (ctx: Ctx, cx: number, cy: number, ok: boolean, r = 14) => {
  ctx.save();
  ctx.fillStyle = ok ? C.olive : C.badge;
  ctx.beginPath();
  ...
  const q = r * 0.4;
  if (ok) {
    ctx.moveTo(cx - q * 1.1, cy);
    ctx.lineTo(cx - q * 0.3, cy + q * 0.9);
    ctx.lineTo(cx + q * 1.2, cy - q * 0.9);
  } else {
...
const pill = (ctx: Ctx, cx: number, y: number, label: string, ok: boolean, p: number) => {
  if (p <= 0) return;
  const w = measure(ctx, label, 26, 600) + 84;
  ctx.save();
  ctx.translate(cx, y + 22);
  ctx.scale(p, p);
  fillRR(ctx, -w / 2, -22, w, 44, 22, C.nightSurface2, C.nightBorder, 2);
  mark(ctx, -w / 2 + 28, 0, ok);
  text(ctx, label, -w / 2 + 54, 9, { size: 26, weight: 600, color: C.nightText });
  ctx.restore();
};
...
const noEdit = (ctx: Ctx, cx: number, cy: number, s: number, col: string) => {
  ...
  const u = s / 22; // pencil: tip bottom-left, body up-right
  ctx.beginPath();
  ctx.moveTo(-11 * u, 0);
...
const pencil = (ctx: Ctx, x: number, y: number) => {
  ...
  ctx.closePath();
  ctx.fill();
  fillRR(ctx, 20, -10, 58, 20, 3, C.clay, C.ink, 3);
  fillRR(ctx, 78, -10, 14, 20, 4, C.lake, C.ink, 3);
  ctx.restore();
};
...
const arrow = (ctx: Ctx, f: number, y: number, k: number, col: string) => {
  ...
  ink(
    ...
    { w: 5, color: col, seed: 800 + y, frame: f, progress: k, dash: [12, 9], wob: 0.4 },
  );
  if (k >= 1) {
    ctx.fillStyle = col;
    ctx.beginPath();
    ctx.moveTo(x1, y);
    ctx.lineTo(x1 + 18, y - 12);
    ctx.lineTo(x1 + 18, y + 12);
    ctx.closePath();
    ctx.fill();
  }
...
const toggle = (
  ...
  label: string,
  on: boolean,
  icon: (cx: number, cy: number, col: string) => void,
) => {
  const w = measure(ctx, label, 26, 600) + 66,
    fg = on ? R["on-accent"] : R.text;
  fillRR(ctx, x, 142, w, 46, 12, on ? R.accent : R.surface, on ? R.accent : R["text-muted"], 2);
  icon(x + 26, 166, on ? R["on-accent"] : R["text-muted"]);
...
const BOUND: P[] = (() => {
  ...
  const edge = (ax: number, ay: number, bx: number, by: number) => {
    ...
    for (let i = 0; i < n; i++) pts.push([lerp(ax, bx, i / n), lerp(ay, by, i / n)]);
  };
...
const DLG = [
  { t: "Create an account", s: "Sign up to continue", x: 470, y: 230 },
  { t: "Allow tracking?", s: "Help us with analytics", x: 560, y: 420 },
  { t: "Send crash report?", s: "Upload it to our servers", x: 650, y: 610 },
];
const ROWS = [
  { t: "Off", s: "Leave it off" },
  { t: "On your Mac", s: "Run a model on your Mac" },
  { t: "Your tools", s: "Connect the tools you already use" },
];
...
const title = (ctx: Ctx, env: Env, l: number) =>
  titleCard(ctx, env, l, l, { no: 7, title: TITLE_LINES, pose: "stays_local", dark: true });
const demo = (ctx: Ctx, l: number, env: Env) => {
  ...
  if (bk > 0) {
    fillRR(ctx, 60, 76, BX - 60, 784, 40, `rgba(169,183,143,${(0.05 * bk).toFixed(3)})`);
    ink(ctx, BOUND, { w: 5, color: C.oliveBright, seed: 700, frame: l, progress: bk, closed: true, dash: [16, 12] });
    text(ctx, "on this Mac", COL, 134, {
      size: 28,
      weight: 600,
      align: "center",
      color: C.oliveBright,
      alpha: seg(l, Q.bound + 12, Q.bound + 26),
    });
  }
  // the window and whichever view is open
  const view =
    l < Q.settings ? "health" : l < Q.health ? "settings" : l < Q.server ? "health" : l < Q.trip ? "server" : "trip";
  const active = view === "health" ? 0 : view === "server" ? 1 : view === "trip" ? 2 : -1;
  appFrame(ctx, T, WIN, {
    notes: ["Health", "Server setup", "Trip plan"],
    active,
    side: SIDE,
    title:
      view === "settings"
  ...
  const healthSecure = l >= Q.secure,
    serverSecure = l >= Q.secure2,
    tripLocked = l >= Q.lock;
  ...
  if (view !== "settings") {
    ...
    toggle(ctx, 1020, "Secure", (view === "health" && healthSecure) || (view === "server" && serverSecure), (x, y, c) =>
      lockIcon(ctx, x, y + 2, 20, c),
    );
  }
  const headLock = (s: string, at: number, icon: "lock" | "noedit") => {
    const p = pop(seg(l, at, at + 12));
    if (p <= 0) return;
    const x = EX + measure(ctx, s, 50, 600) + 36;
    ctx.save();
    ctx.translate(x, 244);
    ctx.scale(p, p);
    if (icon === "lock") lockIcon(ctx, 0, 0, 30, R.accent);
    else noEdit(ctx, 0, -6, 30, R.accent);
    ctx.restore();
  };
  if (view === "health") {
    heading(ctx, T, EX, 262, "Health", 50);
    ["Allergy: penicillin", "Blood type: O+", "Next check-up: March 12"].forEach((s, i) =>
      para(ctx, T, EX, 345 + i * 56, s),
    );
    if (l >= Q.health) headLock("Health", Q.secure, "lock");
  } else if (view === "settings") {
    heading(ctx, T, EX, 262, "AI", 50);
    ROWS.forEach((row, i) => {
      const a = seg(l, Q.rows[i], Q.rows[i] + 10);
      if (a <= 0) return;
      const x = EX,
        y = 300 + i * 132 + (1 - a) * 16,
        w = 720,
        on = i === 1 && l >= Q.pickMac + 3;
      ctx.save();
      ctx.globalAlpha *= a;
      fillRR(ctx, x, y, w, 116, 16, on ? R.tint : R.surface, on ? R.accent : R.border, on ? 3 : 2);
      fillRR(ctx, x + 22, y + 28, 64, 60, 14, R["surface-2"]);
      const ix = x + 54,
        iy = y + 58;
      if (i === 0) {
        ...
        ctx.lineCap = "round";
        ctx.beginPath();
        ctx.arc(ix, iy + 2, 16, -Math.PI / 2 + 0.7, -Math.PI / 2 - 0.7 + Math.PI * 2);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(ix, iy - 18);
        ctx.lineTo(ix, iy - 2);
      ...
      text(ctx, row.t, x + 110, y + 50, { size: 32, weight: 600, color: R.text });
      text(ctx, row.s, x + 110, y + 90, { size: 26, color: R["text-muted"] });
      ctx.beginPath();
      ctx.arc(x + w - 46, y + 58, 15, 0, 6.29);
    ...
    const y = 405,
      kx = EX + measure(ctx, PRE, 30),
      kw = measure(ctx, KEY, 30, 500, FONT.mono);
    if (l > Q.flag) {
      const k = easeOut(seg(l, Q.flag, Q.flag + 8));
      fillRR(ctx, kx - 8, y - 32, (kw + 16) * k, 44, 8, R.tint, R.accent, 2);
    }
    if (l >= Q.type[0] - 6) {
      para(ctx, T, EX, y, PRE);
      text(ctx, typed(KEY, seg(l, Q.type[0], Q.type[1])), kx, y, { size: 30, font: FONT.mono, color: R.text });
    }
    if (l >= Q.type[0] - 6 && l < Q.flag)
      caret(
        ctx,
        T,
        l < Q.type[0] ? kx : kx + measure(ctx, typed(KEY, seg(l, Q.type[0], Q.type[1])), 30, 500, FONT.mono),
        y,
        l,
        30,
      );
    const fp = pop(seg(l, Q.flag + 4, Q.flag + 16));
    if (fp > 0) {
      const lab = "Looks like an API key",
        w = measure(ctx, lab, 26, 600) + 76;
      ctx.save();
      ctx.translate(kx - 8, 470);
      ctx.scale(fp, fp);
      fillRR(ctx, 0, -24, w, 48, 24, R["surface-2"], R.accent, 2);
      lockIcon(ctx, 30, 2, 20, R.accent);
      text(ctx, lab, 54, 9, { size: 26, weight: 600, color: R.text });
      ctx.restore();
    }
    ...
    task(ctx, T, EX, 345, "x", "book the ferry");
    task(ctx, T, EX, 405, " ", "find the pink lake");
    task(ctx, T, EX, 465, " ", "pack a snorkel");
  ...
  if (cp > 0) {
    const cy = CHIP.y + Math.sin(l / 12) * 3;
    ctx.save();
    ctx.translate(COL, cy);
    ctx.scale(cp, cp);
    onDevice(ctx, 0, 0, CHIP.s);
    ctx.restore();
    text(ctx, "on-device model", COL, 372, {
      size: 26,
      weight: 600,
      align: "center",
      color: C.nightText,
      alpha: seg(l, Q.pickMac + 12, Q.pickMac + 24),
    });
  }
  ...
  if (l >= Q.edit[0]) {
    const [e0, e1] = Q.edit,
      x =
        l < e1
          ? lerp(COL - 80, WIN.x + WIN.w + 14, easeIn(seg(l, e0, e1)))
          : lerp(WIN.x + WIN.w + 14, WIN.x + WIN.w + 34, easeOut(seg(l, e1, e1 + 14)));
    pencil(ctx, x, 292 + (l >= e1 ? Math.sin((l - e1) / 6) * 4 : 0));
    burst(ctx, WIN.x + WIN.w + 6, 292, seg(l, e1, e1 + 14));
    if (l >= e1) mark(ctx, x + 36, 262, false, 16 * pop(seg(l, e1, e1 + 10)));
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (l < hit) {
      ...
      x = lerp(2120, BX + 100, k);
      y = lerp(y0 - 160, y0, k);
    } else {
      const k = easeOut(seg(l, hit, hit + 24));
      x = BX + 100 + 90 * k;
      y = y0 + Math.sin((l - hit) / 9) * 8;
      burst(ctx, BX, y0, seg(l, hit, hit + 14));
    }
    cloud(ctx, x, y, 0.95, { frame: l, seed: 60 + i, face: l >= hit && l < hit + 40 ? "dizzy" : "curious" });
    text(ctx, "remote model", x, y + 100, {
      size: 26,
      weight: 600,
      align: "center",
      color: C.nightMuted,
      alpha: seg(l, t0 + 8, t0 + 20),
    });
    if (l >= hit && l < Q.server)
      pill(
        ctx,
        ...
        "refused",
        false,
        pop(seg(l, hit + 4, hit + 16)) * (1 - seg(l, Q.server - 8, Q.server)),
      );
  }
  ...
  if (qs > 0) {
    ...
    actor(ctx, env, l, { pose: p.pose, x: COL, y: 832, h: 280 * qs, squash: p.squash, shadowCol: "#000" });
  }
  // the nags Rotli never shows: popped, struck through, gone
  DLG.forEach((d, i) => {
    const p = pop(seg(l, Q.d[i], Q.d[i] + 14)),
      fk = easeIn(seg(l, Q.fall + i * 6, Q.fall + i * 6 + 24));
    if (p <= 0 || fk >= 1) return;
    ctx.save();
    ctx.globalAlpha *= 1 - fk;
    ctx.translate(d.x + 310, d.y + 75 + fk * 520);
    ctx.rotate(fk * (i % 2 ? -0.35 : 0.3));
    ctx.scale(p, p);
    ctx.translate(-310, -75);
    fillRR(ctx, 0, 0, 620, 150, 20, C.surface, C.ink, 3);
    text(ctx, d.t, 34, 60, { size: 34, weight: 600, color: C.cocoa });
    text(ctx, d.s, 34, 110, { size: 26, color: C.muted });
    fillRR(ctx, 436, 82, 150, 46, 12, C.blue);
    text(ctx, "Continue", 511, 114, { size: 26, weight: 600, align: "center", color: "#ffffff" });
    ink(
      ctx,
      [
        [18, 48],
      ...
      { w: 9, color: C.clay, seed: 30 + i, frame: l, progress: easeOut(seg(l, Q.strike[i], Q.strike[i] + 10)) },
    );
    ctx.restore();
  });
  if (l >= 250 && l < 720) {
  ...
  const hh = 1 - easeIn(seg(l, 344, 356));
  if (hh > 0) host(ctx, env, l, { pose: "stays_local", dark: true, h: 250 * hh });
  ...
  lowerThird(
    ...
    "An on-device model can still read them.",
    true,
  );
  lowerThird(
    ctx,
    ...
    532,
    642,
    "It recognizes common secret shapes too.",
    "API keys, private keys, card and identity numbers.",
    true,
  );
  lowerThird(
    ctx,
    ...
    652,
    DEMO_END,
    "Locked notes can't be edited by any model.",
    "Every model can still read them.",
    true,
  );
};
...
export const ep07Secure: Film = {
  ...
  shots: [
    ...
    },
    {
      ...
      draw: (c, l, e) => {
        c.setTransform(e.scale, 0, 0, e.scale, 0, 0);
        endCard(c, e, DEMO_END + l, l, { pose: "stays_local" });
      },
    },
  ],
  audio: makeScore({
    ...
    clicks: Array.from({ length: Math.floor((Q.type[1] - Q.type[0]) / 3) }, (_, i) => Q.type[0] + i * 3),
    thumps: [Q.hit[0], Q.hit[1], Q.edit[1]],
  }),
};

// crops are in the rendered episode's screen coordinates (camera applied)
const PUSH = { x: 540, y: 120, w: 1000, h: 600 },
  WIDE = { x: 410, y: 30, w: 1490, h: 830 },
  LOCKED = { x: 410, y: 30, w: 1210, h: 830 };
  ...
  vertical: [
    {
      ...
      sub: "Off, on your Mac, or tools you use.",
      hi: "AI",
    },
    {
      frame: 426,
      len: 90,
      crop: { x: 1196, y: 60, w: 724, h: 800 },
      title: "Remote models: refused.",
      sub: "Secure notes never reach them.",
      hi: "refused",
    },
    {
      frame: 548,
      len: 90,
      crop: { x: 560, y: 110, w: 1000, h: 560 },
      title: "Secret shapes, recognized.",
      sub: "An API key marks its note secure.",
      hi: "recognized",
    },
    {
      frame: 664,
      len: 105,
      crop: LOCKED,
      title: "Locked: no AI edits.",
      sub: "Every model can still read it.",
      hi: "Locked",
    },
  ],
  slides: [
    ...
    { frame: 515, crop: WIDE, title: "Remote models: refused.", sub: "Secure notes never reach them.", hi: "refused" },
    {
      ...
      crop: { x: 560, y: 110, w: 1000, h: 560 },
      title: "Secret shapes, recognized.",
      sub: "API keys, card and identity numbers.",
      hi: "recognized",
    },
