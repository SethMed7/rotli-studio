...
const TOTAL = 900,
  DEMO_END = TOTAL - END,
  R = T.roles;
const WIN = { x: 70, y: 90, w: 1400, h: 760 },
  SIDE = 350,
  EX = WIN.x + SIDE + 64;
// cue table (frames)
const Q = {
...
const BODY = [
  "Software that keeps working when the wifi doesn't.",
  "Local-first software, the essay",
  "Ask Maya what she is reading",
  "Files over apps",
];
const BODY_Y = [400, 452, 504, 556],
  OUT_R = 1300,
  LINE5 = "Read them on the ferry",
  LINE5_Y = 608;
...
const camera = (ctx: Ctx, env: Env, l: number) => {
  ...
  ctx.setTransform(env.scale * s, 0, 0, env.scale * s, -env.scale * ox * s, -env.scale * oy * s);
  drift(ctx, env, l);
};
...
const toggle = (ctx: Ctx, on: number) => {
  ...
  ctx.fillStyle = on > 0.5 ? R["on-accent"] : R["text-muted"];
  ctx.beginPath();
  ctx.arc(x + 20 + (w - 40) * on, y + h / 2, 13, 0, 6.29);
  ctx.fill();
};
...
const library = (ctx: Ctx, l: number) => {
  ...
  ["People", "Projects", "Research"].forEach((n, i) => {
    const y = y0 + 46 + i * 44;
    if (i === 2 && flash) fillRR(ctx, x + 18, y - 30, SIDE - 36, 42, 10, R.tint);
    text(ctx, n, x + 52, y, {
      size: 24,
      color: i === 2 && l >= Q.place ? R.text : R["text-muted"],
      weight: i === 2 && l >= Q.place ? 600 : 500,
    });
  });
  if (l >= Q.place)
    text(ctx, "Local-first reading list", x + 70, y0 + 46 + 3 * 44, {
      size: 24,
      color: R.text,
      alpha: seg(l, Q.place, Q.place + 8),
    });
};
const bullet = (ctx: Ctx, x: number, y: number, s: string) => {
  ctx.fillStyle = R["text-muted"];
  ctx.beginPath();
  ctx.arc(x + 10, y - 10, 5, 0, 6.29);
  ctx.fill();
  text(ctx, s, x + 34, y, { size: 30, color: R.text });
  return x + 34 + measure(ctx, s, 30);
};
/** a filing that pops in around the note */
const popIn = (ctx: Ctx, l: number, at: number, x: number, y: number, draw: () => void) => {
  const p = pop(seg(l, at, at + 14));
  if (p <= 0) return;
  ctx.save();
  ctx.translate(x, y);
  ctx.scale(p, p);
...
const demo = (ctx: Ctx, l: number, env: Env) => {
  ...
  camera(ctx, env, l);
  appFrame(ctx, T, WIN, {
    notes: ["Call with Maya", "Trip plan", "Local-first reading list"],
    active: 2,
    side: SIDE,
    title: "local-first-reading-list.md",
  });
  library(ctx, l);
  ...
  if (l >= Q.h1[0]) {
    const s = typed("Local-first reading list", seg(l, Q.h1[0], Q.h1[1]));
    heading(ctx, T, EX, 290, s, 54);
    if (l < Q.b1[0]) cur = [EX + measure(ctx, s, 54, 600), 290];
  }
  [Q.b1, Q.b2, Q.b3, Q.b4].forEach(([t0, t1], i) => {
    if (l < t0) return;
    const s = typed(BODY[i], seg(l, t0, t1)),
      y = BODY_Y[i];
    const e =
      i === 0 ? (text(ctx, s, EX, y, { size: 30, color: R.text }), EX + measure(ctx, s, 30)) : bullet(ctx, EX, y, s);
    if (l < t1 + 6 && (i === 3 || l < [Q.b2, Q.b3, Q.b4][i][0])) cur = [e, y];
  });
  if (l >= Q.b5[0]) {
    const s = typed(LINE5, seg(l, Q.b5[0], Q.b5[1])),
      e = bullet(ctx, EX, LINE5_Y, s);
    cur = [e, LINE5_Y];
  }
  ...
  popIn(ctx, l, Q.tags, EX, 212, () => {
    let x = EX;
    for (const t of ["#reading", "#local-first", "#software"]) x += tagChip(ctx, T, x, 222, t, 26) + 12;
  });
  popIn(ctx, l, Q.sum, EX, 340, () =>
    text(ctx, "Summary  ·  Reading on local-first software, with Maya's picks.", EX, 344, {
      size: 27,
      color: R["text-muted"],
    }),
  );
  const LY = 690;
  popIn(ctx, l, Q.links, EX, LY, () => {
    text(ctx, "Linked to", EX, LY, { size: 27, color: R["text-muted"] });
    let x = EX + measure(ctx, "Linked to", 27) + 18;
    for (const n of ["Call with Maya", "Trip plan"]) {
      text(ctx, n, x, LY, { size: 27, weight: 600, color: R["accent-text"] });
      const w = measure(ctx, n, 27, 600);
      ctx.fillStyle = R.accent;
      ctx.fillRect(x, LY + 7, w, 3);
      x += w + 28;
    }
  });
  popIn(ctx, l, Q.place, EX, 752, () => {
    const s = "Library  ›  Research",
      w = measure(ctx, s, 26, 600) + 36;
    fillRR(ctx, EX, 716, w, 48, 24, R["surface-2"], R.border, 2);
    text(ctx, s, EX + 18, 749, { size: 26, weight: 600, color: R.text });
  });
  // the note's card flies to its shelf in the Library
  if (l >= Q.fly[0] && l < Q.fly[1] + 2) {
    const k = easeInOut(seg(l, Q.fly[0], Q.fly[1]));
    card(ctx, {
      x: lerp(1120, 250, k),
      y: lerp(290, 626, k) - Math.sin(k * Math.PI) * 160,
      rot: lerp(0.08, 0, k),
      scale: lerp(0.9, 0.45, k),
      title: "Local-first reading list",
      lines: 2,
      seed: 51,
      frame: l,
      dark: true,
  ...
  if (ok > 0) {
    ...
    ctx.lineDashOffset = -l * 0.5;
    ctx.beginPath();
    ctx.roundRect(EX - 24, 356, OUT_R - EX + 24, 222, 18);
  ...
  if (l >= Q.hop[0]) {
    const h = hopAlong(
      ...
      p = poseAt(l, [
        [0, "walking"],
        [Q.hop[1], "knowledge_system"],
        [306, "searching"],
        [Q.place + 4, "knowledge_system"],
        [Q.same, "celebrating"],
        [566, "knowledge_system"],
        [Q.off + 8, "rest"],
      ]);
    actor(ctx, env, l, {
      pose: p.pose,
      ...
      flip: l < Q.hop[1],
      shadowCol: "#000",
      lean: h.moving ? 0 : Math.sin(l / 9) * 1.6,
    });
  }
  if ((l >= 92 && l < 146) || (l >= 612 && l < 690)) {
    const pp = ptrAt(l);
    pointer(ctx, pp.x, pp.y, press(l, Q.on) || press(l, Q.off));
  }
  ...
  lowerThird(
    ...
    474,
    606,
    "It organizes around your writing.",
    "Every note keeps its words exactly as you wrote them.",
    true,
  );
...
export const ep05Librarian: Film = {
  ...
  shots: [
    ...
    },
    {
      ...
      draw: (c, l, e) => {
        c.setTransform(e.scale, 0, 0, e.scale, 0, 0);
        endCard(c, e, DEMO_END + l, l, { pose: "knowledge_system" });
      },
    },
  ],
  audio: makeScore({
    ...
    clicks: [Q.h1, Q.b1, Q.b2, Q.b3, Q.b4, Q.b5].flatMap(([a, b]) =>
  ...
  vertical: [
    {
      frame: 160,
      len: 135,
      crop: { x: 0, y: 0, w: 1420, h: 850 },
      title: "Keep working in Main.",
      sub: "Turn the Librarian on.",
      hi: "Main",
    },
    {
      frame: 330,
      len: 135,
      crop: { x: 0, y: 0, w: 1576, h: 876 },
      title: "Tags, summaries, links.",
      sub: "And a place in your Library.",
      hi: "links",
    },
    {
      frame: 500,
      len: 105,
      crop: { x: 60, y: 20, w: 1300, h: 830 },
      title: "Never rewrites.",
      sub: "Every note keeps its words.",
      hi: "rewrites",
    },
    {
      frame: 630,
      len: 135,
      crop: { x: 420, y: 20, w: 1490, h: 840 },
      title: "Turn it off.",
      sub: "The folder is still a complete workspace.",
      hi: "off",
    },
  ],
  slides: [
