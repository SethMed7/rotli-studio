...
export const mountFilm = (film: Film) => {
  ...
  const ready = (async () => {
    ...
    await Promise.all(Object.entries(film.assets.fonts ?? {}).map(async ([spec, url]) => { const [family, weight = "400"] = spec.split("@"); const f = new FontFace(family, `url(${window.__ASSETS__?.[`font:${spec}`] ?? url})`, { weight }); await f.load(); (document.fonts as unknown as { add(f: FontFace): void }).add(f); }));
